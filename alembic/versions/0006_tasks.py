...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("meeting_id", sa.Text, nullable=True),  # optional link to meeting.id
        sa.Column("action_item_id", sa.Text, nullable=True),  # optional link to action_item.id
        sa.Column("title", sa.Text, nullable=False),
        sa.Column("description", sa.Text, nullable=True),
        sa.Column("status", sa.Text, nullable=False, server_default="open"),  # open|in_progress|blocked|done|cancelled
        sa.Column("assignee", sa.Text, nullable=True),
        sa.Column("priority", sa.Text, nullable=True),  # P0|P1|P2
    ...
    op.create_table(
        ...
        sa.Column("type", sa.Text, nullable=False),  # created|status_changed|assignee_changed|note
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Text, primary_key=True),
        sa.Column("task_id", sa.Text, sa.ForeignKey("task.id", ondelete="CASCADE")),
        sa.Column("depends_on_task_id", sa.Text, sa.ForeignKey("task.id", ondelete="CASCADE"))
    )
    ...
    op.create_table(
        ...
        sa.Column("id", sa.Text, primary_key=True),
        sa.Column("task_id", sa.Text, sa.ForeignKey("task.id", ondelete="CASCADE")),
        sa.Column("type", sa.Text, nullable=False),  # jira|github_issue|github_pr|code
        sa.Column("key", sa.Text, nullable=True),    # e.g., JIRA-123
