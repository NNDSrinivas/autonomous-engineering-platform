...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('role', sa.String(16), nullable=False)  # admin|maintainer|developer|viewer
    )
    ...
    op.create_table(
        ...
        sa.Column('phase_budgets', sa.Text),         # JSON {plan,code,review:{tokens,usd_per_day}}
        ...
        sa.Column('require_review_for', sa.Text)     # JSON array of action kinds [edit,cmd,git,pr,jira]
    )
    ...
    op.create_table(
        'change_review',
        sa.Column('id', sa.Integer, primary_key=True),
        sa.Column('change_id', sa.Integer, sa.ForeignKey('change_request.id', ondelete='CASCADE')),
