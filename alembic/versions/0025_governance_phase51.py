...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("autonomy_level", sa.String(16), nullable=False, server_default="standard"),
    ...
    op.create_index("ix_autonomy_policy_user_org_repo", "autonomy_policy", ["user_id", "org_id", "repo"], unique=True)
    ...
    op.create_index("ix_approval_request_org_status", "approval_request", ["org_id", "status"])
    op.create_index("ix_approval_request_requester", "approval_request", ["requester_id"])
    ...
    op.create_table(
        ...
        sa.Column("org_id", sa.String(64), nullable=False),
        sa.Column("action_type", sa.String(64), nullable=False),
        sa.Column("decision", sa.String(32), nullable=False),  # AUTO, APPROVAL_REQUIRED, BLOCKED, APPROVED, REJECTED, EXECUTED
    ...
    op.create_index("ix_gov_audit_log_org_timestamp", "governance_audit_log", ["org_id", "timestamp"])
    op.create_index("ix_gov_audit_log_user_timestamp", "governance_audit_log", ["user_id", "timestamp"])
    op.create_index("ix_gov_audit_log_action_type", "governance_audit_log", ["action_type"])
    op.create_index("ix_gov_audit_log_decision", "governance_audit_log", ["decision"])
    op.create_index("ix_gov_audit_log_risk_score", "governance_audit_log", ["risk_score"])
    ...
    op.create_index("ix_rollback_history_org_timestamp", "rollback_history", ["org_id", "timestamp"])
    op.create_index("ix_rollback_history_original_action", "rollback_history", ["original_action_id"])
