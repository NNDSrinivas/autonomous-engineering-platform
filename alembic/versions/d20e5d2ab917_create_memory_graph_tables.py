...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('embedding', sa.Text(), nullable=True),  # Will be VECTOR(1536) via raw SQL
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), server_default=sa.text('NOW()'), nullable=False),
    ...
    op.execute('ALTER TABLE memory_chunk ALTER COLUMN embedding TYPE vector(1536) USING embedding::vector(1536)')
