  ...
  try:
    ...
    if message.strip().lower() in (
      ...
      if previous_state and previous_state.get("pending_action"):
        ...
        result = await execute_tool(user_id, tool_name, tool_args, db=db)
        elapsed_ms = int((time.monotonic() - started) * 1000)
    ...
    # ---------------------------------------------------------
    # STAGE 2.5: Fast-path for "what is this project / repo?"
    # ---------------------------------------------------------
    if _is_project_overview_request(message):
      fast_result = await _run_project_overview_fast_path(
        user_id=user_id,
        message=message,
        model=model,
        mode=mode,
      ...
      if fast_result is not None:
        # Patch in real duration
        fast_result["duration_ms"] = int((time.monotonic() - started) * 1000)
        return fast_result
      # If fast-path failed, we just fall through to normal pipeline.
    ...
    # ---------------------------------------------------------
    # STAGE 7b: Execute safe tools immediately and use results with LLM
    # ---------------------------------------------------------
    safe_tools = ["repo.inspect", "code.read_files", "code.search"]
    if plan.steps and all(step.tool in safe_tools for step in plan.steps):
        logger.info("[AGENT] Executing safe tools and enriching context for LLM response")
        
        # Execute tools and collect actual results
        enriched_context_parts = [full_context.get("combined", "")]
        executed_actions = []
        
        for step in plan.steps[:3]:  # Limit to first 3 steps
            try:
                logger.info(f"[AGENT] Executing {step.tool} with args: {step.arguments}")
                result = await execute_tool(user_id, step.tool, step.arguments, db=db)
                
                # Convert result to string for context enrichment
                if isinstance(result, dict):
                    result_text = result.get("text") or result.get("content") or json.dumps(result, indent=2)
                else:
                    result_text = str(result)
                
                # Add to context
                enriched_context_parts.append(f"\n<tool_result tool='{step.tool}'>\n{result_text}\n</tool_result>\n")
                
                # Track for actions array
                executed_actions.append({
                    "id": f"step-{len(executed_actions) + 1}",
                    ...
                    "type": "workspaceRead",
                    "filePath": None,
                    "content": result_text[:1000] + ("..." if len(result_text) > 1000 else "")
                })
                
            except Exception as e:  # noqa: BLE001
                logger.warning(f"[AGENT] Tool {step.tool} failed: {e}")
                executed_actions.append({
                    "id": f"step-{len(executed_actions) + 1}",
                    "title": f"{step.description} (failed)",
                    "description": f"{step.description} - Error: {str(e)[:100]}",
                    "tool": step.tool,
                    "arguments": step.arguments,
                    "type": "workspaceRead",
                    "filePath": None,
        ...
        # Use enriched context with LLM to generate intelligent response
        enriched_context = {"combined": "\n".join(enriched_context_parts)}
        
        logger.info("[AGENT] Calling LLM with enriched tool results")
        answer = await call_llm(
            message=message,
        ...
        return {
            ...
            "should_stream": True,
            "state": {"completed": True, "safe_tools_executed": True},
            "duration_ms": elapsed_ms,
        }
