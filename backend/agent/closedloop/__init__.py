...
class Phase5System:
    ...
    async def handle_jira_event(
        ...
        try:
            ...
            logger.error(f"Failed to handle Jira event {event_type}: {e}", exc_info=True)
            return None
    ...
    async def handle_github_event(
        ...
        try:
            ...
            logger.error(f"Failed to handle GitHub event {event_type}: {e}", exc_info=True)
            return None
    ...
    async def handle_slack_mention(
        ...
        try:
            ...
            logger.info(f"Processed Slack mention from {user_id}, started loop {loop_id}")
            return loop_id
    ...
    async def handle_ci_event(
        ...
        try:
            ...
            logger.info(f"Processed CI {event_type} event for {repository}/{branch}, started loop {loop_id}")
            return loop_id
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.orchestrator:
                orchestrator_metrics = await self.orchestrator.get_orchestration_metrics()
        ...
        try:
            ...
            for loop_id, loop_execution in self.orchestrator.active_loops.items():
                loop_info = {
                    ...
                    "safety_level": loop_execution.safety_level.value if loop_execution.safety_level else None,
                ...
                if loop_execution.execution_plan:
                    loop_info["execution_plan"] = {
                        "action_count": len(loop_execution.execution_plan.primary_actions),
    ...
    async def get_loop_details(self, loop_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            details = {
                ...
                "started_at": loop_execution.started_at.isoformat(),
                "completed_at": loop_execution.completed_at.isoformat() if loop_execution.completed_at else None,
                "current_state": loop_execution.current_state.value,
            ...
            if loop_execution.resolved_context:
                details["resolved_context"] = {
                    ...
                    "team_members_count": len(loop_execution.resolved_context.team_members),
                    "related_issues_count": len(loop_execution.resolved_context.related_issues),
                ...
                if loop_execution.resolved_context.primary_object:
                    details["resolved_context"]["primary_object"] = {
                        "type": loop_execution.resolved_context.primary_object.get("type", "unknown"),
                        "key": loop_execution.resolved_context.primary_object.get("key", ""),
                        "title": loop_execution.resolved_context.primary_object.get("title", "")[:100]  # Truncate
                    }
            
            # Add execution plan details
            if loop_execution.execution_plan:
                details["execution_plan"] = {
                    "primary_actions_count": len(loop_execution.execution_plan.primary_actions),
                    "backup_actions_count": len(loop_execution.execution_plan.backup_actions),
                    "overall_confidence": loop_execution.execution_plan.overall_confidence,
                ...
                details["generated_reports"] = [
                    {
                        ...
                        "title": report.title,
                        "target_channels": [channel.value for channel in report.target_channels],
                        "successful_deliveries": [channel.value for channel in report.successful_deliveries],
            ...
            logger.error(f"Failed to get loop details for {loop_id}: {e}", exc_info=True)
            return {"error": str(e)}
    
    async def approve_loop(self, loop_id: str, approved: bool, user_id: str, reason: str = "") -> bool:
        """Approve or reject a pending loop execution"""
        
        if not self.orchestrator:
            return False
        
        return await self.orchestrator.approve_pending_loop(loop_id, approved, user_id, reason)
    
    async def escalate_loop(self, loop_id: str, reason: str, escalation_level: str = "management") -> bool:
...
async def start_autonomous_jira_workflow(
    ...
    try:
        ...
        logger.error(f"Failed to start autonomous Jira workflow for {issue_key}: {e}", exc_info=True)
        return None
...
async def trigger_pr_autofix(
    ...
    try:
        ...
        comment_data = {
            "body": comment_text or "Auto-fix requested",
            "synthetic": True
        }
        ...
        logger.error(f"Failed to trigger PR autofix for {repository}#{pr_number}: {e}", exc_info=True)
        return None
...
async def handle_ci_failure_autonomously(
    ...
    try:
        ...
        return await system.handle_ci_event("build_failed", build_data, repository, branch)
        
    except Exception as e:
        logger.error(f"Failed to handle CI failure for {repository}/{branch}: {e}", exc_info=True)
        return None
