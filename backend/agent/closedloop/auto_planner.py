...
class PlannedAction:
    ...
    prerequisites: List[str] = field(default_factory=list)    # What must be true before execution
            ...
            try:
                ...
                for candidate in ActionType:
                    if candidate.value == normalized or candidate.name.lower() == normalized:
                        return candidate
        logger.warning("Unknown action_type '%s'; defaulting to REQUEST_CLARIFICATION", value)
...
class AutoPlanner:
    ...
    async def create_execution_plan(self, event: ProcessedEvent, context: ResolvedContext) -> ExecutionPlan:
        """
        ...
        if context.context_completeness < self.min_context_completeness:
            logger.warning(f"Insufficient context completeness: {context.context_completeness}")
        ...
        try:
            ...
            if len(prioritized_actions) > self.max_simultaneous_actions:
                prioritized_actions = prioritized_actions[:self.max_simultaneous_actions]
            
            # Create contingency actions
            contingency_actions = await self._create_contingency_actions(event, context, prioritized_actions)
            
            # Create monitoring actions
            monitoring_actions = await self._create_monitoring_actions(event, context, prioritized_actions)
            ...
            execution_window = self._determine_execution_window(context, prioritized_actions)
            prerequisites_met = await self._check_prerequisites(prioritized_actions)
            human_approval_needed = any(action.human_approval_required for action in prioritized_actions)
            ...
            plan = ExecutionPlan(
                ...
                success_criteria=self._define_success_criteria(context, prioritized_actions),
                failure_indicators=self._define_failure_indicators(context, prioritized_actions),
                monitoring_frequency=self._determine_monitoring_frequency(overall_priority),
                created_at=datetime.now(timezone.utc),
                expires_at=datetime.now(timezone.utc) + timedelta(hours=24),  # Plans expire after 24 hours
                plan_id=f"plan_{event.event_id}_{int(datetime.now().timestamp())}"
            )
            
            logger.info(f"Created execution plan {plan.plan_id} with {len(prioritized_actions)} actions")
            return plan
    ...
    async def _plan_jira_actions(self, event: ProcessedEvent, context: ResolvedContext) -> List[PlannedAction]:
    ...
    async def _plan_issue_assignment_actions(self, event: ProcessedEvent, context: ResolvedContext) -> List[PlannedAction]:
        ...
        if assignee and assignee.lower() in ["navi", "bot", "autonomous"]:
            ...
            if complexity < 0.3:  # Simple issue
                actions.append(PlannedAction(
                    ...
                    historical_success=self._get_historical_success(ActionType.IMPLEMENT_FEATURE),
                    ...
                    prerequisites=["issue_has_clear_requirements", "repository_accessible"],
                    safety_checks=["validate_requirements", "run_tests", "check_for_conflicts"],
                    rollback_plan="Close PR if implementation fails validation",
                    human_approval_required=False,
                    escalation_triggers=["implementation_fails", "tests_fail", "conflicts_detected"],
                    ...
                    alternatives_considered=["request_human_review", "gather_more_context"],
                ...
                actions.append(PlannedAction(
                    ...
                    target=issue.get("key", ""),
                    parameters={
                        "gather_types": ["related_issues", "code_examples", "test_cases"],
                    ...
                    alternatives_considered=["immediate_implementation", "request_human_help"],
                ...
                actions.append(PlannedAction(
                    ...
                    alternatives_considered=["attempt_implementation", "escalate_immediately"],
                    risks_identified=["incorrect_implementation", "missed_requirements"],
                    created_at=datetime.now(timezone.utc)
                ))
    ...
    async def _plan_new_issue_actions(self, event: ProcessedEvent, context: ResolvedContext) -> List[PlannedAction]:
        ...
        if priority in ["critical", "blocker"] or "critical" in context.urgency_indicators:
            actions.append(PlannedAction(
                ...
                human_approval_required=False,
                escalation_triggers=[],
                notification_recipients=["oncall@company.com", "management@company.com"],
        ...
        if context.related_issues:
            actions.append(PlannedAction(
                ...
                parameters={
                    ...
                    "target_issues": [ri.get("key") for ri in context.related_issues[:3]],  # Max 3 links
                },
    ...
    async def _plan_comment_response_actions(self, event: ProcessedEvent, context: ResolvedContext) -> List[PlannedAction]:
        ...
        if self._is_navi_mentioned(comment_text):
            actions.append(PlannedAction(
                ...
                parameters={
                    "comment": await self._generate_intelligent_response(comment_text, context),
    ...
    async def _plan_status_change_actions(self, event: ProcessedEvent, context: ResolvedContext) -> List[PlannedAction]:
    ...
    async def _plan_general_issue_actions(self, event: ProcessedEvent, context: ResolvedContext) -> List[PlannedAction]:
    ...
    async def _plan_github_actions(self, event: ProcessedEvent, context: ResolvedContext) -> List[PlannedAction]:
        ...
        if event.trigger_type == "pr_review_requested":
            actions.append(PlannedAction(
                ...
                confidence_score=0.7,
                context_completeness=context.context_completeness,
                historical_success=self._get_historical_success(ActionType.REVIEW_PR),
                ...
                safety_checks=["validate_changes_scope", "check_for_breaking_changes"],
                rollback_plan="Delete review if inappropriate",
                human_approval_required=False,
                escalation_triggers=["security_issues_found", "breaking_changes_detected"],
    ...
    async def _plan_slack_actions(self, event: ProcessedEvent, context: ResolvedContext) -> List[PlannedAction]:
        ...
        if self._is_navi_mentioned(message.get("text", "")):
            actions.append(PlannedAction(
                ...
                parameters={
                    "response": await self._generate_slack_response(message, context),
    ...
    async def _plan_ci_actions(self, event: ProcessedEvent, context: ResolvedContext) -> List[PlannedAction]:
        ...
        if build.get("status") == "failed":
            ...
            if failure_type == "test_failure":
                actions.append(PlannedAction(
                    ...
                    confidence_score=0.6,
                    context_completeness=context.context_completeness,
                    historical_success=self._get_historical_success(ActionType.FIX_BUG),
                    ...
                    escalation_triggers=["fix_causes_more_failures", "tests_still_fail"],
    ...
    async def _plan_deployment_actions(self, event: ProcessedEvent, context: ResolvedContext) -> List[PlannedAction]:
        ...
        if deployment.get("status") == "failed":
            ...
            if environment == "production":
                actions.append(PlannedAction(
                    ...
                    confidence_score=0.9,
                    context_completeness=context.context_completeness,
                    historical_success=self._get_historical_success(ActionType.ROLLBACK_DEPLOYMENT),
                    ...
                    safety_checks=["validate_rollback_safety", "check_rollback_impact"],
                    ...
                    risks_identified=["rollback_causes_data_loss", "service_downtime"],
                    created_at=datetime.now(timezone.utc)
                ))
        
        return actions
    
    async def _plan_generic_actions(self, event: ProcessedEvent, context: ResolvedContext) -> List[PlannedAction]:
    ...
    def _filter_viable_actions(self, actions: List[PlannedAction]) -> List[PlannedAction]:
        ...
        for action in actions:
            ...
            if action.confidence_score < self.min_confidence_threshold:
                logger.debug(f"Filtering out {action.action_type} due to low confidence: {action.confidence_score}")
                continue
            
            # Check context completeness
            if action.context_completeness < self.min_context_completeness:
                logger.debug(f"Filtering out {action.action_type} due to incomplete context: {action.context_completeness}")
                continue
            ...
            if action.confidence_score < min_required:
                logger.debug(f"Filtering out {action.action_type} due to safety requirement: {action.confidence_score} < {min_required}")
                continue
    ...
    async def _create_contingency_actions(self, event: ProcessedEvent, context: ResolvedContext, primary_actions: List[PlannedAction]) -> List[PlannedAction]:
        ...
        for action in primary_actions:
            ...
            contingency_actions.append(PlannedAction(
                action_type=ActionType.ESCALATE_ISSUE,
    ...
    async def _create_monitoring_actions(self, event: ProcessedEvent, context: ResolvedContext, primary_actions: List[PlannedAction]) -> List[PlannedAction]:
        ...
        if primary_actions:
            monitoring_actions.append(PlannedAction(
                action_type=ActionType.GATHER_MORE_CONTEXT,
                priority=ActionPriority.LOW,
                safety_level=SafetyLevel.SAFE,
                confidence_score=0.9,
    ...
    async def _create_context_gathering_plan(self, event: ProcessedEvent, context: ResolvedContext) -> ExecutionPlan:
        ...
        action = PlannedAction(
            ...
            target=event.event_id,
            parameters={
                "context_types": ["related_issues", "team_members", "historical_patterns"],
                "depth": "comprehensive",
            },
            prerequisites=[],
            safety_checks=[],
    ...
    async def _create_safe_fallback_plan(self, event: ProcessedEvent, context: ResolvedContext) -> ExecutionPlan:
    ...
    def _calculate_overall_confidence(self, actions: List[PlannedAction]) -> float:
        ...
        for action in actions:
            weight = 1.0 / max(action.estimated_duration, 1)  # Inverse duration weighting
    ...
    def _determine_overall_safety(self, actions: List[PlannedAction]) -> SafetyLevel:
        ...
        most_restrictive = max(actions, key=lambda a: safety_priority.get(a.safety_level, 1))
        return most_restrictive.safety_level
    
    def _determine_overall_priority(self, actions: List[PlannedAction]) -> ActionPriority:
        ...
        highest_priority = max(actions, key=lambda a: priority_values.get(a.priority, 1))
        return highest_priority.priority
    
    def _determine_execution_window(self, context: ResolvedContext, actions: List[PlannedAction]) -> Optional[Tuple[datetime, datetime]]:
        ...
        for action in actions:
            for prerequisite in action.prerequisites:
                if not await self._check_prerequisite(prerequisite, action):
                    logger.warning(f"Prerequisite '{prerequisite}' not met for {action.action_type}")
                    return False
        
        return True
    
    async def _check_prerequisite(self, prerequisite: str, action: PlannedAction) -> bool:
        ...
        prerequisite_checks = {
            ...
            "related_issues_exist": lambda: len(action.parameters.get("target_issues", [])) > 0,
    ...
    def _define_success_criteria(self, context: ResolvedContext, actions: List[PlannedAction]) -> List[str]:
    ...
    def _define_failure_indicators(self, context: ResolvedContext, actions: List[PlannedAction]) -> List[str]:
    ...
    def _get_historical_success(self, action_type: ActionType) -> float:
        ...
        return self.historical_success_rates.get(action_type, default_rates.get(action_type, 0.5))
    ...
    def _suggest_issue_labels(self, issue: Dict[str, Any], context: ResolvedContext) -> List[str]:
    ...
    async def _generate_intelligent_response(self, comment_text: str, context: ResolvedContext) -> str:
        ...
        if "help" in text_lower or "assist" in text_lower:
            ...
            return "Let me check the current status and provide an update on progress..."
    ...
    async def _generate_slack_response(self, message: Dict[str, Any], context: ResolvedContext) -> str:
