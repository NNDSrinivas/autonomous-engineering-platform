...
from backend.agent.closedloop.event_ingestor import EventIngestor, ProcessedEvent, EventSource, EventType
...
from backend.agent.closedloop.execution_controller import ExecutionController, ExecutionResult, ExecutionStatus
from backend.agent.closedloop.verification_engine import VerificationEngine, VerificationResult
from backend.agent.closedloop.report_dispatcher import ReportDispatcher, GeneratedReport
from backend.agent.closedloop.loop_memory_updater import LoopMemoryUpdater, LearningOutcome
...
class ClosedLoopOrchestrator:
    ...
    def __init__(
        ...
        self.loop_memory_updater = LoopMemoryUpdater(db_session, workspace_path, org_key)
    ...
    async def ingest_external_event(
        ...
        try:
            ...
            if processed_event:
                ...
                logger.info(f"Queued event {processed_event.event_id} from {source.value}")
            ...
            logger.error(f"Failed to ingest event from {source.value}: {e}", exc_info=True)
            raise
    ...
    async def execute_closed_loop(
        ...
        try:
            ...
            execution_plan = await asyncio.wait_for(
                self.auto_planner.create_execution_plan(triggering_event, resolved_context),
            ...
            if await self._requires_approval(execution_plan, safety_level, mode):
                ...
                if not approved:
                    return await self._handle_loop_cancellation(loop_execution, "Approval denied or timeout")
            ...
            for i, action in enumerate(execution_plan.primary_actions):
                logger.info(f"🔧 [{loop_id}] Executing action {i+1}/{len(execution_plan.primary_actions)}: {action.action_type.value}")
                
                execution_result = await asyncio.wait_for(
                    self.execution_controller.execute_action(action, resolved_context, execution_plan),
                ...
                # Update progress
                loop_execution.progress = 0.4 + (0.3 * (i + 1) / len(execution_plan.primary_actions))
            ...
            for execution_result in loop_execution.execution_results:
                verification_result = await asyncio.wait_for(
                    self.verification_engine.verify_execution(execution_result, resolved_context),
            ...
            for execution_result, verification_result in zip(
                ...
                if verification_result and self.config.report_all_executions:
                    verification_report = await self.report_dispatcher.dispatch_verification_results(
            ...
            if self.config.enable_learning:
                ...
                for execution_result, verification_result in zip(
                    ...
                    learning_outcomes = await self.loop_memory_updater.process_execution_outcome(
                        execution_result, verification_result, execution_plan, resolved_context
                    )
    ...
    async def approve_pending_loop(self, loop_id: str, approved: bool, user_id: str, reason: str = "") -> bool:
        ...
        try:
            ...
            if approved:
                ...
                await self._handle_loop_cancellation(loop_execution, f"Rejected by {user_id}: {reason}")
                return False
                
        except Exception as e:
            logger.error(f"Failed to process approval for loop {loop_id}: {e}", exc_info=True)
            return False
    
    async def escalate_loop(self, loop_id: str, escalation_reason: str, escalation_level: str = "management") -> bool:
        ...
        try:
            ...
            if loop_execution.execution_plan and loop_execution.execution_plan.primary_actions:
                action_type = loop_execution.execution_plan.primary_actions[0].action_type
            ...
            logger.info(f"🚨 Loop {loop_id} escalated to {escalation_level}: {escalation_reason}")
            return True
    ...
    async def get_orchestration_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            success_rate = self.successful_loops / total_loops if total_loops > 0 else 0.0
            
            metrics = {
                "orchestration_status": "active" if self.processing_loop_active else "stopped",
            ...
            # Component health
            component_health = {}
            for component_name in ["event_ingestor", "context_resolver", "auto_planner", 
                                 "execution_controller", "verification_engine", 
                                 "report_dispatcher", "loop_memory_updater"]:
                component = getattr(self, component_name, None)
                if component and hasattr(component, 'get_health_status'):
                    component_health[component_name] = await component.get_health_status()
                else:
            ...
            logger.error(f"Failed to generate orchestration metrics: {e}", exc_info=True)
            return {"error": str(e)}
                ...
                try:
                    ...
                    try:
                        ...
                        loop_task = asyncio.create_task(
                            self.execute_closed_loop(event)
                        )
    ...
    async def _periodic_maintenance(self) -> None:
        ...
        try:
            while not self.shutdown_requested:
                await asyncio.sleep(self.config.memory_maintenance_interval_hours * 3600)
                ...
                try:
                    ...
                    if self.config.enable_learning:
                        maintenance_stats = await self.loop_memory_updater.run_memory_maintenance()
    ...
    async def _requires_approval(
        ...
        if mode == OrchestrationMode.AUTONOMOUS and safety_level != SafetyLevel.DANGEROUS:
            return False
        
        # Block dangerous actions entirely if configured
        if safety_level == SafetyLevel.DANGEROUS and self.config.block_high_risk_actions:
            return True
    ...
    async def _complete_loop_execution(self, loop_execution: LoopExecution) -> LoopExecution:
        ...
        try:
            ...
            if self.config.audit_trail_enabled:
                self._add_audit_entry("loop_completed", {
                    "loop_id": loop_execution.loop_id,
    ...
    async def _handle_loop_error(self, loop_execution: LoopExecution, error: Exception) -> LoopExecution:
        ...
        try:
            ...
            # Log error
            logger.error(f"Loop {loop_execution.loop_id} failed: {error}", exc_info=True)
            ...
            if self.config.report_all_executions or self.config.report_only_failures:
                try:
                    await self.report_dispatcher.dispatch_safety_alert(
                        ...
                        ["Review error logs", "Check system health", "Consider manual intervention"],
                        loop_execution.resolved_context
                    )
    ...
    async def _handle_loop_timeout(self, loop_execution: LoopExecution, timeout_reason: str) -> LoopExecution:
    ...
    async def _handle_loop_cancellation(self, loop_execution: LoopExecution, reason: str) -> LoopExecution:
    ...
    def _cleanup_old_loops(self) -> None:
        ...
        try:
            ...
            if len(self.completed_loops) > self.max_history:
                ...
                logger.info(f"Cleaned up old loops, keeping {self.max_history} most recent")
    ...
    def _update_performance_metrics(self) -> None:
        ...
        try:
            ...
            if self.total_loops_processed > 0:
                self.autonomous_success_rate = self.successful_loops / self.total_loops_processed
