...
class ContextResolver:
    ...
    async def resolve_context(self, event: IngestEvent) -> ResolvedContext:
        ...
        try:
            ...
            return ResolvedContext(
                ...
                data_freshness=1.0,
                resolution_time_ms=int(
                    (datetime.now() - resolution_start).total_seconds() * 1000
                ),
    ...
    async def _resolve_jira_context(self, event: IngestEvent) -> ResolvedContext:
        ...
        related_documentation = await self._find_related_documentation(
            issue_key, primary_issue
        )

        # 6. Analyze code files mentioned/related
        related_code_files = await self._identify_related_code_files(
            issue_key, primary_issue
        )
        ...
        historical_patterns = await self._find_historical_patterns(
            issue_key, primary_issue
        )
        ...
        complexity_score = self._calculate_complexity_score(
            primary_issue, related_issues, related_code_files
        )
    ...
    async def _resolve_github_context(self, event: IngestEvent) -> ResolvedContext:
        ...
        return ResolvedContext(
            ...
            related_issues=related_issues,
            related_prs=[],
            related_conversations=await self._get_pr_conversations(
    ...
    async def _resolve_slack_context(self, event: IngestEvent) -> ResolvedContext:
        ...
        return ResolvedContext(
            ...
            related_issues=related_issues,
            related_prs=[],
            related_conversations=await self._get_slack_thread_context(
                channel, message_ts
            ),
            related_documentation=[],
            related_code_files=[],
    ...
    async def _resolve_ci_context(self, event: IngestEvent) -> ResolvedContext:
        ...
        return ResolvedContext(
            ...
            related_issues=[],
            related_prs=(
                await self._find_pr_for_branch(
                    event.tags.get("branch"), event.tags.get("repository")
                )
    ...
    async def _resolve_deployment_context(self, event: IngestEvent) -> ResolvedContext:
        ...
        return ResolvedContext(
            ...
            urgency_indicators=self._identify_deployment_urgency_indicators(
                primary_deployment
            ),
    ...
    async def _find_related_jira_issues(
        self, issue_key: str, primary_issue: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _find_related_documentation(
        self, issue_key: str, issue: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _identify_related_code_files(
        self, issue_key: str, issue: Dict[str, Any]
    ) -> List[str]:
    ...
    async def _find_historical_patterns(
        ...
        try:
            ...
            similar_nodes = await self.memory_service.search(
                query=issue.get("title", ""), node_types=["jira_issue"], limit=5
            )
    ...
    def _assess_jira_risk_factors(
        self, issue: Dict[str, Any], related_issues: List[Dict[str, Any]]
    ) -> List[str]:
    ...
    def _identify_urgency_indicators(self, issue: Dict[str, Any]) -> List[str]:
        ...
        # Keyword indicators
        text_content = (
            f"{issue.get('title', '')} {issue.get('description', '')}".lower()
        )
        urgent_keywords = [
            "urgent",
            "asap",
    ...
    def _is_cache_valid(self, context: ResolvedContext) -> bool:
        ...
        if isinstance(resolved_at, datetime):
            ...
            now = (
                datetime.now(tz=resolved_at.tzinfo)
    ...
    async def _get_github_pr_details(
        self, pr_number: str, repository: str
    ) -> Dict[str, Any]:
    ...
    async def _get_pr_conversations(
        self, pr_number: str, repository: str
    ) -> List[Dict[str, Any]]:
    ...
    async def _get_slack_thread_context(
    ...
    def _identify_deployment_urgency_indicators(
    ...
    async def _get_repository_info_from_issue(
    ...
    async def _extract_dependencies_from_issue(
        self, issue: Dict[str, Any]
    ) -> List[str]:
    ...
    async def _find_pr_for_branch(
        self, branch: Optional[str], repository: Optional[str]
    ) -> List[Dict[str, Any]]:
