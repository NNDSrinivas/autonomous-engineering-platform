...
class ProcessedEvent:
    ...
    def event_summary(self) -> str:
        return self.original_event.summary or self.original_event.title or self.original_event.content or ""
...
class EventProcessor:
    ...
    async def _process_pr_comment(self, event: IngestEvent) -> ProcessedEvent:
        ...
        needs_changes = any(keyword in comment_text.lower() for keyword in change_keywords)
    ...
    async def _process_deployment_failure(self, event: IngestEvent) -> ProcessedEvent:
        ...
        priority = EventPriority.CRITICAL if environment == "production" else EventPriority.HIGH
    ...
    async def _process_slack_mention(self, event: IngestEvent) -> ProcessedEvent:
        ...
        is_eng_channel = any(eng in channel.lower() for eng in ["eng", "dev", "tech", "backend", "frontend"])
        
        priority = EventPriority.HIGH if is_urgent else (EventPriority.NORMAL if is_eng_channel else EventPriority.LOW)
...
class EventIngestor:
    ...
    async def ingest_event(self, event: Union[IngestEvent, Dict[str, Any]]) -> IngestResponse:
        """
        ...
        logger.info(f"Ingesting event: {event.source}:{event.event_type} ({event.external_id})")
        ...
        return IngestResponse(
            status="queued",
            message=f"Event {event.external_id} queued for processing"
        )
    ...
    async def _process_single_event(self, event: IngestEvent) -> None:
        ...
        try:
            ...
            await self._fire_event_callback("event_processed", {
                "event": event,
    ...
    async def _handle_autonomous_action(self, processed_event: ProcessedEvent) -> None:
        ...
        logger.info(f"Handling autonomous action: {trigger_value} for {event.external_id}")
        ...
        if processed_event.confidence_score < 0.5:
            logger.info(f"Confidence too low ({processed_event.confidence_score}) for autonomous action")
            await self._request_human_approval(processed_event)
            self.stats["approval_requests"] += 1
            return
    ...
    async def _execute_autonomous_action(self, processed_event: ProcessedEvent) -> None:
        ...
        try:
            ...
            await self._fire_event_callback("autonomous_action_triggered", {
                "event": event,
    ...
    async def _handle_jira_assignment(self, event: IngestEvent, context: Dict[str, Any]) -> None:
        """Handle Jira issue assignment autonomously"""

        issue_key = context["issue_key"]
        assignee = context["assignee"]
        
        logger.info(f"Starting autonomous work on Jira issue {issue_key} for {assignee}")

        if not self.orchestrator:
            logger.warning("No orchestrator configured; skipping autonomous Jira assignment handling.")
            return
        ...
        logger.info(f"Created autonomous initiative {initiative_id} for Jira {issue_key}")
    
    async def _handle_jira_in_progress(self, event: IngestEvent, context: Dict[str, Any]) -> None:
        ...
        logger.info(f"Jira issue {issue_key} moved to in progress - monitoring for autonomous actions")
    
    async def _handle_pr_comment(self, event: IngestEvent, context: Dict[str, Any]) -> None:
        ...
        logger.info(f"PR #{pr_number} in {repository} has review comments requiring changes")
        
        # This would integrate with PR analysis and automated fix generation
        # For now, just log the action
    
    async def _handle_ci_failure(self, event: IngestEvent, context: Dict[str, Any]) -> None:
    ...
    async def _handle_deployment_failure(self, event: IngestEvent, context: Dict[str, Any]) -> None:
    ...
    async def _handle_slack_mention(self, event: IngestEvent, context: Dict[str, Any]) -> None:
    ...
    async def _request_human_approval(self, processed_event: ProcessedEvent) -> None:
        ...
        logger.info(f"Requesting human approval for {trigger.value if trigger else 'unknown'} on {event.external_id}")
        
        # Fire human approval callback
        await self._fire_event_callback("human_approval_required", {
            "event": event,
            "processed": processed_event,
            "reason": "requires_approval" if processed_event.requires_human_approval else "low_confidence",
        })
    
    async def _fire_event_callback(self, event_type: str, event_data: Dict[str, Any]) -> None:
