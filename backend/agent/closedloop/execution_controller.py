...
class ExecutionController:
    ...
    async def execute_plan(self, plan: ExecutionPlan) -> List[ExecutionResult]:
        ...
        logger.info(
            f"Starting execution of plan {plan.plan_id} with {len(plan.primary_actions)} actions"
        )
        ...
        try:
            ...
            if plan.execution_window:
                if not self._is_within_execution_window(plan.execution_window):
                    logger.info(
                        f"Plan {plan.plan_id} outside execution window, queuing for later"
                    )
            ...
            primary_results = await self._execute_actions_parallel(
                plan.primary_actions, plan.context, plan
            )
            ...
            if not primary_success:
                logger.warning(
                    f"Primary actions failed for plan {plan.plan_id}, executing contingencies"
                )

                # Execute contingency actions
                contingency_results = await self._execute_actions_parallel(
                    plan.contingency_actions, plan.context, plan
                )
            ...
            if plan.monitoring_actions:
                asyncio.create_task(
                    self._execute_monitoring_actions(
                        plan.monitoring_actions, plan.context, plan
                    )
                )
    ...
    async def execute_action(
        ...
        execution_id = (
            f"exec_{action.action_type.value}_{int(datetime.now().timestamp())}"
        )

        logger.info(
            f"Starting execution of action {action.action_type.value} (ID: {execution_id})"
        )
        ...
        try:
            ...
            for attempt in range(max_retries + 1):
                if attempt > 0:
                    ...
                    logger.info(
                        f"Retrying action {action.action_type.value}, attempt {attempt + 1}/{max_retries + 1}"
                    )
                ...
                try:
                    # Execute the action
                    execution_result = await self._execute_single_action(
                        exec_context, result
                    )
                    ...
                    result.error_message = (
                        f"Action timed out after {action.timeout_minutes} minutes"
                    )
                    if attempt < max_retries:
                        continue  # Retry
                    else:
            ...
            if not result.success and action.rollback_plan:
                logger.info(
                    f"Performing rollback for failed action {action.action_type.value}"
                )
                try:
            ...
            logger.info(
                f"Completed execution of action {action.action_type.value} with status {result.status.value}"
            )
    ...
    async def _execute_single_action(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Any:
        ...
        # Set timeout
        timeout = (
            action.timeout_minutes * 60
            if action.timeout_minutes > 0
            else self.default_timeout_minutes * 60
        )

        try:
            # Execute with timeout
            return await asyncio.wait_for(
                executor(exec_context, result), timeout=timeout
            )
        except asyncio.TimeoutError:
            logger.error(
                f"Action {action.action_type.value} timed out after {timeout} seconds"
            )
            raise

    # Action Executors

    async def _execute_assign_issue(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Dict[str, Any]:
    ...
    async def _execute_update_status(
        ...
        action = exec_context.action
        issue_key = action.target
        transition_id = action.parameters.get("transition_id") or action.parameters.get(
            "status"
        )
        ...
        try:
            if not transition_id:
                raise ValueError(
                    "transition_id or status is required to update Jira status"
                )
            await JiraService.transition_issue(self.db, issue_key, transition_id)
            result.execution_logs.append(
                f"Successfully updated {issue_key} status to {transition_id}"
            )
            return {
                "issue_key": issue_key,
                "new_status": transition_id,
                "updated": True,
            }
    ...
    async def _execute_create_subtask(
        ...
        try:
            ...
            result.execution_logs.append(
                f"Successfully created subtask {subtask_key or 'unknown'}"
            )
            return {
                "parent_key": parent_key,
    ...
    async def _execute_link_issues(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for target_key in target_issues:
                await JiraService.link_issues(
                    self.db, source_key, target_key, link_type
                )
                linked_count += 1

            result.execution_logs.append(
                f"Successfully linked {linked_count}/{len(target_issues)} issues"
            )
            return {
    ...
    async def _execute_implement_feature(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result.execution_logs.append(
                f"Prepared implementation plan for {issue_key}"
            )
            return {
                "issue_key": issue_key,
                "task_id": task.id,
    ...
    async def _execute_review_pr(
        ...
        try:
            ...
            review_result = await self._perform_automated_pr_review(
                repo, pr_number, action.parameters
            )
    ...
    async def _execute_notify_team(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Dict[str, Any]:
        ...
        message = action.parameters.get(
            "response", action.parameters.get("message", "")
        )
    ...
    async def _execute_escalate_issue(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Dict[str, Any]:
            ...
            for recipient in action.notification_recipients:
                try:
                    ...
                    notifications_sent.append(
                        {"recipient": recipient, "sent": notification_sent}
                    )
                except Exception as e:
                    logger.error(f"Failed to notify {recipient}: {e}")
                    notifications_sent.append(
                        {"recipient": recipient, "sent": False, "error": str(e)}
                    )

            result.execution_logs.append(
                f"Escalation sent to {len(notifications_sent)} recipients"
            )
    ...
    async def _execute_gather_context(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result.execution_logs.append(
                f"Successfully gathered additional context for {target}"
            )
    ...
    async def _execute_wait_for_human(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if action.parameters.get("notify_team"):
                for recipient in action.notification_recipients:
                    await self._send_wait_notification(
                        recipient, action.target, wait_reason
                    )

            result.execution_logs.append(
                "Notified team about waiting for human intervention"
            )
    ...
    async def _execute_no_action(
        ...
        return {"action_taken": "none", "reason": "no_action_needed"}
    ...
    async def _execute_create_pr(
        ...
        if not repo_full_name or not head:
            raise ValueError(
                "repo_full_name and head branch are required for PR creation"
            )

        ticket_key = action.parameters.get("ticket_key") or action.parameters.get(
            "issue_key"
        )
    ...
    async def _execute_merge_pr(
        ...
        if not repo_full_name:
            repo_full_name = action.parameters.get(
                "repo_full_name"
            ) or action.parameters.get("repo")
        if not pr_number:
            pr_number_raw = action.parameters.get("pr_number") or action.parameters.get(
                "number"
            )
        ...
        result.execution_logs.append(
            f"GitHub PR merge executed for {repo_full_name}#{pr_number}"
        )
        return {
            "repo": repo_full_name,
            "pr_number": pr_number,
    ...
    async def _execute_create_github_issue(
        ...
        result.execution_logs.append(
            f"GitHub issue request executed for {repo_full_name}"
        )
        return {
            "repo": repo_full_name,
            "title": title,
    ...
    async def _execute_refactor_code(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Dict[str, Any]:
    ...
    async def _execute_update_documentation(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Dict[str, Any]:
    ...
    async def _execute_write_tests(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Dict[str, Any]:
    ...
    async def _execute_request_clarification(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Dict[str, Any]:
    ...
    async def _execute_scale_resources(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> Dict[str, Any]:
    ...
    async def _perform_safety_checks(
        ...
        try:
            ...
            return await asyncio.wait_for(
                safety_checker(exec_context, result), timeout=self.safety_check_timeout
            )
        except asyncio.TimeoutError:
    ...
    async def _general_safety_check(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> bool:
        ...
        if action.confidence_score < 0.5:  # Minimum confidence for any action
            logger.warning(
                f"Action {action.action_type.value} has low confidence: {action.confidence_score}"
            )
            return False
    ...
    async def _safety_check_code_changes(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> bool:
    ...
    async def _safety_check_rollback(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> bool:
    ...
    async def _safety_check_service_restart(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ) -> bool:
    ...
    async def _rollback_code_changes(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ):
    ...
    async def _rollback_status_change(
        self, exec_context: ExecutionContext, result: ExecutionResult
    ):
    ...
    async def _request_human_approval(self, plan: ExecutionPlan) -> bool:
        ...
        has_dangerous = any(
            action.safety_level == SafetyLevel.DANGEROUS
            for action in plan.primary_actions
        )

        if has_dangerous:
            logger.warning(
                "Plan contains dangerous actions, approval denied by default"
            )
            return False
    ...
    async def _request_action_approval(
        self, action: PlannedAction, context: Any
    ) -> bool:
    ...
    async def _perform_automated_pr_review(
        self, repo: str, pr_number: str, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _send_wait_notification(
        self, recipient: str, target: str, reason: str
    ) -> bool:
        """Send notification about waiting for human intervention"""
        return True

    async def _find_additional_related_issues(
        self, target: str
    ) -> List[Dict[str, Any]]:
