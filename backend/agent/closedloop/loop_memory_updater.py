...
from backend.agent.memory.learning_engine import LearningEngine, learn_from_approval, learn_from_rejection
from backend.agent.memory.memory_manager import MemoryManager
from backend.agent.memory.memory_capture import (
    MemoryCapture
)
from backend.agent.memory.memory_types import (
    TaskMemory, 
...
class LoopMemoryUpdater:
    ...
    def __init__(self, db_session, workspace_path: Optional[str] = None, org_key: str = "default"):
        self.db = db_session
    ...
    async def process_execution_outcome(
        ...
        try:
            ...
            if execution_result.status == ExecutionStatus.COMPLETED:
                ...
                if verification_result:
                    if verification_result.verification_passed:
                        outcomes.append(await self._learn_from_success(
                            execution_result, verification_result, plan, context
                        ))
                    else:
                        outcomes.append(await self._learn_from_verification_failure(
                            execution_result, verification_result, plan, context
                        ))
            
            elif execution_result.status == ExecutionStatus.FAILED:
                trigger = LearningTrigger.EXECUTION_FAILURE
                outcomes.append(await self._learn_from_failure(
                    execution_result, plan, context
                ))
            ...
            self.learning_stats[trigger]["failed"] += len(outcomes) - len(successful_outcomes)
            ...
            logger.info(f"Processed execution outcome: {len(outcomes)} learning outcomes")
        ...
        try:
            ...
            if approval_status is not None:
                if approval_status:
                    ...
                    outcomes.append(await self._learn_from_user_approval(
                        user_feedback, execution_result, plan, context
                    ))
                else:
                    # User rejected - learn what to avoid
                    outcomes.append(await self._learn_from_user_rejection(
                        user_feedback, execution_result, plan, context
                    ))
            
            # Process contextual feedback (ratings, comments)
            if rating != 0 or feedback_text:
                outcomes.append(await self._process_contextual_feedback(
    ...
    async def process_plan_lifecycle(
        ...
        try:
            if lifecycle_event == "approved":
                outcomes.append(await self._learn_from_plan_approval(
                    plan, context, user_feedback
                ))
                
            elif lifecycle_event == "cancelled":
                outcomes.append(await self._learn_from_plan_cancellation(
                    plan, context, user_feedback
                ))
            ...
            logger.info(f"Processed plan {lifecycle_event}: {len(outcomes)} learning outcomes")
    ...
    async def run_memory_maintenance(self, force: bool = False) -> Dict[str, Any]:
        ...
        if not force and time_since_maintenance.total_seconds() < self.maintenance_interval_hours * 3600:
    ...
    async def get_learning_insights(
        ...
        try:
            ...
            if hasattr(self.learning_service, 'get_learning_stats'):
                bandit_stats = await self.learning_service.get_learning_stats(self.org_key)
    ...
    async def _learn_from_success(
        ...
        try:
            ...
            if self.learning_engine and context and context.primary_object:
                user_id = context.primary_object.get("assignee") or context.primary_object.get("reporter")
                if user_id:
                    approval_data = {
                        ...
                        "quality_score": verification_result.overall_score if verification_result else None
                    }
            ...
            outcome.confidence_delta = 0.1  # Increase confidence for successful patterns
    ...
    async def _learn_from_failure(
        ...
        try:
            ...
            outcome.confidence_delta = -0.05  # Slightly decrease confidence for failed patterns
    ...
    async def _learn_from_verification_failure(
        ...
        try:
            ...
            outcome.success = False
            outcome.error_message = str(e)
            logger.error(f"Failed to learn from verification failure: {e}", exc_info=True)
        ...
        try:
            ...
            if self.learning_engine and user_id and execution_result:
                approval_data = {
                    ...
                    "files_changed": execution_result.result_data.get("files_modified", []) if execution_result.result_data else [],
                    "execution_time": execution_result.duration_seconds
                }
        ...
        try:
            ...
            if self.memory_capture and user_id:
                ...
                memory_entry = ConversationalMemory(
                    ...
                    confidence=min(abs(rating) / 5.0, 1.0)  # Convert rating to confidence
                )
                ...
                if comment:
                    ...
                    outcome.confidence_delta = (rating - 3) * 0.05  # Neutral is 3, so adjust accordingly
    ...
    async def _capture_execution_history(
        ...
        try:
            ...
            task_memory = TaskMemory(
                ...
                step="execution",
                action_taken=execution_result.action.action_type.value,
                result="success" if execution_result.status == ExecutionStatus.COMPLETED else "failed",
                error=execution_result.error_message,
                files_changed=execution_result.result_data.get("files_modified", []) if execution_result.result_data else [],
                user_approved=None,  # Will be updated if user provides feedback
                confidence=execution_result.action.confidence_score,
                importance=0.7 if execution_result.status == ExecutionStatus.COMPLETED else 0.5
            )
            ...
            outcome = LearningOutcome(
                trigger=LearningTrigger.EXECUTION_SUCCESS if execution_result.status == ExecutionStatus.COMPLETED else LearningTrigger.EXECUTION_FAILURE,
                update_type=MemoryUpdateType.EXECUTION_HISTORY,
                success=success
            )
    ...
    async def _flush_learning_buffer(self) -> int:
        ...
        try:
            ...
            for request in self.learning_buffer:
                try:
                    if request.trigger == LearningTrigger.EXECUTION_SUCCESS and request.execution_result:
                    ...
                    elif request.trigger in [LearningTrigger.USER_APPROVAL, LearningTrigger.USER_REJECTION] and request.user_feedback:
                    ...
                    logger.error(f"Failed to flush learning request: {e}", exc_info=True)
    ...
    def _create_bandit_context(self, execution_result: ExecutionResult, context: ResolvedContext) -> Dict[str, Any]:
        """Create context for bandit learning"""
    ...
    def _extract_feedback_patterns(self, feedback_text: str) -> List[str]:
        ...
        # Positive patterns
        if any(word in feedback_lower for word in ["good", "great", "excellent", "perfect", "awesome"]):
        ...
        # Negative patterns  
        if any(word in feedback_lower for word in ["slow", "confusing", "unclear", "wrong"]):
    ...
    def _generate_learning_recommendations(self, insights: Dict[str, Any]) -> List[str]:
        ...
        if success_rate < 0.7:
            recommendations.append("Consider reducing action complexity or improving context resolution")
        
        if success_rate > 0.9:
            recommendations.append("Excellent learning performance - consider increasing automation confidence")
        ...
        if "pattern_learning" in patterns_learned:
            ...
            if pattern_count < 5:
                recommendations.append("Increase pattern detection by encouraging more user feedback")
        
        if "preference_update" in patterns_learned:
            pref_count = patterns_learned["preference_update"]["count"]
            if pref_count > 20:
                recommendations.append("Rich user preference data available - consider personalizing recommendations")
        
        # Memory health
        memory_health = insights.get("memory_health", {})
        if memory_health and "last_maintenance" in memory_health:
            last_maintenance = datetime.fromisoformat(memory_health["last_maintenance"].replace("Z", "+00:00"))
            hours_since_maintenance = (datetime.now(timezone.utc) - last_maintenance).total_seconds() / 3600
            
            if hours_since_maintenance > 48:
                recommendations.append("Run memory maintenance to optimize performance")
        
        return recommendations or ["System performing well - continue current learning patterns"]