...
class GeneratedReport:
    ...
    # Delivery tracking
    delivery_attempts: Dict[ReportChannel, List[Dict[str, Any]]] = field(
...
class ReportDispatcher:
    ...
    async def dispatch_execution_start(
        self, plan: ExecutionPlan, action: PlannedAction, context: ResolvedContext
    ) -> GeneratedReport:
        ...
        template_data = {
            ...
            "prerequisites": (
                ", ".join(action.prerequisites) if action.prerequisites else "None"
            ),
    ...
    async def dispatch_plan_created(
        self, plan: ExecutionPlan, context: ResolvedContext
    ) -> GeneratedReport:
        ...
        template_data = {
            ...
            "estimated_duration": sum(
                action.estimated_duration for action in plan.primary_actions
            ),
    ...
    async def dispatch_daily_summary(
        ...
        template_data = {
            ...
            "action_breakdown": self._format_action_breakdown(
                execution_stats.get("action_breakdown", {})
            ),
    ...
    async def _generate_and_dispatch_report(
        ...
        report = GeneratedReport(
            ...
            detailed_content=(
                template.detailed_template.format(**template_data)
                if template.detailed_template
                else None
            ),
        ...
        for channel in template.channels:
            if channel == ReportChannel.SLACK and template.slack_format:
                report.channel_content[channel] = template.slack_format.format(
                    **template_data
                )
            elif channel == ReportChannel.JIRA and template.jira_format:
                report.channel_content[channel] = template.jira_format.format(
                    **template_data
                )
            elif channel == ReportChannel.EMAIL and template.email_format:
                report.channel_content[channel] = template.email_format.format(
                    **template_data
                )
            else:
                # Use detailed content or summary as fallback
                report.channel_content[channel] = (
                    report.detailed_content or report.summary
                )
        ...
        for channel in report.target_channels:
            try:
                ...
                if not self._check_rate_limit(channel):
                    logger.warning(
                        f"Rate limit exceeded for channel {channel}, queuing for later"
                    )
                    continue
                ...
                if success:
                    ...
                    logger.info(
                        f"Successfully delivered report {report.report_id} to {channel.value}"
                    )
                else:
                    await self._handle_delivery_failure(
                        report, channel, "Delivery failed"
                    )

            except Exception as e:
                logger.error(
                    f"Failed to deliver report {report.report_id} to {channel.value}: {e}"
                )
                await self._handle_delivery_failure(report, channel, str(e))

    async def _deliver_to_channel(
        self, report: GeneratedReport, channel: ReportChannel
    ) -> bool:
    ...
    async def _deliver_to_slack(self, report: GeneratedReport, content: str) -> bool:
        ...
        try:
            ...
            result = await self.slack_client.post_message(
                ...
                text=content,
                attachments=(
                    self._create_slack_attachments(report)
    ...
    async def _handle_delivery_failure(
        self, report: GeneratedReport, channel: ReportChannel, error: str
    ):
        ...
        if len(report.delivery_attempts[channel]) < self.max_retries:
            logger.info(
                f"Scheduling retry for report {report.report_id} to {channel.value}"
            )
            ...
            logger.error(
                f"Max retries exceeded for report {report.report_id} to {channel.value}"
            )
    ...
    async def _retry_delivery(self, report: GeneratedReport, channel: ReportChannel):
        ...
        try:
            ...
            if success:
                ...
                logger.info(
                    f"Retry successful for report {report.report_id} to {channel.value}"
                )
        except Exception as e:
            logger.error(
                f"Retry failed for report {report.report_id} to {channel.value}: {e}"
            )

    async def _escalate_delivery_failure(
        self, report: GeneratedReport, channel: ReportChannel
    ):
        ...
        for fallback in fallback_channels:
            if fallback != channel and fallback not in report.failed_deliveries:
                try:
                    ...
                    logger.error(
                        f"Escalation delivery also failed to {fallback.value}: {e}"
                    )
    ...
    def _check_rate_limit(self, channel: ReportChannel) -> bool:
        ...
        self.rate_limit_trackers[channel] = [
            ts for ts in self.rate_limit_trackers[channel] if ts > minute_ago
        ]
    ...
    def _generate_context_summary(self, context: ResolvedContext) -> str:
        ...
        if context.primary_object:
            ...
            summary_parts.append(
                f"**{obj_type}:** {context.primary_object.get('title', context.primary_object.get('key', 'Unknown'))}"
            )

        if context.related_issues:
            summary_parts.append(
                f"**Related Issues:** {len(context.related_issues)} linked"
            )

        if context.team_members:
            members = [
                member.get("user", "Unknown") for member in context.team_members[:3]
            ]
            summary_parts.append(f"**Team:** {', '.join(members)}")

        if context.urgency_indicators:
            summary_parts.append(
                f"**Urgency:** {', '.join(context.urgency_indicators[:2])}"
            )
    ...
    def _format_execution_results(self, execution_result: ExecutionResult) -> str:
        ...
        if "pr_created" in execution_result.result_data:
            if execution_result.result_data["pr_created"]:
                result_parts.append(
                    f"✅ Pull request created: {execution_result.result_data.get('pr_url', 'N/A')}"
                )
    ...
    def _generate_next_steps(
        ...
        if execution_result.action.action_type in [
            ActionType.IMPLEMENT_FEATURE,
            ActionType.FIX_BUG,
        ]:
            if execution_result.result_data and execution_result.result_data.get(
                "pr_created"
            ):
        ...
        if verification_result:
            if verification_result.verification_passed:
                ...
                next_steps.extend(
                    f"• {rec}" for rec in verification_result.recommendations[:2]
                )
    ...
    def _generate_failure_recommendations(
        ...
        if "timeout" in error_msg:
            recommendations.append(
                "• Increase timeout settings or retry during off-peak hours"
            )
    ...
    def _format_quality_summary(self, verification_result: VerificationResult) -> str:
        ...
        if verification_result.corrections_applied:
            summary_parts.append(
                f"• Auto-corrections: {len(verification_result.corrections_applied)}"
            )
    ...
    def _format_actions_summary(self, actions: List[PlannedAction]) -> str:
        ...
        for i, action in enumerate(actions[:5], 1):  # Limit to 5 actions
            ...
            summary_lines.append(
                f"{i}. {action_name} ({confidence}% confidence, {duration}min)"
            )
    ...
    def _extract_target_users(
        self, action: Optional[PlannedAction], context: Optional[ResolvedContext]
    ) -> List[str]:
    ...
    def _extract_plan_stakeholders(
        self, plan: ExecutionPlan, context: ResolvedContext
    ) -> List[str]:
        ...
        stakeholders.update(
            self._extract_target_users(
                plan.primary_actions[0] if plan.primary_actions else None, context
            )
        )
