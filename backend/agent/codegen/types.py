...
class ChangePlan:
    ...
    def __post_init__(self):
        ...
        if self.file_changes:
            ...
            high_risk_files = [
                fc for fc in self.file_changes if fc.risk_level == "high"
            ]
            medium_risk_files = [
                fc for fc in self.file_changes if fc.risk_level == "medium"
            ]
    ...
    def new_files_count(self) -> int:
        ...
        return len(
            [fc for fc in self.file_changes if fc.change_type == ChangeType.CREATE_FILE]
        )
    ...
    def modified_files_count(self) -> int:
        ...
        return len(
            [fc for fc in self.file_changes if fc.change_type == ChangeType.MODIFY_FILE]
        )
    ...
    def deleted_files_count(self) -> int:
        ...
        return len(
            [fc for fc in self.file_changes if fc.change_type == ChangeType.DELETE_FILE]
        )
    ...
    def remove_file_change(self, file_path: str) -> bool:
        ...
        self.file_changes = [
            fc for fc in self.file_changes if fc.file_path != file_path
        ]
        ...
        for fc in self.file_changes:
            for dep in fc.dependencies:
                if dep not in file_paths:
                    issues.append(
                        f"File {fc.file_path} depends on {dep}, but {dep} is not in the plan"
                    )
    ...
    def optimize_execution_order(self) -> None:
        ...
        while remaining:
            ...
            for file_path, fc in remaining.items():
                deps_satisfied = all(
                    dep in ordered or dep not in remaining for dep in fc.dependencies
                )
                if deps_satisfied:
