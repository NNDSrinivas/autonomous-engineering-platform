...
def build_context(
    ...
    logger.info(
        f"[CONTEXT] Built context with {len(context['combined'])} chars, "
    ...
    if rag_formatted:
        ...
        if slack_threads:
            ...
            for thread in slack_threads[:3]:
                parts.append(
                    f"- **{thread.get('channel')}**: {thread.get('content', '')[:150]}...\n"
                )
            parts.append("\n")
        ...
        if conf_pages:
            ...
            for page in conf_pages[:2]:
                parts.append(
                    f"- **{page.get('title')}**: {page.get('content', '')[:150]}...\n"
                )
            parts.append("\n")
        ...
        if task_memories:
            ...
            for mem in task_memories[:3]:  # Top 3
                parts.append(
                    f"- {mem.get('title')}: {mem.get('content', '')[:150]}...\n"
                )
            parts.append("\n")
