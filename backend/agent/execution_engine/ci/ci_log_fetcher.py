...
class CILogFetcher:
    ...
    async def _fetch_github_logs(self, event: CIEvent) -> CILogs:
        ...
        headers = {
            'Authorization': f'Bearer {self.github_config.token}' if self.github_config and self.github_config.token else '',
            'Accept': 'application/vnd.github.v3+json'
        }
        
        # Fetch workflow run details first
        base_url = self.github_config.base_url if self.github_config and self.github_config.base_url else "https://api.github.com"
        ...
        async with self.session.get(logs_url, headers=headers) as response:
            if response.status != 200:
                logger.warning(f"Could not fetch logs for run {event.run_id}: {response.status}")
                ...
                for file_info in zip_file.filelist:
                    if file_info.filename.endswith('.txt'):
                        with zip_file.open(file_info) as log_file:
                            ...
                            combined_logs.append(f"=== {file_info.filename} ===\n{content}\n")
    ...
    async def _fetch_github_job_logs(self, event: CIEvent, run_data: Dict[str, Any]) -> CILogs:
            ...
            if response.status == 200:
                ...
                for job in jobs_data.get('jobs', []):
                    if job['status'] == 'completed' and job['conclusion'] == 'failure':
                        ...
                        try:
                            async with self.session.get(job_logs_url, headers=headers) as job_response:
                                if job_response.status == 200:
                                    job_logs = await job_response.text()
                                    combined_logs.append(f"=== Job: {job['name']} ===\n{job_logs}\n")
                        except Exception as e:
    ...
    def _parse_logs(self, raw_logs: str, source_url: str, provider: CIProvider) -> CILogs:
        """
        ...
        error_patterns = [
            r'ERROR:', r'FAIL:', r'FAILED:', r'Error:', r'Exception:', 
            r'AssertionError:', r'TypeError:', r'NameError:', r'ImportError:',
            r'✗', r'❌', r'FAILURE', r'BUILD FAILED'
        ]
        
        warning_patterns = [
            r'WARNING:', r'WARN:', r'Warning:', r'⚠️', r'DEPRECATED'
        ]
        ...
        for i, line in enumerate(lines):
            ...
            # Check for errors
            if any(re.search(pattern, line_clean, re.IGNORECASE) for pattern in error_patterns):
            ...
            # Check for warnings
            elif any(re.search(pattern, line_clean, re.IGNORECASE) for pattern in warning_patterns):
