...
from .ci_types import (
    CIEvent, FailureType, RepairAction, RepairResult, 
    CIRepairSession, RepairAuditLog, CIIntegrationContext,
    GitHubActionsConfig, CILogs, FailureContext, RepairPlan,
    RepairConfidence
)
...
class CIRepairOrchestrator:
    ...
    async def handle_ci_failure(
        self,
        event: CIEvent,
        integration_context: Optional[CIIntegrationContext] = None
    ) -> CIRepairSession:
        """
        ...
        logger.info(f"Starting CI repair session {session_id} for {event.provider.value} run {event.run_id}")
        ...
        try:
            ...
            logger.info(f"CI repair session {session_id} completed with status: {session.result.success if session.result else 'unknown'}")
        
        return session
    
    async def _execute_repair_workflow(self, session: CIRepairSession) -> CIRepairSession:
        ...
        logger.info(f"Classified as {session.failure_context.failure_type.value} with {session.failure_context.confidence:.2f} confidence")
    ...
    async def _execute_auto_repair(self, session: CIRepairSession) -> RepairResult:
        ...
        logger.info(f"Executing automatic repair with strategy: {session.repair_plan.repair_strategy}")
        
        # Integration with Phase 4.4 execution engine
        if self.integration_context and self.integration_context.commit_engine_available:
            return await self._execute_with_commit_integration(session)
        else:
            return await self._execute_standalone_repair(session)
    
    async def _execute_with_commit_integration(self, session: CIRepairSession) -> RepairResult:
        ...
        try:
            ...
            if execution_result['success']:
                # Commit changes
                commit_engine = CommitEngine(workspace_root="/tmp")  # TODO: Use actual workspace
                ...
                return RepairResult(
                    ...
                    ci_rerun_id=retry_session.attempts[-1].ci_run_id if retry_session.attempts else None,
                    repair_duration_seconds=int((datetime.now() - (session.started_at or datetime.now())).total_seconds()),
                ...
                return RepairResult(
                    ...
                    action_taken=RepairAction.AUTO_FIX,
                    files_modified=[],
                    error_message=execution_result.get('error', 'Repair execution failed')
                )
        
        except ImportError:
            logger.warning("Phase 4.4 components not available - falling back to standalone repair")
            return await self._execute_standalone_repair(session)
    ...
    async def _execute_standalone_repair(self, session: CIRepairSession) -> RepairResult:
    ...
    async def _generate_repair_suggestion(self, session: CIRepairSession) -> RepairResult:
...
Recommendation: Review the suggested changes and apply manually, or approve auto-repair if confidence is acceptable.
    ...
    def _is_repair_allowed(self, failure_context) -> bool:
        ...
        if self.config.allowed_failure_types:
            if failure_context.failure_type not in self.config.allowed_failure_types:
                logger.info(f"Repair blocked: {failure_context.failure_type.value} not in allowed types")
                return False
        
        # Check confidence threshold  
        if failure_context.confidence < 0.3:  # Minimum confidence
            logger.info(f"Repair blocked: confidence {failure_context.confidence:.2f} too low")
            return False
    ...
    async def _take_safety_snapshot(self) -> str:
        ...
        if self.integration_context and self.integration_context.safety_system_enabled:
            try:
                ...
                snapshot_engine = SnapshotEngine(workspace_root="/tmp")  # TODO: Use actual workspace
                snapshot = snapshot_engine.take_snapshot(files=[])
                snapshot_id = getattr(snapshot, 'id', getattr(snapshot, 'snapshot_id', str(id(snapshot))))
    ...
    async def _create_audit_log(self, session: CIRepairSession):
        ...
        audit_log = RepairAuditLog(
            ...
            files_affected=session.repair_plan.target_files if session.repair_plan else [],
            confidence_score=session.failure_context.confidence if session.failure_context else 0.0,
            approval_required=session.repair_plan.requires_approval if session.repair_plan else False
        )
    ...
    def get_repair_statistics(self) -> Dict[str, Any]:
        ...
        successful_sessions = sum(1 for s in self.repair_history if s.result and s.result.success)
        success_rate = successful_sessions / total_sessions
        
        confidences = [s.failure_context.confidence for s in self.repair_history if s.failure_context]
        avg_confidence = sum(confidences) / len(confidences) if confidences else 0.0
        
        # Find most common failure type
        failure_types = [s.failure_context.failure_type for s in self.repair_history if s.failure_context]
        if failure_types:
        ...
        return {
            ...
            'escalated_sessions': sum(1 for s in self.repair_history if s.human_escalated)
        }
    ...
    async def get_active_sessions(self) -> List[Dict[str, Any]]:
        ...
        for session_id, session in self.active_sessions.items():
            ...
            active_info.append({
                ...
                'failure_type': session.failure_context.failure_type.value if session.failure_context else 'unknown',
