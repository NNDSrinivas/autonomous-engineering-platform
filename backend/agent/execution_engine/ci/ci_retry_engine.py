...
class CIRetryEngine:
    ...
    async def retry_ci_pipeline(
        ...
        try:
            ...
            for attempt_num in range(max_attempts):
                logger.info(f"CI retry attempt {attempt_num + 1}/{max_attempts} for {event.run_id}")
                ...
                if not self._should_continue_retry(attempt, attempt_num, max_attempts - 1):
                    break
    ...
    async def _retry_github_actions(self, event: CIEvent, attempt: RetryAttempt) -> RetryAttempt:
        ...
        try:
            async with self.session.post(retry_url, headers=headers) as response:
                ...
                if response.status == 201:
                    ...
                    logger.info(f"Successfully triggered GitHub Actions retry for {event.run_id}")
                    ...
                    attempt.status = RetryStatus.FAILED
                    attempt.error_message = "Workflow cannot be re-run (possibly already running)"
                    ...
                    attempt.status = RetryStatus.FAILED
                    attempt.error_message = f"GitHub API error: {response.status} - {response_data}"
    ...
    async def _retry_jenkins(self, event: CIEvent, attempt: RetryAttempt) -> RetryAttempt:
        """Retry Jenkins build"""
    ...
    async def _retry_circleci(self, event: CIEvent, attempt: RetryAttempt) -> RetryAttempt:
    ...
    def _check_rate_limits(self) -> bool:
        ...
        if len(self.active_retries) >= self.max_concurrent_retries:
            logger.warning(f"Concurrent retry limit ({self.max_concurrent_retries}) exceeded")
            return False
        
        return True
    
    def _should_continue_retry(self, attempt: RetryAttempt, attempt_num: int, max_attempts: int) -> bool:
    ...
    def _create_rate_limited_session(self, event: CIEvent, session_id: str) -> RetrySession:
    ...
    def _increment_daily_retry_count(self):
        ...
        logger.debug(f"Daily retry count: {self.daily_retry_count}/{self.max_daily_retries}")
    ...
    def get_retry_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'rate_limit_status': 'ok' if self.daily_retry_count < self.max_daily_retries * 0.8 else 'approaching_limit'
        }
    
    async def monitor_retry_progress(self, event: CIEvent, new_run_id: str) -> Dict[str, Any]:
