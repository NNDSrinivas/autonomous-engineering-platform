...
class FailureClassifier:
    ...
    def classify_failure(self, logs: CILogs) -> FailureContext:
        ...
        logger.info(f"Classified CI failure as {best_type.value} with {confidence:.2f} confidence")
        return context
    ...
    def _initialize_classification_rules(self) -> List[ClassificationRule]:
        ...
        return [
            ...
            ClassificationRule(
                ...
                patterns=[
                    r'FAIL:', r'FAILED:', r'Test.*fail', r'AssertionError', r'test.*error',
                    r'expect.*to.*equal', r'expect.*to.*be', r'should.*but.*got',
                    r'✗.*test', r'❌.*test', r'failing test', r'test suite fail',
                    r'jest.*fail', r'mocha.*fail', r'pytest.*fail', r'unittest.*fail'
                ],
            ...
            ClassificationRule(
                ...
                patterns=[
                    r'BUILD FAILED', r'compilation.*error', r'compile.*fail', 
                    r'cannot find.*module', r'module not found', r'unresolved import',
                    r'webpack.*error', r'babel.*error', r'tsc.*error', r'build.*error',
                    r'syntax error', r'parse.*error', r'invalid syntax'
                ],
                confidence_weight=0.85,
                context_keywords=['build', 'compile', 'webpack', 'babel', 'tsc', 'module', 'import']
            ),
            ...
            ClassificationRule(
                ...
                patterns=[
                    r'Type.*error', r'TypeError', r'type.*mismatch', r'type.*check.*fail',
                    r'Property.*does not exist', r'Cannot.*assign.*type', r'is not assignable',
                    r'Expected.*but got', r'mypy.*error', r'tsc.*type.*error',
                    r'AttributeError', r'has no attribute', r'NoneType.*object'
                ],
                confidence_weight=0.9,
                context_keywords=['type', 'interface', 'class', 'property', 'method', 'null', 'undefined']
            ),
            ...
            ClassificationRule(
                ...
                patterns=[
                    r'lint.*error', r'eslint.*error', r'pylint.*error', r'flake8.*error',
                    r'formatting.*error', r'style.*error', r'code.*style', 
                    r'prettier.*error', r'black.*error', r'isort.*error',
                    r'unused.*variable', r'unused.*import', r'missing.*semicolon'
                ],
                confidence_weight=0.8,
                context_keywords=['lint', 'style', 'format', 'unused', 'missing', 'semicolon', 'indent']
            ),
            ...
            ClassificationRule(
                ...
                patterns=[
                    r'environment.*variable.*not.*found', r'missing.*env', r'env.*not.*set',
                    r'secret.*not.*found', r'token.*not.*provided', r'API.*key.*missing',
                    r'configuration.*missing', r'config.*error', r'credential.*error'
                ],
                confidence_weight=0.9,
                context_keywords=['env', 'environment', 'secret', 'token', 'key', 'config', 'credential']
            ),
            ...
            ClassificationRule(
                ...
                patterns=[
                    r'dependency.*error', r'package.*not.*found', r'npm.*error', r'pip.*error',
                    r'yarn.*error', r'poetry.*error', r'requirements.*error',
                    r'node_modules.*error', r'package.*lock.*error', r'version.*conflict',
                    r'could not.*resolve.*dependency'
                ],
                confidence_weight=0.85,
                context_keywords=['dependency', 'package', 'npm', 'pip', 'yarn', 'version', 'install']
            ),
            ...
            ClassificationRule(
                ...
                patterns=[
                    r'security.*vulnerability', r'security.*scan.*fail', r'CVE-', 
                    r'vulnerability.*found', r'security.*alert', r'snyk.*error',
                    r'audit.*fail', r'security.*check.*fail'
                ],
            ...
            ClassificationRule(
                ...
                patterns=[
                    r'performance.*regression', r'performance.*test.*fail', r'timeout.*error',
                    r'too.*slow', r'exceeds.*threshold', r'memory.*limit', r'cpu.*limit',
                    r'lighthouse.*fail', r'performance.*budget'
                ],
                confidence_weight=0.8,
                context_keywords=['performance', 'timeout', 'slow', 'memory', 'cpu', 'threshold']
            ),
            ...
            ClassificationRule(
                ...
                patterns=[
                    r'deployment.*fail', r'deploy.*error', r'infrastructure.*error',
                    r'docker.*error', r'container.*error', r'k8s.*error', r'kubernetes.*error',
                    r'terraform.*error', r'cloudformation.*error', r'helm.*error'
                ],
                confidence_weight=0.85,
                context_keywords=['deploy', 'deployment', 'docker', 'container', 'k8s', 'terraform']
            )
        ]
    ...
    def _initialize_keyword_weights(self) -> Dict[str, float]:
        ...
        return {
            ...
            'test': 0.9, 'fail': 0.8, 'error': 0.8, 'exception': 0.9,
            'assert': 0.85, 'expect': 0.8, 'build': 0.7, 'compile': 0.8,
            
            # Medium confidence indicators  
            'warning': 0.4, 'deprecated': 0.3, 'missing': 0.6, 'not found': 0.7,
            
            # Context-specific weights
            'typescript': 0.6, 'javascript': 0.6, 'python': 0.6, 'java': 0.6,
            'docker': 0.7, 'kubernetes': 0.7, 'terraform': 0.7
        }
    ...
    def _analyze_error_content(self, error_lines: List[str]) -> Dict[str, List[str]]:
        ...
        return {
            'messages': messages[:10],  # Limit to most relevant
            'related': related
        }
    ...
    def _extract_file_references(self, logs: str) -> Dict[str, List[str]]:
        ...
        for pattern in file_patterns:
            ...
            for match in matches:
                ...
                file_path = re.sub(r'".*$', '', file_path)  # Remove trailing quotes/info
    ...
    def _extract_stack_traces(self, logs: str) -> List[str]:
        ...
        for line in lines:
            ...
            if any(re.search(pattern, line, re.IGNORECASE) for pattern in stack_patterns):
                ...
                if line.startswith('  ') or line.startswith('\t') or re.match(r'^\s*at ', line):
    ...
    def _score_failure_types(self, logs: CILogs, error_analysis: Dict) -> Dict[FailureType, float]:
    ...
    def _select_best_classification(self, type_scores: Dict[FailureType, float]) -> Tuple[FailureType, float]:
    ...
    def _extract_relevant_logs(self, logs: CILogs, failure_type: FailureType) -> List[str]:
    ...
    def _determine_failure_location(self, logs: CILogs, file_analysis: Dict) -> Optional[Dict[str, Any]]:
