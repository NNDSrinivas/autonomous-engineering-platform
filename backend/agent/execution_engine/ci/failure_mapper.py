...
class FailureMapper:
    ...
    def map_failure_to_repair_plan(
        self, failure_context: FailureContext, workspace_path: str
    ) -> RepairPlan:
        """
        ...
        target_files = self._identify_target_files(failure_context, workspace_path)

        # Determine repair strategy
        repair_strategy = self._select_repair_strategy(
            failure_context.failure_type, target_files, failure_context.error_messages
        )
    ...
    def _identify_target_files(
        ...
        if not target_files:
            target_files = self._search_relevant_files(failure_context, workspace_path)
        ...
        try:
            for root, dirs, files in os.walk(workspace_path):
                ...
                for file in files:
                    ...
                    if any(
                        self._file_matches_pattern(file, pattern)
    ...
    def _filter_files_by_type(
        self, files: List[str], failure_type: FailureType
    ) -> List[str]:
        ...
        for file_path in files:
            ...
            relevance = self._calculate_file_relevance(file_path, failure_type)
        ...
        filtered.sort(
            key=lambda f: self._calculate_file_relevance(f, failure_type), reverse=True
        )

        return filtered

    def _calculate_file_relevance(
        self, file_path: str, failure_type: FailureType
    ) -> float:
    ...
    def _determine_repair_action(
        self, confidence: float, failure_context: FailureContext
    ) -> RepairAction:
    ...
    def _predict_changes(
        ...
        if failure_type == FailureType.TYPE_ERROR:
            ...
            changes.extend(["Update test assertions", "Fix test data/mocks"])

        elif failure_type == FailureType.BUILD_ERROR:
            changes.extend(["Fix import statements", "Resolve module dependencies"])
    ...
    def _generate_safety_checks(self, failure_context: FailureContext) -> List[str]:
        ...
        if failure_context.failure_type == FailureType.SECURITY_SCAN:
            ...
            checks.extend(
                ["Verify type safety is maintained", "Check for proper null handling"]
            )
