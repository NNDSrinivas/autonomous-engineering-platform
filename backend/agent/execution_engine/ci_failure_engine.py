...
class CIFailureAnalyzer:
    ...
    def analyze_ci_failure(
        self, ci_log: str, job_name: str, step_name: str
    ) -> List[CIFailure]:
        """
    ...
    def _extract_failure_details(
        ...
        file_path, line_number = self._extract_file_reference(
            error_line, log_lines, line_index
        )
    ...
    def _extract_file_reference(
        self, error_line: str, log_lines: List[str], line_index: int
    ...
    def _generate_suggested_fix(
        self, failure_type: str, match: re.Match, error_line: str
    ) -> Optional[str]:
    ...
    def convert_to_diagnostic_issues(
        ...
        for failure in failures:
            ...
            diagnostic = {
                ...
                "severity": (
                    ...
                    in ["missing_dependency", "build_failure", "type_error"]
                    else 2
                ),
                ...
                "fixable": failure.failure_type
                in ["missing_dependency", "lint_failure"],
...
class CIFailureFetcher:
    ...
    async def fetch_latest_failures(
...
ESLint found 2 errors
        ...
        for sim_failure in simulated_failures:
            failures = self.analyzer.analyze_ci_failure(
                sim_failure["log"], sim_failure["job"], sim_failure["step"]
            )
            all_failures.extend(failures)

        return all_failures

    async def fetch_github_actions_failures(
        ...
        logger.info(
            f"GitHub Actions integration not yet implemented for {repo}:{run_id}"
        )
        return []
...
class CIFailureTaskGrounder:
    ...
    async def ground_ci_fix_task(
        ...
        if not any(keyword in user_input.lower() for keyword in ci_keywords):
            return {"type": "rejected", "reason": "Not a CI-related request"}

        # Fetch latest CI failures
        ci_failures = await self.fetcher.fetch_latest_failures(context)

        if not ci_failures:
            return {"type": "rejected", "reason": "No CI failures found to fix"}
        ...
        return {
            ...
            "task": {
                ...
                "inputs": {
                    "diagnostics": [
                        {"path": issue["file"], "diagnostics": [issue]}
