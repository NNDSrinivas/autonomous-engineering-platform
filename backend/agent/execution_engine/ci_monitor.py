...
class CIMonitor:
        ...
        try:
            ...
            while time.time() - start_time < timeout_seconds:
                ...
                if ci_status:
                    ...
                    if result.overall_status in [CIStatus.SUCCESS, CIStatus.FAILURE, CIStatus.CANCELLED]:
                        result.completed_at = time.time()
                        result.total_duration = int(result.completed_at - result.started_at)
                        
                        # Analyze results and suggest actions
                        await self._analyze_ci_results(result)
                        
                        logger.info(f"CI monitoring complete for PR #{pr_number}: {result.overall_status.value}")
                        return result
            ...
            result.suggested_action = "CI monitoring timed out - check pipeline manually"
    ...
    async def _fetch_ci_status(self, repository: str, pr_number: int) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            elapsed_time = time.time() - self.monitoring_jobs.get(pr_number, time.time())
    ...
    async def _analyze_ci_results(self, result: CIResult) -> None:
        ...
        if result.overall_status == CIStatus.FAILURE:
            ...
            if not failed_jobs:
                result.suggested_action = "❌ CI failed but no specific job failures identified"
                return
            
            # Categorize failures
            failure_types = [job.failure_type for job in failed_jobs if job.failure_type]
            
            if CIFailureType.BUILD_ERROR in failure_types:
                result.suggested_action = "🔧 Build failure detected - NAVI can analyze and fix build issues"
                result.can_retry = True
                
            elif CIFailureType.TEST_FAILURE in failure_types:
                test_job = next(job for job in failed_jobs if job.failure_type == CIFailureType.TEST_FAILURE)
                if self._is_flaky_test(test_job):
                    result.suggested_action = "🔄 Flaky test detected - recommend retry"
                    result.can_retry = True
                else:
                    result.suggested_action = "🧪 Test failure - NAVI can analyze test logs and suggest fixes"
                    result.can_retry = True
                    
            elif CIFailureType.LINT_ERROR in failure_types:
                result.suggested_action = "📝 Linting issues - NAVI can auto-fix code style problems"
                result.can_retry = True
                
            elif CIFailureType.SECURITY_SCAN in failure_types:
                result.suggested_action = "🔒 Security issues found - manual review required"
