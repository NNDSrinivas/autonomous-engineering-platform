...
class Executor(Generic[T], ABC):
    ...
    @abstractmethod 
    async def plan_fix(self, task: T, analysis: AnalysisResult, context: Dict[str, Any]) -> FixPlan:
        """
    ...
    @abstractmethod
    async def propose_diff(self, task: T, plan: FixPlan, context: Dict[str, Any]) -> DiffProposal:
        """
    ...
    @abstractmethod
    async def verify_results(self, task: T, apply_result: ApplyResult, context: Dict[str, Any]) -> VerificationResult:
        """
    ...
    async def orchestrate_full_workflow(self, task: T, apply_result: ApplyResult, verification: VerificationResult, context: Dict[str, Any]) -> Dict[str, Any]:
        """
...
class ExecutionEngine:
    ...
    async def execute_task(self, task: GroundedTask, context: Optional[Dict[str, Any]] = None) -> ExecutionResult:
        """
        ...
        try:
            ...
            logger.info(f"[{task_id}] Analysis complete: {analysis.total_issues} issues found")
            ...
            logger.info(f"[{task_id}] Plan complete: {len(plan.steps)} steps, {plan.risk_level} risk")
            
            # Phase 3: Propose
            logger.info(f"[{task_id}] Starting diff proposal phase")
            proposal = await executor.propose_diff(task, plan, context)
            logger.info(f"[{task_id}] Proposal complete: {proposal.total_files} files to change")
    ...
    async def execute_approved_changes(self, execution_result: ExecutionResult, context: Optional[Dict[str, Any]] = None) -> ExecutionResult:
        """
        ...
        try:
            ...
            apply_result = await executor.apply_changes(execution_result.proposal, context)
            logger.info(f"[{task_id}] Changes applied: {len(apply_result.files_updated)} files updated")
            ...
            if original_task:
                verification = await executor.verify_results(original_task, apply_result, context)
                ...
                verification = VerificationResult(
                    ...
                    verification_details="Applied changes successfully" if apply_result.success else "Failed to apply some changes",
                    success=apply_result.success,
                    message="Changes applied and verified" if apply_result.success else "Some changes failed to apply"
                )
            
            # Phase 4.4: Enterprise Autonomy Stack (if enabled and successful)
            workflow_result = None
            if (apply_result.success and verification.status == "resolved" and 
                hasattr(executor, 'orchestrate_full_workflow') and original_task):
                try:
                    logger.info(f"[{task_id}] Starting Phase 4.4 Enterprise Autonomy workflow")
                    workflow_result = await executor.orchestrate_full_workflow(
                        original_task, apply_result, verification, context
                    )
                    logger.info(f"[{task_id}] Phase 4.4 workflow: {workflow_result.get('final_status', 'unknown')}")
                except Exception as e:
                    logger.warning(f"[{task_id}] Phase 4.4 workflow failed (non-critical): {e}")
                    workflow_result = {"success": False, "error": str(e), "final_status": "workflow_error"}
            ...
            execution_result.workflow_result = workflow_result  # Store Phase 4.4 results
            execution_result.execution_time_ms = total_time
            execution_result.user_approved = True
            execution_result.status = ExecutionStatus.COMPLETED if apply_result.success and verification.status == "resolved" else ExecutionStatus.FAILED
            execution_result.final_report = self._generate_final_report(execution_result)
            execution_result.success = apply_result.success and verification.status == "resolved"
            
            return execution_result
            
        except Exception as e:
            logger.error(f"[{task_id}] Failed to apply approved changes: {e}", exc_info=True)
    ...
    def _generate_proposal_report(self, analysis: AnalysisResult, plan: FixPlan, proposal: DiffProposal) -> str:
    ...
    def _generate_final_report(self, result: ExecutionResult) -> str:
        ...
        if result.success:
            ...
            if result.workflow_result:
                ...
                if workflow.get("success") and workflow.get("final_status") == "completed_successfully":
