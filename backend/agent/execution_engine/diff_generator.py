...
class DiffGenerator:
    ...
    def __init__(self, workspace_root: str):
        ...
        self.ast_engine = MultiLanguageASTEngine(workspace_root)  # Phase 4.3 AST-aware fixing
    ...
    async def _generate_file_diff(
        ...
        change_summary = self._generate_change_summary(issues, lines_added, lines_removed)
    ...
    def _parse_json_best_effort(self, content: str, file_path: str) -> Optional[Dict[str, Any]]:
            ...
            try:
                ...
                if trailing:
                    ...
                    if merged is not None:
                        return merged
                    logger.warning(f"Ignoring trailing JSON content in {file_path} after position {end}")
                return parsed
    ...
    async def _fix_reference_errors_ast(
        ...
        for issue in issues:
            ...
            if fix_result:
                ...
                logger.info(f"AST-fixed undefined variable '{variable_name}' in {file_path}")
            else:
                logger.warning(f"AST fixer could not handle '{variable_name}' in {file_path}")
    ...
    async def _fix_syntax_errors_ast(
        ...
        for issue in issues:
            logger.warning(f"Syntax error detected in {file_path}:{issue.line} - {issue.message}")
    ...
    def _fix_reference_error(self, content: str, issue: DiagnosticIssue, file_path: str) -> str:
        """
    ...
    def _fix_js_reference_error(self, content: str, var_name: str, line_num: int) -> str:
        """
        ...
        if var_name in ['React', 'useState', 'useEffect', 'Component']:
            ...
            lines[line_num] = f"const {var_name}: any = undefined; // Fix: Added declaration"
        
        else:
            # Generic variable declaration
            lines.insert(line_num - 1, f"const {var_name} = undefined; // Fix: Added missing declaration")
        
        return '\n'.join(lines)
    
    def _fix_python_reference_error(self, content: str, var_name: str, line_num: int) -> str:
        """
        ...
        if var_name in ['os', 'sys', 'json', 're', 'time', 'datetime']:
            ...
            # Generic variable declaration  
            lines.insert(line_num - 1, f"{var_name} = None  # Fix: Added missing declaration")
        
        return '\n'.join(lines)
    
    def _fix_import_error(self, content: str, issue: DiagnosticIssue, file_path: str) -> str:
        """
        Fix import errors by adding missing imports.
        """
        # Extract module name from error message
        module_match = re.search(r"Cannot find module ['\"]([^'\"]+)['\"]", issue.message)
        if not module_match:
            module_match = re.search(r"Module ['\"]([^'\"]+)['\"] not found", issue.message)
        ...
        if ext in ['.ts', '.tsx', '.js', '.jsx']:
            ...
            if module_name.startswith('./') or module_name.startswith('../'):
                ...
                # Package import
                import_line = f"import {module_name.replace('-', '_')} from '{module_name}';"
    ...
    def _fix_syntax_error(self, content: str, issue: DiagnosticIssue, file_path: str) -> str:
        """
    ...
    def _fix_unused_variable(self, content: str, issue: DiagnosticIssue, file_path: str) -> str:
        """
    ...
    def _is_likely_component(self, var_name: str) -> bool:
        ...
        if not var_name:
            return False
        return (var_name[0].isupper() and 
                not var_name.startswith('React') and
                var_name not in ['Component', 'useState', 'useEffect'])
    ...
    def _is_likely_type_error(self, var_name: str) -> bool:
        ...
        return (var_name[0].isupper() and 
                not var_name.startswith('React') and
                var_name not in ['Component', 'useState', 'useEffect', 'Error', 'Exception'])
    ...
    def _group_issues_by_file(self, issues: List[DiagnosticIssue]) -> Dict[str, List[DiagnosticIssue]]:
    ...
    def _generate_change_summary(self, issues: List[DiagnosticIssue], lines_added: int, lines_removed: int) -> str:
