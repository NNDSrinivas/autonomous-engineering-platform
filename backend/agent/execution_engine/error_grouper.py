...
class ErrorGrouper:
    ...
    def group_diagnostics(self, issues: List[DiagnosticIssue]) -> Dict[str, Any]:
        ...
        return {
            ...
            "complexity_score": self._calculate_complexity_score(by_file, cascading_errors)
        }
    
    def _detect_cascading_errors(self, issues: List[DiagnosticIssue]) -> List[Dict[str, Any]]:
        """
        ...
        for issue in issues:
            if issue.category in ["ReferenceError", "ImportError"]:
                if "not defined" in issue.message or "Cannot find name" in issue.message:
                    ...
                    for pattern in self.reference_patterns:
                        ...
                        if match:
                            ...
                            break
                elif "module" in issue.message.lower() or "import" in issue.message.lower():
    ...
    def _detect_shared_causes(self, issues: List[DiagnosticIssue]) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_fix_order(self, by_file: Dict[str, List], cascading_errors: List[Dict[str, Any]]) -> List[str]:
        """
        ...
        remaining_files = [(file, len(issues)) for file, issues in by_file.items() 
    ...
    def _create_batch_groups(self, issues: List[DiagnosticIssue], cascading_errors: List[Dict[str, Any]]) -> List[List[DiagnosticIssue]]:
        """
        ...
        for issue in issues:
            if id(issue) not in cascading_issue_ids and id(issue) not in processed_issues:
    ...
    def _calculate_complexity_score(self, by_file: Dict[str, List], cascading_errors: List[Dict[str, Any]]) -> float:
        """
