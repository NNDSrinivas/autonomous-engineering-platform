...
from .ci import CIRepairOrchestrator, CIEvent, CIIntegrationContext
...
class FixProblemsExecutor(Executor[GroundedTask]):
    ...
    def __init__(self, workspace_root: Optional[str] = None, github_service=None):
        ...
        self.pr_engine = (
            PREngine(workspace_root, github_service) if workspace_root else None
        )
        self.ci_monitor = CIMonitor(github_service)

        # Phase 4.5 components - Enterprise Safety System
        self.snapshot_engine = (
            SnapshotEngine(workspace_root) if workspace_root else None
        )
        self.rollback_engine = (
            RollbackEngine(workspace_root, self.snapshot_engine)
            if workspace_root
            else None
        )
    ...
    async def analyze(
        self, task: GroundedTask, context: Dict[str, Any]
    ) -> AnalysisResult:
        """
        ...
        for diagnostic_entry in diagnostics_data:
            ...
            logger.info(
                f"Processing diagnostic entry: path='{file_path}', diagnostics_count={len(file_diagnostics)}"
            )
        ...
        logger.info(
            f"Phase 4.3 Analysis complete: {result.total_issues} issues, {result.fixable_count} fixable"
        )
        logger.info(
            f"Files affected: {len(grouped_analysis['by_file'])}, Cascading errors: {len(grouped_analysis['cascading_errors'])}"
        )
        return result

    async def _analyze_single_diagnostic(
        self, file_path: str, diagnostic: Dict[str, Any], workspace_root: str
    ) -> DiagnosticIssue:
        ...
        logger.info(
            f"Classification result: category='{category}', confidence={confidence}, fixable={fixable}"
        )
    ...
    def _classify_diagnostic(self, message: str) -> tuple[str, float, bool]:
        ...
        if any(
            ...
            for keyword in ["syntax", "expected", "unexpected"]
        ):
            return ("SyntaxError", 0.6, True)
        elif any(
            keyword in message.lower() for keyword in ["not defined", "undefined"]
        ):
            return ("ReferenceError", 0.7, True)
        elif any(
            keyword in message.lower()
            for keyword in ["import", "module", "cannot resolve"]
        ):
    ...
    def _estimate_complexity(self, issues: List[DiagnosticIssue]) -> str:
        ...
        high_count = sum(
            1 for issue in issues if issue.category in high_complexity_categories
        )
        medium_count = sum(
            1 for issue in issues if issue.category in medium_complexity_categories
        )
    ...
    def _estimate_enhanced_complexity(
        self, issues: List[DiagnosticIssue], grouped_analysis: Dict[str, Any]
    ) -> str:
        """
    ...
    async def plan_fix(
        self, task: GroundedTask, analysis: AnalysisResult, context: Dict[str, Any]
    ) -> FixPlan:
        """
        ...
        estimated_time = self._estimate_fix_time(
            analysis.fixable_count, analysis.estimated_complexity
        )
    ...
    async def propose_diff(
        self, task: GroundedTask, plan: FixPlan, context: Dict[str, Any]
    ) -> DiffProposal:
        """
        ...
        proposal = await self.diff_generator.generate_diff_proposal(
            analysis=analysis_result, plan=plan, context=context
        )

        logger.info(
            f"Phase 4.3 Diff proposal generated: {len(proposal.files_changed)} files, +{proposal.total_additions}/-{proposal.total_deletions} lines"
        )
        return proposal

    def _generate_diff_explanation(
        self, plan: FixPlan, files_changed: List[FileDiff]
    ) -> str:
        """Generate explanation of what the diffs accomplish"""
        explanation = (
            "These changes implement the fix plan to resolve diagnostic issues:\n\n"
        )
    ...
    async def apply_changes(
        ...
        logger.info(
            f"Phase 4.3 Apply complete: {len(result.files_updated)} updated, {len(result.files_failed)} failed"
        )
        return result

    async def verify_results(
        self, task: GroundedTask, apply_result: ApplyResult, context: Dict[str, Any]
    ) -> VerificationResult:
        """
        ...
        if self.file_mutator:
            verification = await self.file_mutator.verify_changes(
                apply_result, original_issues, context
            )
        else:
    ...
    async def orchestrate_full_workflow(
        ...
        """
        logger.info(
            "🚀 Starting Phase 4.4 Enterprise Autonomy workflow with Phase 4.5 safety"
        )
        ...
        try:
            ...
            if self.snapshot_engine and apply_result.files_updated:
                ...
                try:
                    ...
                    logger.info(
                        f"✅ Safety snapshot created: {snapshot.metadata.snapshot_id}"
                    )
            ...
            commit_info = await self._create_staff_commit(
                apply_result, context, branch_name
            )
            ...
            if pr_result and pr_result.success:
                ...
                if pr_result.pr_number is not None:
                    ...
                    workflow_result["ci_status"] = (
                        ci_result.overall_status.value if ci_result else "unknown"
                    )
                ...
                if ci_result and self.rollback_engine:
                    rollback_needed = await self._evaluate_rollback_necessity(
                        ci_result, context
                    )
                    if rollback_needed:
            ...
            if self.snapshot_engine:
                ...
                workflow_result["rollback_available"] = (
                    self.rollback_engine.can_rollback()
            ...
            logger.info(
                "🎉 Phase 4.4 workflow with Phase 4.5 safety completed successfully"
            )
    ...
    async def _create_staff_commit(
        self, apply_result: ApplyResult, context: Dict[str, Any], branch_name: str
    ):
...
All changes verified and diagnostics are now clean."""
        ...
        commit_info = await self.commit_engine.commit_changes(
            files=apply_result.files_updated, message=summary, description=description
        )
    ...
    async def _monitor_ci_pipeline(
        self, repository: str, pr_number: int, context: Dict[str, Any]
    ):
        ...
        timeout_minutes = context.get(
            "ci_timeout_minutes", 15
        )  # Shorter for initial implementation
    ...
    async def _decide_next_action(
        self, ci_result, context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def perform_rollback(
        self, trigger: RollbackTrigger, snapshot_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if preview:
                logger.info(
                    f"Rolling back {preview['files_count']} files from {preview['created_at']}"
                )
            ...
            if snapshot_id and self.snapshot_engine:
                ...
                rollback_result = self.rollback_engine.rollback_to_snapshot(
                    snapshot, trigger
                )
            else:
    ...
    async def _evaluate_rollback_necessity(
        self, ci_result, context: Dict[str, Any]
    ) -> bool:
        ...
        failure_threshold = context.get(
            "rollback_ci_threshold", 3
        )  # Number of failures

        if (
            hasattr(ci_result, "failed_checks")
            and len(ci_result.failed_checks) >= failure_threshold
        ):
            logger.warning(
                f"CI has {len(ci_result.failed_checks)} failures, exceeding threshold of {failure_threshold}"
            )
            return True
        ...
        if hasattr(ci_result, "overall_status"):
            ...
            if ci_result.overall_status.value in critical_statuses:
                logger.warning(
                    f"CI status {ci_result.overall_status.value} suggests rollback"
                )
                return True
    ...
    def get_safety_status(self) -> Dict[str, Any]:
        ...
        if not self.snapshot_engine or not self.rollback_engine:
            return {"safety_available": False, "error": "Safety system not initialized"}
        ...
        try:
            ...
            try:
                rollback_stats = (
                    self.rollback_engine.get_rollback_statistics()
                    if hasattr(self.rollback_engine, "get_rollback_statistics")
                    else {}
                )
            except Exception as e:
            ...
            return {"safety_available": False, "error": str(e)}
    ...
    async def handle_ci_failure(
        self, ci_event: CIEvent, auto_repair: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Handling CI failure for {ci_event.repo_name} run {ci_event.run_id}"
        )
        ...
        try:
            ...
            result = {
                "success": (
                    repair_session.result.success if repair_session.result else False
                ),
                "session_id": repair_session.session_id,
                "failure_type": (
                    repair_session.failure_context.failure_type.value
                    if repair_session.failure_context
                    else "unknown"
                ),
                "confidence": (
                    repair_session.failure_context.confidence
                    if repair_session.failure_context
                    else 0.0
                ),
                "action_taken": (
                    repair_session.result.action_taken.value
                    if repair_session.result
                    else "none"
                ),
                "files_modified": (
                    repair_session.result.files_modified
                    if repair_session.result
                    else []
                ),
                "duration_seconds": (
                    int(
                        (
                            repair_session.completed_at - repair_session.started_at
                        ).total_seconds()
                    )
                    if repair_session.completed_at and repair_session.started_at
                    else 0
                ),
            ...
            logger.info(
                f"CI repair completed: {result['success']} - {result['action_taken']}"
            )
            return result
    ...
    async def execute_ci_auto_repair(
        ...
        try:
            ...
            logger.info(
                f"Executing CI auto-repair: {repair_strategy} for {failure_type}"
            )

            # Create synthetic task for repair execution
            synthetic_diagnostics = self._create_diagnostics_from_ci_context(
                repair_context
            )

            # Use existing analysis and repair capabilities
            synthetic_task = self._create_synthetic_task(
                synthetic_diagnostics, repair_strategy
            )

            # Execute repair using existing workflow
            analysis = await self.analyze(
                synthetic_task, {"workspace_root": self.workspace_root}
            )

            if not getattr(
                analysis, "can_fix", True
            ):  # Default to True if attribute missing
                return {
                    "success": False,
                    ...
                    "confidence": getattr(
                        analysis,
                        "overall_confidence",
                        repair_context.get("confidence", 0.0),
                    ),
                }
            ...
            plan_success = (
                analysis.fixable_count > 0
                if hasattr(analysis, "fixable_count")
                else True
            )
    ...
    def _create_diagnostics_from_ci_context(
        self, repair_context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _create_synthetic_task(
        self, diagnostics: List[Dict[str, Any]], repair_strategy: str
    ) -> GroundedTask:
    ...
    def get_ci_repair_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": str(e), "total_sessions": 0, "success_rate": 0.0}
