...
class PrCommentAnalyzer:
    ...
    def analyze_comment(
        self, comment: PrComment, context: ReviewContext
    ) -> CommentClassification:
        """
    ...
    def analyze_multiple_comments(
        self, comments: List[PrComment], context: ReviewContext
    ...
    def get_fixable_comments(
        ...
        logger.info(
            f"Found {len(fixable)} fixable comments out of {len(classifications)}"
        )
        return fixable
    ...
    def _extract_comment_features(
        ...
        features = {
            ...
            "file_type": (
                self._get_file_type(comment.file_path) if comment.file_path else None
            ),
    ...
    def _classify_comment_type(
        self, features: Dict[str, Any]
    ) -> Tuple[CommentType, float]:
    ...
    def _determine_action(
        self, comment_type: CommentType, features: Dict[str, Any], confidence: float
    ) -> FixAction:
        ...
        action_map = {
            ...
            CommentType.LOGIC_ERROR: (
                FixAction.CODE_FIX if confidence > 0.7 else FixAction.ESCALATE
            ),
    ...
    def _is_comment_fixable(
        self, comment_type: CommentType, features: Dict[str, Any]
    ) -> bool:
    ...
    def _generate_reasoning(
        ...
        reasoning = (
            f"Classified as {comment_type.value} (confidence: {confidence:.2f}). "
        )
