...
class PrCommentFetcher:
    ...
    async def fetch_pr_context(
        ...
        try:
            ...
            files_changed = await self.github_service.get_pr_files(
                repository, pr_number
            )
            file_paths = (
                [f.get("filename", "") for f in files_changed] if files_changed else []
            )
            ...
            logger.error(
                f"Failed to fetch PR context for {repository}#{pr_number}: {e}"
            )
            return None
    ...
    async def fetch_pr_comments(
        ...
        try:
            ...
            issue_comments = await self.github_service.get_pr_comments(
                repository, pr_number
            )
            ...
            review_comments = await self.github_service.get_pr_review_comments(
                repository, pr_number
            )
            ...
            logger.error(
                f"Failed to fetch PR comments for {repository}#{pr_number}: {e}"
            )
            return []

    async def fetch_new_comments_since(
        self, repository: str, pr_number: int, since: datetime
    ) -> List[PrComment]:
        """
        ...
        new_comments = [
            comment for comment in all_comments if comment.created_at > since
        ]
    ...
    async def fetch_unresolved_comments(
        ...
        unresolved_comments = [
            comment for comment in all_comments if not comment.is_resolved
        ]
    ...
    async def post_comment_reply(
        self, repository: str, pr_number: int, comment_id: str, reply_text: str
    ) -> bool:
        """
        ...
        try:
            ...
            result = await self.github_service.add_pr_comment(
                repository, pr_number, reply_text
            )
    ...
    async def resolve_comment(
        ...
        try:
            ...
            result = await self.github_service.resolve_review_comment(
