...
class RollbackEngine:
    ...
    def __init__(
        self, workspace_root: str, snapshot_engine: Optional[SnapshotEngine] = None
    ):
    ...
    def rollback_to_snapshot(
        ...
        try:
            ...
            logger.info(
                f"Restoring git state: branch={snapshot.git_state.current_branch}"
            )
    ...
    def rollback_files_only(
        self, file_paths: List[str], trigger: RollbackTrigger
    ...
    def emergency_rollback(self) -> Optional[RollbackResult]:
        ...
        return self.rollback_to_snapshot(
            latest_snapshot, RollbackTrigger.SAFETY_OVERRIDE, verify_integrity=False
        )

    def _restore_file(
        self, file_path: str, file_state: FileState, verify_integrity: bool = True
    ) -> None:
    ...
    def _restore_git_state(self, git_state: GitState) -> None:
        ...
        try:
            ...
            if result.returncode != 0:
                logger.warning(
                    "Not in a git repository, skipping git state restoration"
                )
                return
            ...
            if current_branch != git_state.current_branch:
                logger.info(
                    f"Switching from {current_branch} to {git_state.current_branch}"
                )
                ...
                if result.returncode != 0:
                    raise RuntimeError(
                        f"Failed to checkout branch {git_state.current_branch}: {result.stderr}"
                    )
            ...
            if git_state.commit_sha:
                ...
                if result.returncode != 0:
                    logger.warning(
                        f"Failed to reset to {git_state.commit_sha}: {result.stderr}"
                    )
