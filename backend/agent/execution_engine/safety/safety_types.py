...
class GitState(BaseModel):
    ...
    uncommitted_changes: List[str] = Field(
        default_factory=list, description="List of uncommitted files"
    )
...
class RollbackResult(BaseModel):
    ...
    files_failed: List[str] = Field(
        default_factory=list, description="List of files that failed to restore"
    )
    ...
    error_message: Optional[str] = Field(
        default=None, description="Error message if rollback failed"
    )
...
class SafetyReport(BaseModel):
    ...
    snapshot_available: bool = Field(
        description="Whether a rollback snapshot is available"
    )
    snapshot_age_minutes: Optional[int] = Field(
        default=None, description="Age of latest snapshot in minutes"
    )
    risk_factors: List[str] = Field(
        default_factory=list, description="Current risk factors"
    )
    recommended_actions: List[str] = Field(
        default_factory=list, description="Recommended safety actions"
    )
    can_rollback: bool = Field(description="Whether rollback is currently possible")
    rollback_scope: List[str] = Field(
        default_factory=list, description="What would be rolled back"
    )
