...
from .safety_types import (
    FileState, GitState, SnapshotMetadata, SafetyStatus, 
    SafetyReport
)
...
class Snapshot:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Snapshot":
        ...
        files = {
            ...
            for path, state_data in data["files"].items()
        }
    ...
    def get_total_size(self) -> int:
        ...
        return sum(len(file_state.content.encode('utf-8')) for file_state in self.files.values())
    ...
    def cleanup_old_snapshots(self, keep_count: int = 5) -> int:
        ...
        for snapshot in snapshots_to_remove:
            try:
                ...
                logger.warning(f"Failed to cleanup snapshot {snapshot.metadata.snapshot_id}: {e}")
