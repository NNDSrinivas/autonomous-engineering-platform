...
class DiagnosticIssue(BaseModel):
    ...
    severity: Literal["error", "warning", "info", "hint"] = Field(description="Issue severity")
    source: Optional[str] = Field(default=None, description="Diagnostic source (e.g., 'eslint', 'typescript')")
    code: Optional[str] = Field(default=None, description="Diagnostic code if available")
    category: str = Field(description="Issue category (e.g., 'ReferenceError', 'SyntaxError')")
    confidence: float = Field(ge=0.0, le=1.0, description="Confidence in analysis")
    fixable: bool = Field(default=True, description="Whether this issue can be automatically fixed")
...
class AnalysisResult(BaseModel):
    ...
    estimated_complexity: Literal["low", "medium", "high"] = Field(description="Estimated fix complexity")
...
class FixStep(BaseModel):
    ...
    risk_level: Literal["low", "medium", "high"] = Field(description="Risk level of this step")
...
class FixPlan(BaseModel):
    ...
    requires_tests: bool = Field(default=False, description="Whether tests should be run after fixes")
...
class DiffProposal(BaseModel):
    ...
    explanation: str = Field(description="Detailed explanation of why these changes fix the issues")
    ...
    risk_assessment: Literal["low", "medium", "high"] = Field(description="Risk assessment")
    safety_checks: List[str] = Field(description="Safety checks performed before proposing")
...
class ApplyResult(BaseModel):
    ...
    backup_created: bool = Field(default=False, description="Whether backups were created")
    # Phase 4.3 additions
    session_id: Optional[str] = Field(default=None, description="Session ID for tracking changes")
    backup_location: Optional[str] = Field(default=None, description="Location of backup files")
...
class VerificationResult(BaseModel):
    ...
    status: Literal["resolved", "partially_resolved", "failed", "introduced_new_issues"] = Field(
        description="Overall verification status"
    )
    verification_details: str = Field(description="Detailed verification report")
    next_steps: List[str] = Field(default_factory=list, description="Suggested next steps if needed")
    # Phase 4.3 additions
    success: bool = Field(description="Whether verification passed")
    message: str = Field(description="Verification message")
    remaining_issues_list: List[Any] = Field(default_factory=list, alias="remaining_issues_data", description="Detailed remaining issues")
    new_issues_detected: List[Any] = Field(default_factory=list, description="List of new issues detected")
    files_verified: List[str] = Field(default_factory=list, description="List of files that were verified")
...
class ExecutionResult(BaseModel):
    ...
    analysis: Optional[AnalysisResult] = Field(default=None, description="Analysis results")
    plan: Optional[FixPlan] = Field(default=None, description="Fix plan generated")
    proposal: Optional[DiffProposal] = Field(default=None, description="Diff proposal created")
    apply_result: Optional[ApplyResult] = Field(default=None, description="Application results")
    verification: Optional[VerificationResult] = Field(default=None, description="Verification results")
    workflow_result: Optional[Dict[str, Any]] = Field(default=None, description="Phase 4.4 workflow results")
    execution_time_ms: int = Field(description="Total execution time in milliseconds")
    user_approved: bool = Field(default=False, description="Whether user approved the changes")
