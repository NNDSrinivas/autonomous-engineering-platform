...
class AuditLogger:
    ...
    def log_decision(
        ...
        logger.info(
            f"Audit log: {decision} for {action_type} by {user_id} (risk: {risk_score:.2f})"
        )
    ...
    def log_execution(
        ...
        logger.info(
            f"Execution audit: {action_type} by {user_id} -> {execution_result}"
        )
    ...
    def log_rollback(
        ...
        entry = AuditEntry(
            ...
            artifacts={"original_action_id": original_action_id, **(artifacts or {})},
        )

        self._store_audit_entry(entry)

        logger.info(
            f"Rollback audit: {original_action_id} by {user_id} -> {rollback_result}"
        )
    ...
    def get_risk_insights(self, org_id: str, days: int = 30) -> Dict[str, Any]:
    ...
    def _extract_decision_factors(
        self, context: ActionContext, risk_score: float
    ) -> Dict[str, Any]:
        """Extract key decision factors for audit trail"""
        factors = {
            "risk_score": risk_score,
            "action_scope": (
                len(context.target_files) if context and context.target_files else 0
            ),
        }
