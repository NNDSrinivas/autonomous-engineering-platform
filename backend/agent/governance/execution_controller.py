...
from ..closedloop.execution_controller import ExecutionController, ExecutionResult, ExecutionStatus
...
class GovernedExecutionController(ExecutionController):
    ...
    def __init__(
        ...
        # Execution state
        self.pending_executions: Dict[str, Dict] = {}  # approval_id -> execution context
        self.governance_enabled = True
        self.safety_mode = True
        
        logger.info(f"Initialized governed execution controller for user {self.user_id}")
    ...
    async def execute_action(
        ...
        *,
        ...
        try:
            ...
            if result.status == ExecutionStatus.COMPLETED and self._supports_rollback(action):
    ...
    async def rollback_action(
        ...
        try:
            logger.info(f"Initiating rollback {rollback_id} requested by {requester_id}")
    ...
    async def _run_safety_checks(
        ...
        try:
            ...
            if action_context.touches_auth and action_context.touches_prod:
                issues.append("Action affects both authentication and production systems")
            ...
            if any(
                ...
                if not context_data.get("destructive_confirmed", False):
                    issues.append("Destructive operation requires explicit confirmation")
    ...
    def _detect_auth_impact(self, action: PlannedAction, context_data: Dict[str, Any]) -> bool:
        ...
        # Check target files
        auth_patterns = ['auth', 'security', 'permission', 'role', 'oauth', 'jwt', 'session']
        target_files = context_data.get("target_files", [])
    ...
    def _detect_prod_impact(self, context_data: Dict[str, Any]) -> bool:
        ...
        if any(indicator.lower() in ['production', 'prod', 'live'] for indicator in env_indicators):
            return True
    ...
    def _supports_rollback(self, action: PlannedAction) -> bool:
        ...
        rollback_supported = [
            'code_edit', 'config_change', 'feature_toggle', 
        ...
        rollback_forbidden = [
            'data_deletion', 'account_deletion', 'permanent_migration',
            'security_breach_response', 'audit_log_entry'
        ]
