...
class AutonomyPolicyStore:
        ...
        self.default_policies = {
            ...
            "platform_team": AutonomyPolicy(
                ...
                require_approval_for=[
                    "deploy_prod"
                ],  # Only prod deployments need approval
            ),
        }
    ...
    def _get_policy_from_db(
        self, user_id: str, org_id: str, repo: Optional[str] = None
    ) -> Optional[AutonomyPolicy]:
    ...
    def _get_default_policy_for_user(
        ...
        try:
            ...
            if default_policy:
                ...
                return AutonomyPolicy(
                    ...
                    blocked_actions=(
                        default_policy.blocked_actions.copy()
                        if default_policy.blocked_actions
                        else []
                    ),
                    auto_allowed_actions=(
                        default_policy.auto_allowed_actions.copy()
                        if default_policy.auto_allowed_actions
                        else []
                    ),
                    require_approval_for=(
                        default_policy.require_approval_for.copy()
                        if default_policy.require_approval_for
                        else []
                    ),
                )
        ...
        try:
            ...
            self.db.execute(
                ...
                """,
                {
                    ...
                    "auto_allowed_actions": json.dumps(
                        policy.auto_allowed_actions or []
                    ),
                    "require_approval_for": json.dumps(
                        policy.require_approval_for or []
                    ),
    ...
    def _policy_from_db_row(self, row) -> AutonomyPolicy:
        ...
        return AutonomyPolicy(
            ...
            blocked_actions=(
                json.loads(row["blocked_actions"]) if row["blocked_actions"] else []
            ),
            auto_allowed_actions=(
                json.loads(row["auto_allowed_actions"])
                if row["auto_allowed_actions"]
                else []
            ),
            require_approval_for=(
                json.loads(row["require_approval_for"])
