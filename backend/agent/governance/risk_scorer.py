...
class RiskScorer:
    ...
    def calculate_risk(
        ...
        if sensitivity_score > 0:
            ...
            if sensitivity_score > 0.5:
                reasons.append(
                    "Modifies sensitive files (config, auth, infrastructure)"
                )
            else:
    ...
    def _affects_production(self, action_type: str, context: ActionContext) -> bool:
        ...
        if context.branch and context.branch.lower() in [
            "main",
            "master",
            "production",
            "prod",
        ...
        if context.repo and any(
            keyword in context.repo.lower() for keyword in ["prod", "production"]
        ):
            return True
