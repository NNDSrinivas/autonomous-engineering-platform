...
class BlastRadiusAnalyzer:
    ...
    def analyze_change_impact(
        ...
        indirectly_affected, impact_paths = self._trace_dependency_impact(
            directly_affected
        )

        # Identify affected external dependencies
        affected_dependencies = self._identify_affected_dependencies(
            changed_files, incidents
        )

        # Calculate overall risk score
        risk_score = self._calculate_impact_risk(
            directly_affected, indirectly_affected, affected_dependencies, incidents
        )

        # Determine blast radius category
        blast_radius = self._categorize_blast_radius(
            directly_affected, indirectly_affected, changed_files
        )

        # Generate recommendations
        recommendations = self._generate_impact_recommendations(
            blast_radius, directly_affected, indirectly_affected, risk_score
        )
    ...
    def _identify_directly_affected_services(
    ...
    def _trace_dependency_impact(
        self, directly_affected: List[str]
            ...
            while queue:
                ...
                for service, dependencies in self.dependency_graph.items():
                    for dep in dependencies:
                        if (
                            dep.target_service == current_service
    ...
    def _generate_impact_recommendations(
        ...
        if blast_radius == "SYSTEM_WIDE":
            recommendations.append(
                "🚨 SYSTEM-WIDE IMPACT: Coordinate with all affected teams"
            )
            ...
            recommendations.append(
                "🔗 CROSS-SERVICE IMPACT: Notify affected service owners"
            )
            ...
            recommendations.append(
                "🎯 SERVICE-LEVEL IMPACT: Focus testing on affected service"
            )
        ...
        if risk_score > 0.7:
            recommendations.append(
                "⚠️ HIGH RISK: Consider feature flags or canary deployment"
            )
        ...
        if critical_services:
            recommendations.append(
                f"🏗️ Critical services affected: {', '.join(critical_services)}"
            )
            recommendations.append("👥 Require senior engineer approval")

        if indirectly_affected:
            recommendations.append(
                f"📡 Monitor downstream services: {', '.join(indirectly_affected[:3])}"
            )
    ...
    def get_service_risk_profile(
        self, service_name: str, incidents: List[Incident]
    ) -> ChangeRisk:
        ...
        if incident_count > 0:
            ...
            recent_incidents = [
                ...
                if (incidents[0].timestamp - inc.timestamp).days
                <= 30  # Assuming incidents are sorted
            ]
            if recent_incidents:
                risk_factors.append(
                    f"{len(recent_incidents)} incidents in last 30 days"
                )
