...
class FlakyTestDetector:
    ...
    def analyze_test_stability(self, incidents: List[Incident]) -> Tuple[List[FlakyTest], TestSuiteHealth]:
        """
    ...
    def _filter_test_incidents(self, incidents: List[Incident]) -> List[Incident]:
        ...
        for incident in incidents:
            ...
            if incident.error_message and self._error_suggests_test_failure(incident.error_message):
                test_incidents.append(incident)
        
        return test_incidents
    
    def _analyze_individual_tests(self, test_incidents: List[Incident]) -> Dict[str, TestStability]:
    ...
    def _calculate_test_stability(self, test_path: str, incidents: List[Incident]) -> TestStability:
        ...
        if len(incidents) >= 2:
            ...
            estimated_runs = max(failures, int(time_span.total_seconds() / (6 * 3600)))  # Every 6 hours
        else:
        ...
        flakiness_score = self._calculate_flakiness_score(sorted_incidents, failure_rate)
    ...
    def _calculate_flakiness_score(self, sorted_incidents: List[Incident], failure_rate: float) -> float:
        """
        ...
        if time_span.total_seconds() == 0:
            ...
            for i in range(1, len(sorted_incidents)):
                interval = (sorted_incidents[i].timestamp - sorted_incidents[i-1].timestamp).total_seconds()
            ...
            if len(time_intervals) >= 2:
                ...
                temporal_factor = min(1.0, std_interval / mean_interval) if mean_interval > 0 else 0.0
            else:
                temporal_factor = 0.5
        
        # Factor 2: Failure rate optimum (most flaky around 50%)
        rate_factor = 1.0 - abs(failure_rate - 0.5) * 2  # Peak at 0.5, drops to 0 at 0.0 and 1.0
        
        # Factor 3: Error message diversity (flaky tests often have varying errors)
        error_messages = [inc.error_message for inc in sorted_incidents if inc.error_message]
        if error_messages:
    ...
    def _calculate_stability_confidence(self, incidents: List[Incident], failure_rate: float) -> float:
    ...
    def _identify_flaky_tests(self, stability_map: Dict[str, TestStability], 
                             incidents: List[Incident]) -> List[FlakyTest]:
        ...
        for test_path, stability in stability_map.items():
            if stability.flakiness_score >= self.flakiness_threshold:
                ...
                test_incidents = [
                    ...
                    if test_path in inc.files
                ]
                
                # Analyze impact and generate recommendations
                flaky_test = self._analyze_flaky_test(test_path, stability, test_incidents)
                flaky_tests.append(flaky_test)
        
        # Sort by priority (highest first)
        return sorted(flaky_tests, key=lambda x: x.stabilization_priority, reverse=True)
    
    def _analyze_flaky_test(self, test_path: str, stability: TestStability, 
                           incidents: List[Incident]) -> FlakyTest:
        ...
        priority = self._calculate_stabilization_priority(stability, impact_level, len(incidents))
    ...
    def _hypothesize_root_cause(self, stability: TestStability, incidents: List[Incident]) -> str:
        ...
        if any(keyword in error_text for keyword in ["timeout", "connection", "network"]):
        ...
        elif any(keyword in error_text for keyword in ["order", "sequence", "dependency"]):
    ...
    def _calculate_impact_level(self, stability: TestStability, incidents: List[Incident]) -> str:
    ...
    def _determine_recommended_action(self, stability: TestStability, impact_level: str) -> str:
    ...
    def _calculate_stabilization_priority(self, stability: TestStability, 
                                         impact_level: str, incident_count: int) -> int:
    ...
    def _assess_suite_health(self, stability_map: Dict[str, TestStability], 
                           flaky_tests: List[FlakyTest]) -> TestSuiteHealth:
        """Assess overall test suite health"""
        ...
        critical_flaky = len([ft for ft in flaky_tests if ft.impact_level == "CRITICAL"])
        ...
        recommendations = self._generate_suite_recommendations(flaky_tests, overall_stability)
    ...
    def _generate_suite_recommendations(self, flaky_tests: List[FlakyTest], 
                                      overall_stability: float) -> List[str]:
        """Generate recommendations for test suite health"""
        recommendations = []
        
        if overall_stability < 0.8:
            recommendations.append("🚨 URGENT: Test suite stability below 80% - immediate attention required")
        
        critical_tests = [ft for ft in flaky_tests if ft.impact_level == "CRITICAL"]
        if critical_tests:
            recommendations.append(f"⛔ Disable/quarantine {len(critical_tests)} critical flaky tests immediately")
        
        high_impact_tests = [ft for ft in flaky_tests if ft.impact_level == "HIGH"]
        if len(high_impact_tests) > 2:
            recommendations.append(f"🔥 Prioritize fixing {len(high_impact_tests)} high-impact flaky tests")
        
        # Pattern-based recommendations
        race_condition_tests = [ft for ft in flaky_tests if "race" in ft.root_cause_hypothesis.lower()]
        if len(race_condition_tests) >= 2:
            recommendations.append("🏃 Multiple race condition tests detected - review test parallelization")
        
        timeout_tests = [ft for ft in flaky_tests if "timeout" in ft.root_cause_hypothesis.lower()]
        if len(timeout_tests) >= 2:
            recommendations.append("⏱️ Multiple timeout-related flaky tests - review external dependencies")
        
        if len(flaky_tests) > 5:
            recommendations.append("🛠️ Consider implementing flaky test quarantine system")
    ...
    def _error_suggests_test_failure(self, error_message: str) -> bool:
        ...
        test_keywords = [
            "test failed", "assertion", "assert", "expect",
            "junit", "pytest", "test case", "test suite"
        ]
