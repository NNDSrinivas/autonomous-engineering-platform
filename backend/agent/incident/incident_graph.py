...
class IncidentGraph:
        ...
        for incident in incidents:
            ...
            for i, file1 in enumerate(files):
                for file2 in files[i+1:]:
                    self._add_or_update_edge(file1, file2, "co_occurs", 0.1, incident.id)
        ...
        for incident in incidents:
            if incident.author:
                ...
                for file_path in incident.files:
                    ...
                    self._add_or_update_edge(author_node, file_node, "authored_by", 0.3, incident.id)
    ...
    def _build_dependency_edges(self, incidents: List[Incident]) -> None:
        ...
        for incident in incidents:
            if incident.incident_type == IncidentType.DEPENDENCY_ISSUE and incident.tags:
                for tag in incident.tags:
                    if tag.startswith("dependency:"):
                        dep_name = tag.replace("dependency:", "")
                        ...
                        for file_path in incident.files:
                            file_node = f"file:{file_path}"
                            self._add_or_update_edge(dep_node, file_node, "depends_on", 0.5, incident.id)
            ...
            for incident2 in sorted_incidents[i+1:]:
                ...
                if time_diff <= timedelta(hours=1):
                    if self._have_shared_entities(incident1, incident2):
                        ...
                        strength = max(0.1, 1.0 - (time_diff.total_seconds() / 3600))  # Closer in time = stronger
                        self._add_or_update_edge(node1, node2, "temporal_cascade", strength, incident2.id)
                else:
    ...
    def _have_shared_entities(self, incident1: Incident, incident2: Incident) -> bool:
        ...
        return (
            bool(set(incident1.files) & set(incident2.files)) or  # Shared files
            (incident1.author and incident1.author == incident2.author) or  # Same author
            (incident1.repo == incident2.repo)  # Same repo
        )
    
    def _add_or_update_edge(self, source: str, target: str, relationship_type: str, strength: float, incident_id: str) -> None:
    ...
    def _identify_hotspots(self) -> None:
        ...
        for node_id, node in hotspots.items():
            ...
            self.patterns.append(CausalPattern(
    ...
    def _identify_flaky_patterns(self) -> None:
        ...
        test_files = {k: v for k, v in self.nodes.items() 
                     if v.entity_type == "file" and ("test" in v.entity_id.lower() or "spec" in v.entity_id.lower())}
    ...
    def _identify_author_patterns(self) -> None:
        ...
        author_nodes = {k: v for k, v in self.nodes.items() if v.entity_type == "author"}
    ...
    def _identify_cascade_patterns(self) -> None:
        ...
        cascade_edges = [e for e in self.edges.values() if e.relationship_type == "temporal_cascade"]
    ...
    def get_graph_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_hotspots": [
                ...
                for node in sorted(self.nodes.values(), key=lambda n: n.failure_count, reverse=True)[:5]
