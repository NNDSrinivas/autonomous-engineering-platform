...
class IncidentOrchestrator:
    ...
    def __init__(self, incident_store: IncidentStore):
        ...
        logger.info("IncidentOrchestrator initialized - Autonomous Engineering Intelligence ready")
    
    def analyze_repository_health(self, repo: str, 
                                 lookback_days: int = 30) -> IntelligenceSummary:
        """
        ...
        pattern_analysis = self.pattern_analyzer.analyze_incidents(incidents, incident_graph)
        flaky_tests, test_health = self.flaky_test_detector.analyze_test_stability(incidents)
        
        # Synthesize intelligence
        critical_issues = self._identify_critical_issues(pattern_analysis, flaky_tests, test_health)
        systemic_problems = self._identify_systemic_problems(pattern_analysis, incident_graph)
        immediate_actions = self._prioritize_immediate_actions(critical_issues, systemic_problems)
        preventive_measures = self._generate_preventive_measures(pattern_analysis, flaky_tests)
        ...
        return IntelligenceSummary(
            ...
            next_review_date=self._calculate_next_review_date(critical_issues, systemic_problems)
        )
    
    def make_incident_decision(self, incident: Incident, 
                              context_incidents: Optional[List[Incident]] = None) -> IncidentDecision:
        """
        ...
        if context_incidents is None:
            context_incidents = self.incident_store.get_recent_incidents(incident.repo, limit=50)
        
        # Analyze incident in context
        incident_graph = build_incident_graph([incident] + context_incidents)
        pattern_analysis = self.pattern_analyzer.analyze_incidents([incident] + context_incidents, incident_graph)
        ...
        decision = self._determine_incident_action(incident, is_recurring, severity, impact_analysis)
        ...
        decision.evidence = {
            ...
            "similar_incidents": len([
                inc for inc in context_incidents 
    ...
    def predict_and_prevent(self, changed_files: List[str], 
                           repo: str, 
                           author: Optional[str] = None) -> RegressionRisk:
        """
        ...
        """
        logger.info(f"Predicting regression risk for {len(changed_files)} files in {repo}")
        ...
        impact_analysis = self.blast_radius_analyzer.analyze_change_impact(changed_files, incidents)
        
        # Augment recommendations with system-level thinking
        enhanced_recommendations = risk.recommendations + impact_analysis.recommendations
        risk.recommendations = list(dict.fromkeys(enhanced_recommendations))[:8]  # Remove duplicates, limit
    ...
    def handle_ci_failure(self, ci_event: Dict[str, Any]) -> IncidentDecision:
        ...
        if incident.incident_type == IncidentType.TEST_FAILURE:
            flaky_tests, _ = self.flaky_test_detector.analyze_test_stability(context_incidents + [incident])
            
            # If this looks like a flaky test, handle differently
            for flaky_test in flaky_tests:
                if any(file_path in flaky_test.test_path for file_path in incident.files):
    ...
    def _identify_critical_issues(self, pattern_analysis: AnalysisResult, 
                                 flaky_tests: List[FlakyTest],
                                 test_health: TestSuiteHealth) -> List[str]:
        """Identify critical issues requiring immediate attention"""
        issues = []
        
        # Critical systemic issues
        critical_systemic = [si for si in pattern_analysis.systemic_issues if si.severity == "CRITICAL"]
        if critical_systemic:
            issues.append(f"CRITICAL: {len(critical_systemic)} systemic issues requiring immediate action")
        
        # Critical flaky tests
        critical_flaky = [ft for ft in flaky_tests if ft.impact_level == "CRITICAL"]
        if critical_flaky:
            issues.append(f"CRITICAL: {len(critical_flaky)} critical flaky tests blocking pipeline")
        
        # High-frequency recurring failures
        high_freq_failures = [rf for rf in pattern_analysis.recurring_failures if rf.failure_rate > 1.0]
        if high_freq_failures:
            issues.append(f"HIGH: {len(high_freq_failures)} files failing daily or more")
        
        # Test suite health
        if test_health.overall_stability < 0.5:
            issues.append("CRITICAL: Test suite stability below 50% - pipeline unreliable")
        
        return issues
    
    def _identify_systemic_problems(self, pattern_analysis: AnalysisResult, 
                                   incident_graph: IncidentGraph) -> List[str]:
        """Identify systemic problems affecting multiple components"""
        problems = []
        
        # Architectural issues
        arch_issues = [si for si in pattern_analysis.systemic_issues if si.issue_type == "architecture"]
        if arch_issues:
            problems.append("Architecture: Cascading failures indicate tight coupling")
        
        # Process issues  
        process_issues = [si for si in pattern_analysis.systemic_issues if si.issue_type == "process"]
        if process_issues:
        ...
        if len(hotspots) >= 3:
            problems.append(f"Quality: {len(hotspots)} failure hotspots need refactoring")
    ...
    def _prioritize_immediate_actions(self, critical_issues: List[str], 
                                     systemic_problems: List[str]) -> List[str]:
        ...
        if any("CRITICAL" in issue for issue in critical_issues):
            ...
            actions.append("🔒 Consider deployment freeze until critical issues resolved")
    ...
    def _generate_preventive_measures(self, pattern_analysis: AnalysisResult, 
                                     flaky_tests: List[FlakyTest]) -> List[str]:
        """Generate preventive measures to avoid future incidents"""
        measures = []
        
        # Based on recurring failures
        if pattern_analysis.recurring_failures:
            measures.append("Implement pre-commit hooks for files with high failure rates")
    ...
    def _is_recurring_incident(self, incident: Incident, analysis: AnalysisResult) -> bool:
        """Check if incident is part of a recurring pattern"""
        for recurring_failure in analysis.recurring_failures:
            if any(file_path in incident.files for file_path in [recurring_failure.entity]):
                return True
        return False
    
    def _assess_incident_severity(self, incident: Incident, context: List[Incident]) -> str:
        ...
        similar_recent = [
            inc for inc in context 
            if (inc.failure_type == incident.failure_type and 
                (incident.timestamp - inc.timestamp).days <= 7)
        ]
    ...
    def _determine_incident_action(self, incident: Incident, is_recurring: bool, 
                                  severity: str, impact: ImpactAnalysis) -> IncidentDecision:
        ...
        return IncidentDecision(
            ...
            monitoring_requirements=self._generate_monitoring_requirements(incident, impact)
        )
    ...
    def _generate_monitoring_requirements(self, incident: Incident, 
        ...
        if impact.directly_affected_services:
            requirements.append(f"Monitor health of: {', '.join(impact.directly_affected_services)}")
        
        if incident.incident_type in [IncidentType.PERFORMANCE_REGRESSION, IncidentType.RUNTIME_ERROR]:
    ...
    def _calculate_intelligence_confidence(self, analysis: AnalysisResult, 
                                         flaky_test_count: int, 
                                         incident_count: int) -> float:
    ...
    def _calculate_next_review_date(self, critical_issues: List[str], 
                                   systemic_problems: List[str]) -> datetime:
        """Calculate when next review should happen"""
        from datetime import timedelta
        
        if critical_issues:
            return datetime.now() + timedelta(days=1)  # Daily review for critical issues
