...
class IncidentStore:
    ...
    def _init_database(self) -> None:
        ...
        with sqlite3.connect(str(self.db_path)) as conn:
            ...
            conn.execute("CREATE INDEX IF NOT EXISTS idx_repo_timestamp ON incidents(repo, timestamp)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_incident_type ON incidents(incident_type)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_resolved ON incidents(resolved)")
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                """, (
                    ...
                    json.dumps(data.get('failure_context')) if data.get('failure_context') else None,
                    json.dumps(data.get('related_incidents')) if data.get('related_incidents') else None,
                    data['severity'],
                    json.dumps(data.get('tags')) if data.get('tags') else None
                ))
                
                conn.commit()
                logger.info(f"Recorded incident: {incident.id} ({incident.incident_type.value}) in {incident.repo}")
            ...
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                for row in cursor.fetchall():
                    ...
                    if incident_data['failure_context']:
                        incident_data['failure_context'] = json.loads(incident_data['failure_context'])
                    if incident_data['related_incidents']:
                        incident_data['related_incidents'] = json.loads(incident_data['related_incidents'])
    ...
    def get_incidents_by_type(self, incident_type: IncidentType, repo: Optional[str] = None, limit: int = 100) -> List[Incident]:
    ...
    def get_incidents_by_files(self, files: List[str], repo: str, limit: int = 50) -> List[Incident]:
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                if cursor.rowcount > 0:
                    conn.commit()
                    logger.info(f"Marked incident {incident_id} as resolved with commit {resolution_commit}")
                    return True
                else:
    ...
    def get_incident_statistics(self, repo: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                # Total incidents
                cursor = conn.execute(f"SELECT COUNT(*) as total FROM incidents {where_clause}", params)
                ...
                cursor = conn.execute(f"""
                    ...
                    ORDER BY count DESC
                """, params)
                by_type = {row['incident_type']: row['count'] for row in cursor.fetchall()}
                ...
                resolution_rate = resolution_data['resolved'] / resolution_data['total'] if resolution_data['total'] > 0 else 0
    ...
    def cleanup_old_incidents(self, days_to_keep: int = 90) -> int:
        ...
        try:
            with sqlite3.connect(str(self.db_path)) as conn:
                ...
                logger.info(f"Cleaned up {deleted_count} old incidents (older than {days_to_keep} days)")
