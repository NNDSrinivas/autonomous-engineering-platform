...
class PatternAnalyzer:
    ...
    def analyze_incidents(
        self, incidents: List[Incident], incident_graph: Optional[IncidentGraph] = None
    ) -> AnalysisResult:
        """
        ...
        recommendations = self._generate_recommendations(
            recurring_failures, systemic_issues, temporal_patterns
        )

        # Calculate overall confidence
        confidence_score = self._calculate_confidence_score(
            recurring_failures, systemic_issues
        )
    ...
    def _analyze_recurring_failures(
        ...
        filtered_failures = [
            ...
            if rf.failure_count >= self.min_recurrence_threshold
            and rf.confidence >= self.min_confidence
        ]
            ...
            if len(file_incident_list) >= self.min_recurrence_threshold:
                ...
                recurring_failures.append(
                    RecurringFailure(
                        ...
                        suggested_remediation=self._suggest_file_remediation(
                            file_path, failure_types
                        ),
                    )
                )
    ...
    def _analyze_service_recurrence(
        self, incidents: List[Incident]
    ) -> List[RecurringFailure]:
                ...
                if confidence >= self.min_confidence:
                    ...
                    recurring_failures.append(
                        RecurringFailure(
                            ...
                            suggested_remediation=self._suggest_service_remediation(
                                service, service_incident_list
                            ),
                        )
                    )
    ...
    def _analyze_systemic_issues(
        self, incidents: List[Incident], incident_graph: IncidentGraph
    ) -> List[SystemicIssue]:
    ...
    def _analyze_architectural_issues(
        self, incidents: List[Incident], incident_graph: IncidentGraph
    ) -> List[SystemicIssue]:
                    ...
                    issues.append(
                        SystemicIssue(
                            ...
                            severity=self._calculate_severity(
                                len(pattern.incidents), pattern.confidence
                            ),
    ...
    def _analyze_process_issues(self, incidents: List[Incident]) -> List[SystemicIssue]:
        ...
        deployment_failures = [
            ...
            if inc.incident_type == IncidentType.DEPLOYMENT_FAILURE
        ]

        if (
            len(deployment_failures) >= 5
        ):  # Many deployment failures suggest process issues
    ...
    def _analyze_tooling_issues(self, incidents: List[Incident]) -> List[SystemicIssue]:
        ...
        build_failures = [
            inc for inc in incidents if inc.incident_type == IncidentType.BUILD_FAILURE
        ]
        ...
        if len(build_failures) >= 3:
            ...
            if (
                most_common and most_common[1] >= 2
            ):  # Same error type appears multiple times
            ...
            if len(set(all_incidents)) >= 5:  # Significant incident count
                ...
                issues.append(
                    SystemicIssue(
                        ...
                        severity=self._calculate_severity(
                            len(set(all_incidents)), confidence
                        ),
                        confidence=confidence,
    ...
    def _find_incident_spikes(
        self, sorted_incidents: List[Incident]
    ) -> List[TemporalPattern]:
    ...
    def _find_incident_trends(
        ...
        recent_incidents = [
            inc for inc in sorted_incidents if inc.timestamp >= earliest_for_trend
        ]

        if len(recent_incidents) >= 10:
            # Simple trend analysis: compare first half vs second half
            mid_point = len(recent_incidents) // 2
            first_half_rate = mid_point / max(
                1,
                (
                    recent_incidents[mid_point].timestamp
                    - recent_incidents[0].timestamp
                ).days,
            )
            second_half_rate = (len(recent_incidents) - mid_point) / max(
                1,
                (
                    recent_incidents[-1].timestamp
                    - recent_incidents[mid_point].timestamp
                ).days,
            )
    ...
    def _generate_recommendations(
        ...
        if len(recurring_failures) >= 3:
            recommendations.append(
                "🔥 URGENT: Address recurring failure hotspots - multiple files/tests failing repeatedly"
            )

            file_failures = [
                rf for rf in recurring_failures if rf.entity_type == "file"
            ]
            if len(file_failures) >= 2:
                recommendations.append(
                    "📁 Focus on file-level stability - consider refactoring high-failure files"
                )

            test_failures = [
                rf for rf in recurring_failures if rf.entity_type == "test"
            ]
            if len(test_failures) >= 2:
                recommendations.append(
                    "🧪 Test suite needs attention - stabilize or quarantine flaky tests"
                )

        # Recommendations based on systemic issues
        high_severity_issues = [
            si for si in systemic_issues if si.severity in ["HIGH", "CRITICAL"]
        ]
        if high_severity_issues:
            recommendations.append(
                "⚠️ CRITICAL: Systemic issues detected requiring architectural attention"
            )

            arch_issues = [
                si for si in systemic_issues if si.issue_type == "architecture"
            ]
            if arch_issues:
                recommendations.append(
                    "🏗️ Architecture review needed - cascading failures indicate tight coupling"
                )

            process_issues = [
                si for si in systemic_issues if si.issue_type == "process"
            ]
            if process_issues:
                recommendations.append(
                    "⚙️ Process improvements needed - deployment/build pipeline instability"
                )
        ...
        if spike_patterns:
            recommendations.append(
                "📈 Investigate incident spikes - possible external triggers or deployments"
            )

        trend_patterns = [tp for tp in temporal_patterns if tp.pattern_type == "trend"]
        if trend_patterns:
            recommendations.append(
                "📊 Monitor incident trends - system may be degrading over time"
            )

        # Overall recommendations
        if len(recurring_failures) + len(systemic_issues) >= 5:
            recommendations.append(
                "🎯 Recommend immediate incident response team formation"
            )
            recommendations.append(
                "📋 Create action plan with priority ordering based on business impact"
            )
    ...
    def _calculate_recurrence_confidence(self, incidents: List[Incident]) -> float:
        ...
        distribution_factor = min(
            1.0, time_span.days / 30.0
        )  # More concerning if spread over time

        confidence = (
            count_factor * 0.4 + consistency_factor * 0.3 + distribution_factor * 0.3
        )
    ...
    def _calculate_confidence_score(
        ...
        for si in systemic_issues:
            weight = si.incident_count * (
                2 if si.severity in ["HIGH", "CRITICAL"] else 1
            )
            total_weight += weight
    ...
    def _suggest_file_remediation(
        self, file_path: str, failure_types: List[str]
    ) -> str:
    ...
    def _suggest_service_remediation(
        self, service: str, incidents: List[Incident]
    ) -> str:
