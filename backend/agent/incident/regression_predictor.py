...
class RegressionPredictor:
    ...
    def predict_regression_risk(self, 
        ...
        """
        logger.info(f"Predicting regression risk for {len(changed_files)} changed files")
        ...
        overall_risk = self._calculate_overall_risk(file_risks, changed_files, incidents, author)
        
        # Determine risk level
        risk_level = self._determine_risk_level(overall_risk)
        
        # Generate risk factors and explanations
        risk_factors = self._generate_risk_factors(file_risks, incidents, branch, author)
        ...
        predicted_failures = self._predict_failure_types(file_risks, historical_incidents, incidents)
        
        # Generate recommendations
        recommendations = self._generate_recommendations(risk_level, file_risks, risk_factors)
        
        # Calculate prediction confidence
        confidence = self._calculate_prediction_confidence(file_risks, len(historical_incidents))
    ...
    def _update_prediction_model(self, incidents: List[Incident]) -> None:
        ...
        if (self.last_model_update is None or 
            (datetime.now() - self.last_model_update).days >= 7):
    ...
    def _update_file_risk_cache(self, incidents: List[Incident]) -> None:
        ...
        for file_path, file_incident_list in file_incidents.items():
            if len(file_incident_list) >= self.min_incidents_for_prediction:
                risk = self._calculate_individual_file_risk(file_path, file_incident_list, incidents)
                self.file_risk_cache[file_path] = risk
    
    def _calculate_file_risks(self, changed_files: List[str], incidents: List[Incident]) -> List[FileRisk]:
        ...
        for file_path in changed_files:
            if file_path in self.file_risk_cache:
                ...
                if len(file_incidents) >= 2:  # Lower threshold for changed files
                    risk = self._calculate_individual_file_risk(file_path, file_incidents, incidents)
                    ...
                    risk = FileRisk(
                        ...
                        last_failure=file_incidents[-1].timestamp if file_incidents else None,
                        failure_types=[inc.failure_type for inc in file_incidents],
                        failure_rate=0.0,
                        risk_factors=["New or rarely-changed file"] if not file_incidents else []
                    )
                    file_risks.append(risk)
        
        return sorted(file_risks, key=lambda x: x.risk_score, reverse=True)
    
    def _calculate_individual_file_risk(self, file_path: str, 
                                      file_incidents: List[Incident], 
                                      all_incidents: List[Incident]) -> FileRisk:
        """Calculate risk metrics for an individual file"""
        # Basic metrics
        failure_count = len(file_incidents)
        last_failure = max(inc.timestamp for inc in file_incidents) if file_incidents else None
        failure_types = [inc.failure_type for inc in file_incidents]
        
        # Calculate failure rate (failures per estimated change)
        if file_incidents:
            time_span = max(inc.timestamp for inc in file_incidents) - min(inc.timestamp for inc in file_incidents)
            estimated_changes = max(1, int(time_span.days / 7))  # Assume ~1 change per week
            failure_rate = failure_count / estimated_changes
        else:
            failure_rate = 0.0
        
        # Calculate base risk score
        risk_score = self._calculate_base_risk_score(failure_count, failure_rate, file_incidents)
        
        # Apply risk modifiers
        risk_score = self._apply_risk_modifiers(risk_score, file_path, file_incidents)
        
        # Generate risk factors
        risk_factors = self._generate_file_risk_factors(file_path, file_incidents, risk_score)
    ...
    def _calculate_base_risk_score(self, failure_count: int, failure_rate: float, 
        ...
        # Recency component
        if incidents:
            days_since_last = (datetime.now() - max(inc.timestamp for inc in incidents)).days
    ...
    def _apply_risk_modifiers(self, base_score: float, file_path: str, 
                             incidents: List[Incident]) -> float:
    ...
    def _calculate_overall_risk(self, file_risks: List[FileRisk], 
                               changed_files: List[str],
                               incidents: List[Incident],
    ...
    def _generate_risk_factors(self, file_risks: List[FileRisk], 
                              incidents: List[Incident],
                              branch: str, 
                              author: Optional[str]) -> List[str]:
        ...
        if high_risk_files:
            factors.append(f"{len(high_risk_files)} files with high historical failure rates")
        
        # Recent failures
        recent_failures = [
            fr for fr in file_risks 
            if fr.last_failure and (datetime.now() - fr.last_failure).days <= 7
        ]
        if recent_failures:
            factors.append(f"{len(recent_failures)} files failed recently (last 7 days)")
        
        # Critical file involvement
        critical_files = [fr for fr in file_risks if self._is_critical_file(fr.file_path)]
        if critical_files:
            factors.append(f"{len(critical_files)} critical system files being modified")
        ...
        if author:
            ...
            if len(author_incidents) > 5:
                factors.append(f"Author has {len(author_incidents)} historical incidents")
    ...
    def _find_related_incidents(self, changed_files: List[str], 
    ...
    def _predict_failure_types(self, file_risks: List[FileRisk], 
                              related_incidents: List[Incident],
                              all_incidents: List[Incident]) -> List[str]:
        ...
        for file_risk in file_risks:
            for failure_type in file_risk.failure_types:
                failure_type_counts[failure_type] += int(file_risk.risk_score * 100)  # Convert to int
        ...
        for incident in related_incidents:
            ...
            failure_type_counts[incident.failure_type] += int(recency_weight * 100)  # Convert to int
    ...
    def _generate_recommendations(self, risk_level: str, 
        ...
        if risk_level == "CRITICAL":
            recommendations.append("🚨 CRITICAL RISK: Consider deferring this change or adding extensive testing")
        ...
        high_risk_files = [fr for fr in file_risks if fr.risk_score > 0.5]
        if high_risk_files:
            test_files = [fr.file_path for fr in high_risk_files if self._is_test_file(fr.file_path)]
            if test_files:
                recommendations.append(f"🧪 Pay special attention to test files: {len(test_files)} have high failure rates")
            
            critical_files = [fr.file_path for fr in high_risk_files if self._is_critical_file(fr.file_path)]
            if critical_files:
                recommendations.append("🏗️ Critical files modified - consider feature flags or gradual rollout")
        
        # Recommendations based on risk factors
        if any("recent" in factor.lower() for factor in risk_factors):
            recommendations.append("🕐 Recent failures detected - investigate root cause before proceeding")
        
        if any("author" in factor.lower() for factor in risk_factors):
            recommendations.append("👨‍💻 Consider pair programming or additional code review")
        
        return recommendations[:6]  # Limit to top 6 recommendations
    
    def _calculate_prediction_confidence(self, file_risks: List[FileRisk], 
                                        historical_incident_count: int) -> float:
        ...
        data_confidence = min(1.0, historical_incident_count / 10.0)  # More history = higher confidence
    ...
    def _is_critical_file(self, file_path: str) -> bool:
        ...
        critical_indicators = [
            "/main/", "/core/", "/api/", "/server/", 
            "config", "database", "auth", "security",
    ...
    def _generate_file_risk_factors(self, file_path: str, incidents: List[Any], risk_score: float) -> List[str]:
        ...
        recent_incidents = [inc for inc in incidents if 
                          (datetime.now() - inc.timestamp).days < 7]
        if recent_incidents:
