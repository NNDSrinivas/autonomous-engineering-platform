...
async def enrich_jira_context(
    ...
    try:
        ...
        logger.info(f"Enriching Jira {issue_id} with org context using terms: {search_terms}")
            ...
            for term in search_terms:
                if term in content:
                    related_slack.append({
                        ...
                        "channel": artifact.get("metadata", {}).get("channel", "Unknown"),
                        "author": artifact.get("metadata", {}).get("author", "Unknown"),
                        "timestamp": artifact.get("metadata", {}).get("timestamp", ""),
                        "relevance_score": _calculate_relevance(content, search_terms)
                    })
        ...
        for artifact in confluence_artifacts:
            ...
            for term in search_terms:
                if term in content or term in title_match:
                    related_docs.append({
                        "title": artifact.get("metadata", {}).get("title", "Untitled"),
                        "url": artifact.get("metadata", {}).get("url", ""),
                        "content": artifact.get("content", "")[:500],  # Truncate
                        "space": artifact.get("metadata", {}).get("space", "Unknown"),
                        "relevance_score": _calculate_relevance(content + " " + title_match, search_terms)
                    })
                    break
            ...
            for term in search_terms:
                if term in content or term in title_match:
                    related_meetings.append({
                        "topic": artifact.get("metadata", {}).get("topic", "Untitled Meeting"),
        ...
        for artifact in github_artifacts:
            ...
            for term in search_terms:
                if term in content or term in title_match:
                    related_prs.append({
                        "title": artifact.get("metadata", {}).get("title", "Untitled PR"),
                        "number": artifact.get("metadata", {}).get("number", ""),
                        "url": artifact.get("metadata", {}).get("url", ""),
                        "state": artifact.get("metadata", {}).get("state", "unknown"),
                        "author": artifact.get("metadata", {}).get("author", "Unknown"),
                        "relevance_score": _calculate_relevance(content + " " + title_match, search_terms)
                    })
                    break
...
def format_enriched_context_for_llm(enriched: Dict[str, Any]) -> str:
    ...
    if enriched.get("slack"):
        ...
        for msg in enriched["slack"][:5]:  # Top 5
            sections.append(f"- [{msg['channel']}] {msg['author']}: {msg['content'][:200]}...")
        sections.append("")
    ...
    if enriched.get("prs"):
        ...
        for pr in enriched["prs"]:
            sections.append(f"- PR #{pr['number']}: {pr['title']} ({pr['state']}) by {pr['author']}")
        sections.append("")
