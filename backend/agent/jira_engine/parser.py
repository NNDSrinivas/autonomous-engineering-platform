...
def parse_jira_issue(issue: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if fields.get("assignee"):
            assignee = (
                fields["assignee"].get("emailAddress") or 
                fields["assignee"].get("displayName") or
                fields["assignee"].get("name")
            )
        ...
        if fields.get("reporter"):
            reporter = (
                fields["reporter"].get("emailAddress") or
                fields["reporter"].get("displayName") or
                fields["reporter"].get("name")
            )
        ...
        normalized = {
            ...
            "url": f"https://jira.company.com/browse/{issue.get('key')}" if issue.get("key") else None,
