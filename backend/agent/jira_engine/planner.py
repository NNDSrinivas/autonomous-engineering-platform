    ...
    try:
        ...
        if enriched_context.get("slack"):
            ...
            for msg in enriched_context["slack"][:3]:  # Top 3
                prompt_parts.append(f"- [{msg['channel']}] {msg['author']}: {msg['content'][:200]}...")
        ...
        if enriched_context.get("meetings"):
            ...
            for meeting in enriched_context["meetings"]:
                prompt_parts.append(f"- {meeting['topic']} ({meeting['date']}): {meeting['notes'][:200]}...")
        
        if enriched_context.get("prs"):
            prompt_parts.append(f"\n**Related Pull Requests ({len(enriched_context['prs'])}):**")
            for pr in enriched_context["prs"][:3]:
                prompt_parts.append(f"- PR #{pr['number']}: {pr['title']} ({pr['state']})")
        ...
        if user_preferences:
            ...
            prompt_parts.append(f"- Coding style: {user_preferences.get('coding_style', 'Standard')}")
            prompt_parts.append(f"- Testing: {user_preferences.get('testing', 'Required')}")
            prompt_parts.append(f"- Documentation: {user_preferences.get('documentation', 'Required')}")
            prompt_parts.append("")
        ...
        plan_dict = {
            ...
            "acceptance_criteria": _extract_list_section(plan_text, "ACCEPTANCE CRITERIA"),
