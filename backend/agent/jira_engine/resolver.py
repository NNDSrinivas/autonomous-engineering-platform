...
def resolve_jira_target(
    ...
    try:
        ...
        vague_references = [
            "this", "that", "the issue", "the ticket", "the task", "the story",
            "it", "this one", "current", "active"
        ]
        ...
        if scored_issues and scored_issues[0][1] > 0.3:
            ...
            logger.info(f"Resolved Jira from relevance scoring: {issue_key} (score: {scored_issues[0][1]:.2f})")
            return issue_key
        ...
        user_issues = [
            ...
            if issue.get("fields", {}).get("assignee", {}).get("emailAddress") == user_email
        ]
