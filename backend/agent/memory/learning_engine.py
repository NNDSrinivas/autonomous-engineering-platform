...
async def learn_from_approval(
    user_id: str, approval_data: Dict[str, Any], db_session=None
) -> bool:
    """
    ...
    try:
        ...
        for pattern in patterns:
            ...
            frequency = (
                approval_data.get("_observation_cache", {}).get(cache_key, 0) + 1
            )
            ...
            if frequency >= 3:
                ...
                logger.info(
                    f"Learned coding preference for {user_id}: {pattern['type']}"
                )
...
async def learn_from_rejection(
    user_id: str, rejection_data: Dict[str, Any], db_session=None
) -> bool:
    """
...
async def learn_from_code_change(
    user_id: str, change_data: Dict[str, Any], db_session=None
) -> bool:
    """
...
async def learn_explicit_preference(
    user_id: str, preference_data: Dict[str, Any], db_session=None
) -> bool:
    """
    ...
    try:
        ...
        logger.info(
            f"Learned explicit preference for {user_id}: {preference_key} = {preference_value}"
        )
        return True
...
async def learn_coding_style(user_id: str, db_session=None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        approved_tasks = await retrieve_task_memories(
            user_id=user_id, approved_only=True, limit=50, db_session=db_session
        )
...
async def detect_patterns(user_id: str, context: str, db_session=None) -> List[str]:
    """
    ...
    try:
        ...
        memories = await retrieve_user_memories(
            user_id=user_id, query=context, limit=20, db_session=db_session
        )
...
def _extract_code_patterns(code_diff: str) -> List[Dict[str, str]]:
    ...
    if "->" in code_diff or ": str" in code_diff or ": int" in code_diff:
        patterns.append(
            {"type": "type_hints", "description": "Adds explicit type hints"}
        )

    # Null/None checks
    if "if not" in code_diff or "is None" in code_diff or "is not None" in code_diff:
        patterns.append(
            {"type": "null_checking", "description": "Adds explicit null/None checks"}
        )

    # Try-catch error handling
    if "try:" in code_diff and "except" in code_diff:
        patterns.append(
            {"type": "try_catch", "description": "Uses try-catch for error handling"}
        )

    # Logging
    if "logger." in code_diff:
        patterns.append({"type": "logging", "description": "Adds logging statements"})

    # Docstrings
    if '"""' in code_diff or "'''" in code_diff:
        patterns.append({"type": "docstrings", "description": "Includes docstrings"})
