...
class MemoryManager:
    ...
    async def run_maintenance(self, user_id: Optional[str] = None) -> Dict[str, int]:
        ...
        """
        stats = {"pruned": 0, "merged": 0, "reindexed": 0}
...
async def prune_old_memories(user_id: Optional[str] = None, db_session=None) -> int:
    """
...
async def merge_similar_memories(
    user_id: Optional[str] = None, similarity_threshold: float = 0.95, db_session=None
) -> int:
    """
...
async def archive_old_memories(
    user_id: Optional[str] = None, days_old: int = 180, db_session=None
) -> int:
    """
