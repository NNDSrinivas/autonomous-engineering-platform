...
async def retrieve_memories(
    ...
    try:
        ...
        logger.info(f"[MEMORY] Found {context.get_total_count()} total memories ({len(memories)} traditional + {len(org_snippets)} org snippets)")
        return context
    ...
    for source, key in source_mapping.items():
        ...
        for i, item in enumerate(items):
            snippet = OrgSnippet(
                ...
                source=source,
                title=item.get("title") or item.get("summary") or f"{source.title()} Item",
