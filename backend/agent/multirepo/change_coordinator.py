...
class ChangeCoordinator:
    ...
    def __init__(
        ...
        self.impact_analyzer = impact_analyzer or ImpactAnalyzer(
            repo_registry=self.repo_registry, graph_builder=self.graph_builder
        )
    ...
    async def create_coordinated_change(
        self,
        title: str,
        description: str,
        ...
        coordinated_change = CoordinatedChange(
            ...
            risk_level=(
                impact_analysis.overall_risk if impact_analysis else RiskLevel.MINIMAL
            ),
        )
        ...
        logger.info(
            f"Coordinated change {change_id} created with {len(change_requests)} repositories"
        )
        return change_id

    async def execute_coordinated_change(
        self, change_id: str, dry_run: bool = False
    ) -> bool:
        """
    ...
    async def _calculate_deployment_order(
        ...
        repo_graph = await self.graph_builder.build_dependency_graph(
            repo_filter=repo_names, include_external=False
        )
    ...
    async def _analyze_coordinated_impact(
        self, change_requests: List[ChangeRequest]
    ...
    def _infer_change_type(self, change_request: ChangeRequest):
        ...
        if any("api" in f.lower() or "schema" in f.lower() for f in changed_files):
            ...
            "migration" in f.lower() or "database" in commit_msg for f in changed_files
        ):
            return ChangeType.DATABASE_CHANGE
        elif "config" in commit_msg or any(
            "config" in f.lower() for f in changed_files
        ):
            return ChangeType.CONFIGURATION_CHANGE
        elif "infrastructure" in commit_msg or any(
            "terraform" in f.lower() for f in changed_files
        ):
    ...
    async def _generate_rollback_plan(
        self, change_requests: List[ChangeRequest], dependency_order: List[str]
    ) -> Dict[str, Any]:
    ...
    async def _execute_single_change(
        self, change_request: ChangeRequest, dry_run: bool
    ) -> bool:
        """Execute a change for a single repository"""
        try:
            if dry_run:
                logger.info(
                    f"[DRY RUN] Would execute change for {change_request.repo_name}"
                )
            ...
            logger.error(
                f"Failed to execute change for {change_request.repo_name}: {e}"
            )
            return False
    ...
    async def _rollback_coordinated_change(self, change_id: str) -> bool:
        ...
        rollback_order = change.rollback_plan.get(
            "order", list(reversed(change.dependency_order))
        )
        ...
        try:
            for repo_name in rollback_order:
                if change.progress.get(repo_name) == "completed":
                    ...
                    change.rollback_plan["commands"].get(repo_name, {}).get(
                        "custom_commands", []
                    )
...
async def create_coordinated_change(
    title: str, description: str, change_requests: List[ChangeRequest]
) -> str:
    """Convenience function to create coordinated change"""
    coordinator = ChangeCoordinator()
    return await coordinator.create_coordinated_change(
        title, description, change_requests
    )
