...
class ContractAnalyzer:
    ...
    def __init__(self):
        ...
        self.supported_formats = [
            "openapi", "swagger", "json_schema", "graphql", 
            "protobuf", "avro", "asyncapi"
        ]
        logger.info("ContractAnalyzer initialized for multi-format contract analysis")
    
    def analyze_contract_changes(self, 
                               old_file_path: str,
                               new_file_path: str,
                               contract_type: Optional[str] = None) -> ContractChange:
        """
        ...
        if not old_path.exists() or not new_path.exists():
            logger.warning(f"Contract file not found: {old_file_path} or {new_file_path}")
        ...
        logger.info(f"Analyzing {contract_type} contract changes: {old_file_path} -> {new_file_path}")
    ...
    def discover_contracts(self, repo_path: str) -> List[APIContract]:
        ...
        patterns = {
            "openapi": ["**/*openapi*.{yaml,yml,json}", "**/*swagger*.{yaml,yml,json}", "**/api-docs*.{yaml,yml,json}"],
    ...
    def _analyze_openapi_changes(self, old_path: Path, new_path: Path) -> ContractChange:
    ...
    def _analyze_openapi_path_changes(self, path: str, 
        ...
        for method in common_methods:
            ...
            # Check parameters
            old_params = {p.get('name'): p for p in old_method_spec.get('parameters', [])}
            new_params = {p.get('name'): p for p in new_method_spec.get('parameters', [])}
    ...
    def _analyze_openapi_schema_changes(self, 
                                       old_schemas: Dict[str, Any],
                                       new_schemas: Dict[str, Any]) -> List[BreakingChange]:
    ...
    def _analyze_json_schema_changes(self, old_path: Path, new_path: Path) -> ContractChange:
        ...
        try:
            ...
            overall_impact = ChangeImpact.MAJOR if breaking_changes else ChangeImpact.PATCH
    ...
    def _analyze_json_schema_object_changes(self, 
                                           path: str,
                                           old_schema: Dict[str, Any], 
                                           new_schema: Dict[str, Any]) -> List[BreakingChange]:
    ...
    def _analyze_graphql_changes(self, old_path: Path, new_path: Path) -> ContractChange:
    ...
    def _analyze_protobuf_changes(self, old_path: Path, new_path: Path) -> ContractChange:
    ...
    def _parse_contract(self, file_path: Path, contract_type: str) -> Optional[APIContract]:
    ...
    def _extract_graphql_types(self, schema_text: str) -> Dict[str, str]:
        ...
        type_matches = re.finditer(r'type\s+(\w+)\s*{([^}]*)}', schema_text, re.MULTILINE | re.DOTALL)
...
def analyze_contract_changes(old_file: str, new_file: str, 
                           contract_type: Optional[str] = None) -> ContractChange:
