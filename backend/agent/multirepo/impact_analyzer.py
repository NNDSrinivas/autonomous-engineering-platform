...
class ImpactAnalyzer:
    ...
    def __init__(self, 
        ...
        logger.info("ImpactAnalyzer initialized with cross-repository blast radius capabilities")
    ...
    async def analyze_change_impact(self, 
                                  ...
                                  include_transitive: bool = True) -> ImpactAnalysis:
        """
        ...
        analysis.total_estimated_effort_hours = sum(r.estimated_effort_hours for r in affected_repos)
        analysis.teams_affected = list(set(r.repo_name.split('-')[0] for r in affected_repos))
        ...
        logger.info(f"Impact analysis complete: {analysis.blast_radius} repositories affected")
        return analysis
    
    async def _find_affected_repositories(self,
                                        origin_repo: str,
                                        repo_graph: RepoGraph,
                                        changed_files: Optional[List[str]] = None) -> List[AffectedRepository]:
        ...
        while queue:
            ...
            if distance > 0:
                ...
                if repo_meta:
                    ...
                    affected_repo.affected_components = await self._identify_affected_components(
    ...
    async def _calculate_impact_paths(self,
                                    origin_repo: str,
                                    affected_repos: List[AffectedRepository],
                                    repo_graph: RepoGraph) -> List[ImpactPath]:
    ...
    def _find_shortest_path(self, start: str, end: str, graph: RepoGraph) -> List[str]:
        ...
        while queue:
            ...
            neighbors = (
                graph.get_dependencies(current) + 
                graph.get_dependents(current)
            )
    ...
    def _calculate_path_risk_multiplier(self, path_repos: List[str], graph: RepoGraph) -> float:
    ...
    async def _analyze_contract_impacts(self,
                                      origin_repo: str,
                                      affected_repos: List[AffectedRepository],
                                      changed_files: Optional[List[str]] = None) -> None:
        ...
        contract_files = [
            ...
            if any(pattern in f.lower() for pattern in [
                'openapi', 'swagger', 'schema', '.proto', '.graphql'
            ])
        ]
    ...
    async def _calculate_risk_assessment(self,
                                       origin_repo: str,
                                       affected_repos: List[AffectedRepository],
                                       change_type: ChangeType,
                                       repo_graph: RepoGraph) -> Dict[str, Any]:
    ...
    async def _generate_recommendations(self,
        ...
        if risk_level in [RiskLevel.CRITICAL, RiskLevel.HIGH]:
            ...
            recommendations['approach'] = (
                "Standard deployment with basic monitoring"
            )
    ...
    def _estimate_effort_hours(self, repo_meta: RepoMeta, impact_type: str, distance: int) -> int:
        ...
        complexity_multiplier = {
            ...
            "database": 2.0,
            "infrastructure": 1.2
        }.get(repo_meta.repo_type.value if hasattr(repo_meta.repo_type, 'value') else str(repo_meta.repo_type), 1.0)
    ...
    async def _assess_repo_risk_factors(self,
                                      repo_meta: RepoMeta,
                                      impact_type: str,
...
async def analyze_change_impact(repo_name: str, 
                              change_description: str,
                              change_type: ChangeType) -> ImpactAnalysis:
    """Convenience function for change impact analysis"""
    analyzer = ImpactAnalyzer()
    return await analyzer.analyze_change_impact(repo_name, change_description, change_type)