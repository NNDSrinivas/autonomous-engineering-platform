...
class RepoGraph:
    ...
    def get_dependencies(self, repo_name: str, 
    ...
    def get_dependents(self, repo_name: str, 
                      transitive: bool = False) -> List[str]:
...
class RepoGraphBuilder:
    ...
    def build_repo_graph(self, 
                        repos: Optional[List[RepoMeta]] = None,
                        include_archived: bool = False) -> RepoGraph:
        """
    ...
    def analyze_critical_path(self, graph: RepoGraph, 
                             target_repo: str) -> List[str]:
        """
    ...
    def identify_shared_components(self, graph: RepoGraph) -> List[str]:
        ...
        shared_components.sort(
            key=lambda name: graph.nodes[name].fan_in, 
            reverse=True
        )
            ...
            if node.repo.business_domain:
                for other_name, other_node in service_nodes.items():
                    if (name != other_name and 
                        other_node.repo.business_domain != node.repo.business_domain and
                        other_node.repo.metadata.get('is_api_service', False)):
                        
                        # Check if there's already a dependency
                        existing = any(
                            edge.target == other_name and edge.dependency_type == DependencyType.API_DEPENDENCY
        ...
        for lib_name, lib_node in library_nodes.items():
            ...
            for name, node in graph.nodes.items():
                if (name != lib_name and 
                    node.repo.primary_language == lib_node.repo.primary_language and
                    node.repo.repo_type.value in ['service', 'frontend']):
                    
                    # Check if dependency already exists
                    existing = any(
                        edge.target == lib_name
                        for edge in node.outgoing_edges
                    )
    ...
    def _infer_infrastructure_dependencies(self, graph: RepoGraph) -> None:
        ...
        for infra_name, infra_node in infra_nodes.items():
            ...
            for name, node in graph.nodes.items():
                if (name != infra_name and
                    node.repo.business_domain == infra_node.repo.business_domain and
                    node.repo.repo_type.value == 'service'):
