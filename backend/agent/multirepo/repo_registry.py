...
class RepoRegistry:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """,
                (
                    ...
                    repo.business_domain,
                    repo.criticality,
                    repo.sla_tier,
                    json.dumps(repo.package_managers),
                    json.dumps(repo.frameworks),
                    json.dumps(repo.databases),
                    json.dumps(repo.cloud_services),
                    json.dumps(repo.depends_on),
                    json.dumps(repo.dependents),
                    json.dumps(repo.shared_libraries),
                    repo.deployment_frequency,
                    repo.last_activity.isoformat() if repo.last_activity else None,
                    json.dumps(repo.contributors),
                    json.dumps(repo.main_contributors),
                    repo.lines_of_code,
                    repo.test_coverage,
                    repo.security_score,
                    repo.quality_score,
    ...
    def find_dependencies(
        self, repo_name: str, include_transitive: bool = False
    ) -> List[str]:
        """
    ...
    def update_dependencies(
        self, repo_name: str, dependencies: List[str], dependents: List[str]
    ) -> None:
        """
        ...
        logger.info(
            f"Updated dependencies for {repo_name}: {len(dependencies)} deps, {len(dependents)} dependents"
        )
    ...
    def get_organization_stats(self) -> Dict[str, Any]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # Total counts
            stats["total_repos"] = conn.execute(
                "SELECT COUNT(*) FROM repositories"
            ).fetchone()[0]
            stats["active_repos"] = conn.execute(
                "SELECT COUNT(*) FROM repositories WHERE NOT is_archived"
            ).fetchone()[0]
            ...
            conn.row_factory = sqlite3.Row
            rows = conn.execute(
                "SELECT primary_language FROM repositories WHERE NOT is_archived AND primary_language IS NOT NULL"
            ).fetchall()
            language_counts = {}
            for row in rows:
