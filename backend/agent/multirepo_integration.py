...
class NaviMultiRepoIntegration:
    ...
    def __init__(self, workspace_root: str):
        ...
        logger.info(f"NaviMultiRepoIntegration initialized for workspace: {workspace_root}")
    ...
    async def get_system_context(self, force_refresh: bool = False) -> SystemContext:
        ...
        if (not force_refresh and 
            self._system_context and 
            self._last_context_update and
            (now - self._last_context_update).seconds < self._context_ttl):
        ...
        try:
            ...
            logger.info(f"System context refreshed: {len(org_repo_names)} repos, health: {health_report.overall_health_score:.2f}")
    ...
    async def enhance_intent_with_system_context(self, intent: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if intent.get("kind") == "implement_feature":
                ...
                if any(keyword in feature_description.lower() for keyword in 
                       ["api", "database", "auth", "shared", "common", "library"]):
                    intent["system_context"]["potential_cross_repo_impact"] = True
                    intent["system_context"]["recommendation"] = "Consider cross-repository impact analysis"
            
            elif intent.get("kind") == "fix_bug":
                # For bug fixes, consider system-wide debugging
                bug_description = intent.get("raw_text", "")
                if any(keyword in bug_description.lower() for keyword in 
                       ["production", "outage", "critical", "urgent", "system"]):
                    intent["system_context"]["system_wide_analysis"] = True
                    intent["system_context"]["recommendation"] = "Perform system health analysis"
            
            logger.debug(f"Enhanced intent with system context for {system_context.current_repository}")
    ...
    async def analyze_change_impact(self,
                                  ...
                                  change_type: str = "code_change") -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "affected_repositories": [r.repo_name for r in impact_analysis.affected_repositories],
    ...
    async def plan_coordinated_change(self,
        ...
        try:
            ...
            logger.info(f"Planning coordinated change across {len(affected_repos)} repositories")
            ...
            return {
                ...
                "deployment_order": (change_status or {}).get("deployment_order", affected_repos),
                "requires_approval": (change_status or {}).get("requires_approval", True),
                "estimated_duration": f"{impact_analysis.get('estimated_effort_hours', 4)} hours",
                "rollback_strategy": impact_analysis.get("rollback_strategy", "Standard rollback"),
                "coordination_complexity": "high" if len(affected_repos) > 3 else "medium"
            }
    ...
    async def get_architectural_guidance(self,
        ...
        """
        try:
            logger.info(f"Requesting architectural guidance: {problem_description[:100]}...")
    ...
    async def get_system_health_insights(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "system_status": "healthy" if health_report.overall_health_score > 0.8 else 
                               "concerning" if health_report.overall_health_score > 0.6 else "critical",
                "repository_summary": {
                ...
                "recommendations_count": len(health_report.immediate_actions) + len(health_report.strategic_improvements)
            }
...
def get_multi_repo_integration(workspace_root: str) -> NaviMultiRepoIntegration:
    ...
    if workspace_root not in _integration_instances:
        _integration_instances[workspace_root] = NaviMultiRepoIntegration(workspace_root)
    return _integration_instances[workspace_root]

# Convenience functions for NAVI orchestrator integration
async def enhance_navi_intent_with_system_context(intent: Dict[str, Any], workspace_root: str) -> Dict[str, Any]:
...
async def analyze_navi_change_impact(change_description: str, workspace_root: str, 
                                   changed_files: Optional[List[str]] = None, change_type: str = "code_change") -> Dict[str, Any]:
    """Analyze system-wide impact of NAVI changes"""
    integration = get_multi_repo_integration(workspace_root)
    return await integration.analyze_change_impact(change_description, changed_files or [], change_type)

async def get_navi_architectural_guidance(problem_description: str, workspace_root: str, 
                                        context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Get Principal Engineer-level guidance for NAVI"""
    integration = get_multi_repo_integration(workspace_root)
    return await integration.get_architectural_guidance(problem_description, context or {})
