...
class KnowledgeAggregator:
    ...
    def __init__(self):
        ...
        self.pattern_cache = {}
        logger.info("KnowledgeAggregator initialized - ready to transform signals into knowledge")
    
    def aggregate_patterns(self, signals: List[OrgSignal], 
                          min_pattern_frequency: int = 3,
                          confidence_threshold: float = 0.6) -> List[OrgPattern]:
        """
        ...
        patterns.extend(self._identify_success_practices(signals, min_pattern_frequency))
        
        # Author/team patterns
        patterns.extend(self._identify_author_patterns(signals, min_pattern_frequency))
        
        # Temporal patterns
        patterns.extend(self._identify_temporal_patterns(signals, min_pattern_frequency))
        
        # Cross-repository patterns
        patterns.extend(self._identify_cross_repo_patterns(signals, min_pattern_frequency))
        ...
        logger.info(f"Identified {len(high_confidence_patterns)} high-confidence patterns")
        return high_confidence_patterns
    
    def generate_team_knowledge(self, signals: List[OrgSignal], 
                               team_name: str) -> TeamKnowledge:
        """
        ...
        failure_signals = [s for s in team_signals if s.signal_type in [
        ...
        for signal in failure_signals:
            if signal.cause:
                ...
                key_phrases = [' '.join(words[i:i+2]) for i in range(len(words)-1)]
                mistake_causes.update(key_phrases)
        
        common_mistakes = [phrase for phrase, freq in mistake_causes.most_common(5) if freq >= 2]
        
        # Identify preferred practices (from successful signals)
        success_signals = [s for s in team_signals if s.signal_type in [
        ...
        collaboration_patterns = {}
        all_signals = [s for s in signals if s.author in [ts.author for ts in team_signals]]
        team_repos = set(s.repo for s in team_signals)
        
        for signal in all_signals:
            if signal.repo not in team_repos and signal.team and signal.team != team_name:
                collaboration_patterns[signal.team] = collaboration_patterns.get(signal.team, 0) + 1
    ...
    def generate_org_knowledge(self, signals: List[OrgSignal], 
                              org_name: str) -> OrgKnowledge:
        """
        ...
        # Cross-team insights
        cross_team_insights = self._analyze_cross_team_patterns(org_signals, team_knowledge)
        
        # Critical insights
        critical_insights = self._extract_critical_insights(patterns, team_knowledge)
        
        # Improvement opportunities
        improvement_opportunities = self._identify_improvement_opportunities(patterns, team_knowledge)
        ...
        confidence_score = self._calculate_knowledge_confidence(org_signals, patterns, team_knowledge)
    ...
    def _identify_failure_hotspots(self, signals: List[OrgSignal], 
                                  min_frequency: int) -> List[OrgPattern]:
        ...
        for file_path, signal_ids in file_failures.items():
            if len(signal_ids) >= min_frequency:
                ...
                pattern = OrgPattern(
                    ...
                    first_seen=min(s.timestamp for s in failure_signals if s.id in signal_ids),
                    last_seen=max(s.timestamp for s in failure_signals if s.id in signal_ids),
    ...
    def _identify_success_practices(self, signals: List[OrgSignal], 
                                   min_frequency: int) -> List[OrgPattern]:
        ...
        for tag, signal_ids in tag_success.items():
            if len(signal_ids) >= min_frequency:
                ...
                pattern = OrgPattern(
                    ...
                    first_seen=min(s.timestamp for s in success_signals if s.id in signal_ids),
                    last_seen=max(s.timestamp for s in success_signals if s.id in signal_ids),
                    trend="stable",
                    impact_level="MEDIUM",
                    recommendations=[
    ...
    def _identify_author_patterns(self, signals: List[OrgSignal], 
                                 min_frequency: int) -> List[OrgPattern]:
        ...
        for author, author_sigs in author_signals.items():
            if len(author_sigs) >= min_frequency:
                ...
                failures = len([s for s in author_sigs if s.signal_type in [
                    SignalType.CI_FAILURE, SignalType.PR_REJECTION, SignalType.ROLLBACK
                ]])
                successes = len([s for s in author_sigs if s.signal_type in [
                    SignalType.CI_SUCCESS, SignalType.PR_APPROVAL, SignalType.DEPLOYMENT_SUCCESS
                ]])
    ...
    def _identify_temporal_patterns(self, signals: List[OrgSignal], 
                                   min_frequency: int) -> List[OrgPattern]:
        ...
        for signal in signals:
            if signal.signal_type in [SignalType.CI_FAILURE, SignalType.DEPLOYMENT_FAILURE]:
    ...
    def _identify_cross_repo_patterns(self, signals: List[OrgSignal], 
                                     min_frequency: int) -> List[OrgPattern]:
    ...
    def _analyze_cross_team_patterns(self, signals: List[OrgSignal], 
                                    team_knowledge: Dict[str, TeamKnowledge]) -> Dict[str, Any]:
        ...
        for team1, knowledge1 in team_knowledge.items():
            insights["team_collaboration_matrix"][team1] = knowledge1.collaboration_patterns
    ...
    def _extract_critical_insights(self, patterns: List[OrgPattern], 
                                  team_knowledge: Dict[str, TeamKnowledge]) -> List[str]:
        ...
        if critical_patterns:
            insights.append(f"CRITICAL: {len(critical_patterns)} critical patterns require immediate action")
        
        # Teams with high risk
        risky_teams = [
            team for team, knowledge in team_knowledge.items() 
            if knowledge.quality_score < 0.4 and knowledge.activity_level == "HIGH"
        ]
        if risky_teams:
            insights.append(f"HIGH: Teams {risky_teams} have high activity but low quality")
        
        return insights
    
    def _identify_improvement_opportunities(self, patterns: List[OrgPattern], 
                                          team_knowledge: Dict[str, TeamKnowledge]) -> List[str]:
        """Identify improvement opportunities"""
        opportunities = []
        
        # High-frequency failure patterns
        failure_hotspots = [p for p in patterns if p.pattern_type == "failure_hotspot" and p.frequency >= 5]
        if failure_hotspots:
            opportunities.append(f"Refactor {len(failure_hotspots)} failure hotspots to improve reliability")
        ...
        if low_activity_teams:
            opportunities.append("Increase engagement and knowledge sharing for low-activity teams")
        
        return opportunities
    
    def _extract_success_practices(self, patterns: List[OrgPattern], 
                                  team_knowledge: Dict[str, TeamKnowledge]) -> List[str]:
        ...
        if high_quality_teams:
            practices.append(f"Adopt practices from high-performing teams: {high_quality_teams}")
        
        return practices
    
    def _identify_risk_areas(self, patterns: List[OrgPattern], 
                            team_knowledge: Dict[str, TeamKnowledge]) -> List[str]:
        ...
        if risk_patterns:
            risks.append(f"{len(risk_patterns)} high-impact patterns pose organizational risk")
    ...
    def _calculate_knowledge_confidence(self, signals: List[OrgSignal], 
                                       patterns: List[OrgPattern],
                                       team_knowledge: Dict[str, TeamKnowledge]) -> float:
        ...
        pattern_confidence = sum(pattern_confidences) / len(pattern_confidences) if pattern_confidences else 0.5
        
        # Team knowledge completeness
        team_confidence = len(team_knowledge) / max(1, len(set(s.team for s in signals if s.team)))
