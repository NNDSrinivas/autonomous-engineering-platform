...
class LearningCollector:
    ...
    def collect_from_pr_event(self, pr_data: Dict[str, Any]) -> OrgSignal:
        ...
        logger.info(
            f"Collected PR signal: {signal_type.value} for {repo_name} by {author}"
        )
        return signal
    ...
    def collect_from_ci_event(self, ci_data: Dict[str, Any]) -> OrgSignal:
        ...
        logger.info(
            f"Collected CI signal: {signal_type.value} for {repo_name} - {status}"
        )
        return signal
    ...
    def _assess_pr_impact(self, files: List[str], pr: Dict[str, Any]) -> str:
        ...
        if any(
            any(pattern in file_path.lower() for pattern in customer_patterns)
        ...
        if any(
            any(pattern in file_path.lower() for pattern in shared_patterns)
            for file_path in files
        ):
            return "ORG"
