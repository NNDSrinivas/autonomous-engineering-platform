...
class OrgMemoryStore:
    ...
    def get_org_patterns(self, org: str, days: int = 30) -> Dict[str, Any]:
        ...
        for signal in signals:
            ...
            patterns["signal_breakdown"][signal_type] = (
                patterns["signal_breakdown"].get(signal_type, 0) + 1
            )
        ...
        patterns["failure_hotspots"] = dict(
            sorted(file_failures.items(), key=lambda x: x[1], reverse=True)[:10]
        )
        ...
        patterns["most_problematic_files"] = list(patterns["failure_hotspots"].keys())[
            :5
        ]

        # Most active authors
        patterns["most_active_authors"] = sorted(
            author_signals.items(), key=lambda x: x[1]["total"], reverse=True
        )[:5]
    ...
    def get_team_knowledge(self, team: str, days: int = 90) -> Dict[str, Any]:
        ...
        failure_signals = [
            ...
            for s in team_signals
            if s.signal_type
            in [SignalType.CI_FAILURE, SignalType.PR_REJECTION, SignalType.ROLLBACK]
        ]
        ...
        knowledge["common_issues"] = sorted(
            issue_causes.items(), key=lambda x: x[1], reverse=True
        )[:5]
        ...
        knowledge["expertise_areas"] = sorted(
            team_files.items(), key=lambda x: x[1], reverse=True
        )[:10]
    ...
    def get_cross_team_patterns(self, days: int = 30) -> Dict[str, Any]:
        ...
        patterns["shared_repositories"] = {
            repo: list(teams) for repo, teams in repo_teams.items() if len(teams) > 1
        }
    ...
    def calculate_org_health_score(self, org: str, days: int = 30) -> float:
        ...
        recent_signals = [
            s for s in signals if (datetime.now() - s.timestamp).days <= 7
        ]
        recent_failures = len(
            [
                s
                ...
                if s.signal_type
                in [SignalType.CI_FAILURE, SignalType.ROLLBACK, SignalType.INCIDENT]
            ]
        )
        recent_penalty = min(0.2, recent_failures * 0.02)

        health_score = max(
            0.0, min(1.0, success_rate - critical_penalty - recent_penalty)
        )
    ...
    def get_learning_insights(self, days: int = 90) -> Dict[str, Any]:
        ...
        if len(failure_signals) > len(success_signals):
            insights["key_learnings"].append(
                "Organization has more failures than successes - focus on quality improvements"
            )
        ...
        if high_risk_files:
            insights["policy_suggestions"].append(
                f"Require extra review for high-risk files: {high_risk_files[:3]}"
            )
        ...
        for signal in failure_signals:
            if signal.author:
                author_failures[signal.author] = (
                    author_failures.get(signal.author, 0) + 1
                )
        ...
        if high_failure_authors:
            insights["improvement_opportunities"].append(
                "Provide additional training or support for developers with high failure rates"
            )
