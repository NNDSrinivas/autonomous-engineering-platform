...
async def retrieve_org_context(
    ...
    try:
        logger.info(
            f"[ORG] Retrieving org context for user={user_id}, query='{query[:50]}...'"
        )
        ...
        if include_github and jira_keys:
            ...
            result["github_prs"] = await _search_github_for_jira(jira_keys, user_id, db)
        ...
        logger.info(
            f"[ORG] Retrieved {len(result['jira_issues'])} Jira issues, "
...
async def _search_slack_for_jira(
    jira_keys: List[str], user_id: str, db
) -> List[Dict[str, Any]]:
