...
class ReplanAnalyzer:
    ...
    def should_replan(
        self, plan_graph: PlanGraph, context: ExecutionContext
    ) -> Tuple[bool, Optional[ReplanTrigger], Dict[str, Any]]:
        ...
        for task in blocked_tasks:
            if task.started_at:
                ...
                if (
                    blocked_duration
                    > self.failure_thresholds["max_blocked_duration_hours"]
                ):
        ...
        if self._is_behind_schedule(plan_graph, context):
            return (
                True,
                ReplanTrigger.TIMELINE_PRESSURE,
                {
                    "schedule_delay_percent": self._calculate_schedule_delay(
                        plan_graph, context
                    ),
                    "critical_path_status": self._analyze_critical_path_status(
                        plan_graph
                    ),
                },
            )
    ...
    def _is_behind_schedule(
        ...
        failed_and_blocked = progress["status_counts"].get("FAILED", 0) + progress[
            "status_counts"
        ].get("BLOCKED", 0)

        return (
            completion_rate < 25 and failed_and_blocked > progress["total_tasks"] * 0.2
        )

    def _calculate_schedule_delay(
        self, plan_graph: PlanGraph, context: ExecutionContext
    ) -> float:
...
class AdaptiveReplanner:
    ...
    def __init__(
        ...
        planner: Optional[PlannerV3] = None,
    ):
    ...
    def evaluate_replan_need(
        ...
        should_replan, trigger, details = self.analyzer.should_replan(
            plan_graph, context
        )
        ...
        if trigger is None:
            logger.warning(
                "Replan needed but no trigger provided, using MANUAL_REQUEST"
            )
        ...
        replan_context = ReplanContext(
            ...
            new_information={},
            replan_scope=(
                self._determine_replan_scope(trigger, details, progress)
                if trigger
                else "task"
            ),
        )
    ...
    async def replan(
        ...
        logger.info(
            f"Starting replan for initiative {initiative.id}, trigger: {replan_context.trigger.value}"
        )
        ...
        try:
            if replan_context.replan_scope == "minimal":
                ...
                return await self._partial_replan(
                    initiative, current_plan, context, replan_context
                )
            else:  # full replan
                return await self._full_replan(
                    initiative, current_plan, context, replan_context
                )
    ...
    def _minimal_replan(
        self, current_plan: PlanGraph, replan_context: ReplanContext
    ) -> ReplanResult:
        ...
        for task in failed_tasks:
            ...
            task.execution_log.append(
                f"Reset for retry due to {replan_context.trigger.value}"
            )
        ...
        for task in blocked_tasks:
            if len(task.task.dependencies) > 0:
                ...
                task.status = TaskStatus.PLANNED
                changes.append(
                    f"Removed blocking dependency {removed_dep} from {task.task.title}"
                )
        ...
        return ReplanResult(
            ...
            recommendations=[
                ...
                "Consider partial replan if issues persist",
            ],
    ...
    async def _partial_replan(
        ...
        remaining_goal = self._extract_remaining_goal(
            initiative, current_plan, replan_context
        )
        ...
        impact_analysis = {
            ...
            "risk_level": "medium",
            "estimated_additional_hours": sum(
                task.estimated_hours for task in new_decomposition.tasks
            ),
    ...
    def _determine_replan_scope(
        self, trigger: ReplanTrigger, details: Dict[str, Any], progress: Dict[str, Any]
    ) -> str:
        ...
        if (
            trigger == ReplanTrigger.TIMELINE_PRESSURE
    ...
    def _extract_lessons_learned(
        ...
        if failed_tasks:
            ...
            for task_type, count in failure_patterns.items():
                lessons.append(
                    f"{task_type} tasks are challenging (failed {count} times)"
                )
        ...
        if blocked_tasks:
            lessons.append(
                f"Dependency management needs improvement ({len(blocked_tasks)} tasks blocked)"
            )
        ...
        for task in failed_tasks:
            if task.failure_reason:
                failed_approaches.append(
                    f"Approach '{task.task.title}' failed: {task.failure_reason}"
                )
    ...
    def get_replan_analytics(
        ...
        return {
            ...
            "success_rate": (
                successful_replans / total_replans if total_replans > 0 else 0
            ),
            "common_reasons": sorted(
                reason_counts.items(), key=lambda x: x[1], reverse=True
            ),
            "replans_requiring_approval": sum(
                1 for r in relevant_replans if r["approval_required"]
            ),
        }
