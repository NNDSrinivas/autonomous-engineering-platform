...
from sqlalchemy import Column, String, DateTime, Text, JSON, LargeBinary, Boolean, Integer
...
class CheckpointModel(Base):
    ...
    def to_metadata(self) -> CheckpointMetadata:
        ...
        return CheckpointMetadata(
            ...
            execution_context=execution_context_val if isinstance(execution_context_val, dict) else {},
            progress_summary=progress_summary_val if isinstance(progress_summary_val, dict) else {},
...
class CheckpointEngine:
    ...
    def create_checkpoint(
        ...
        try:
            ...
            checkpoint = CheckpointModel(
                ...
                description=description or f"Checkpoint at {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}",
                tags=tags or [],
                execution_context=asdict(execution_context),
            ...
            logger.info(f"Created checkpoint {checkpoint_id} for initiative {initiative_id}")
    ...
    def restore_checkpoint(
        ...
        checkpoint = self.db.query(CheckpointModel).filter_by(
            checkpoint_id=checkpoint_id
        ).first()
        ...
        try:
            ...
            logger.info(f"Restored checkpoint {checkpoint_id} for initiative {checkpoint.initiative_id}")
    ...
    def get_latest_checkpoint(self, initiative_id: str) -> Optional[CheckpointMetadata]:
        ...
        checkpoint = self.db.query(CheckpointModel).filter_by(
            initiative_id=initiative_id,
            is_valid=True
        ).order_by(CheckpointModel.created_at.desc()).first()
    ...
    def get_checkpoint_analytics(self, initiative_id: str) -> Dict[str, Any]:
        ...
        checkpoints = self.db.query(CheckpointModel).filter_by(
            initiative_id=initiative_id,
            is_valid=True
        ).all()
        ...
        for cp in checkpoints:
            checkpoint_types[cp.checkpoint_type] = checkpoint_types.get(cp.checkpoint_type, 0) + 1
        ...
        return {
            ...
            "avg_restores_per_checkpoint": sum(restore_counts) / len(restore_counts) if restore_counts else 0,
            "timeline_days": timeline_days,
            "first_checkpoint": min(creation_dates).isoformat() if creation_dates else None,
            "latest_checkpoint": max(creation_dates).isoformat() if creation_dates else None,
        }
            ...
            if plan_graph_data is not None:
                try:
                    ...
                    if not isinstance(plan_data, dict):
                        logger.error(f"Checkpoint {checkpoint.checkpoint_id} plan data corrupted")
                        return False
                except Exception as e:
                    logger.error(f"Checkpoint {checkpoint.checkpoint_id} pickle load failed: {e}")
                    return False
    ...
    def _restore_plan_graph(self, plan_data: Dict[str, Any]) -> PlanGraph:
        ...
        from backend.agent.planning.task_decomposer import DecomposedTask, TaskType, TaskPriority
        ...
        for task_id, node_dict in node_data.items():
            if task_id in plan_graph.nodes:
                ...
                if node_dict["completed_at"]:
                    node.completed_at = datetime.fromisoformat(node_dict["completed_at"])
