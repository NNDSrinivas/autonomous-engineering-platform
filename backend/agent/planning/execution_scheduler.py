...
class TaskExecutor:
    ...
    async def can_execute(self, task: DecomposedTask, context: ExecutionContext) -> Tuple[bool, str]:
        """Check if this executor can handle the task"""
        raise NotImplementedError
    
    async def execute(self, task: DecomposedTask, context: ExecutionContext) -> Tuple[bool, Dict[str, Any]]:
...
class ExecutionScheduler:
    ...
    async def execute_plan(
        ...
        try:
            while not self._is_plan_complete(plan_graph):
                ...
                if not ready_tasks:
                    ...
                    if blocked_tasks or failed_tasks:
                        logger.warning(f"Plan execution stalled. Blocked: {len(blocked_tasks)}, Failed: {len(failed_tasks)}")
                        break
                    else:
                ...
                await self._execute_ready_tasks(ready_tasks, plan_graph, context, progress_callback)
            ...
            logger.info(f"Plan execution completed for initiative {context.initiative_id}")
            ...
            if context.auto_approve_low_risk:
                for task in tasks_to_execute:
                    if self._is_low_risk_task(task) and task.approval_status is None:
                        plan_graph.set_task_approval(task.task.id, True, "system_auto_approval")
        
        # Execute tasks in parallel
        if tasks_to_execute:
            await asyncio.gather(*[
                self._execute_single_task(task, plan_graph, context, progress_callback)
                for task in tasks_to_execute
            ])
    ...
    async def _execute_single_task(
        ...
        try:
            ...
            if progress_callback:
                progress_callback("task_started", {"task_id": task_id, "task": task_node.to_dict()})
            
            # Find appropriate executor
            executor = await self._find_executor(task_node.task, context)
            if not executor:
                error_msg = f"No executor found for task type {task_node.task.task_type.value}"
                plan_graph.fail_task(task_id, error_msg)
                logger.error(f"Task {task_id} failed: {error_msg}")
                
                if progress_callback:
                    progress_callback("task_failed", {"task_id": task_id, "reason": error_msg})
            ...
            try:
                ...
                if success:
                    ...
                    if result.get("status") == "pending_approval":
                        ...
                        if progress_callback:
                            progress_callback("task_pending_approval", {
                                "task_id": task_id,
                                "approvers": result.get("approvers", [])
                            })
                    else:
                        plan_graph.fail_task(task_id, result.get("error", "Execution failed"))
            ...
            if progress_callback:
                progress_callback("task_error", {"task_id": task_id, "error": error_msg})
    
    async def _find_executor(self, task: DecomposedTask, context: ExecutionContext) -> Optional[TaskExecutor]:
    ...
    def _is_plan_complete(self, plan_graph: PlanGraph) -> bool:
        ...
        non_terminal_count = sum(
            count for status, count in progress["status_counts"].items()
    ...
    def get_execution_status(self, plan_graph: PlanGraph) -> Dict[str, Any]:
        ...
        return {
            ...
            "blocked_tasks": [node.to_dict() for node in plan_graph.get_blocked_tasks()],
