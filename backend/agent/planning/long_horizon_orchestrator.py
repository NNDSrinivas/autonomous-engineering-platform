...
from backend.agent.planning.initiative_store import InitiativeStore, Initiative, InitiativeStatus
from backend.agent.planning.task_decomposer import TaskDecomposer
from backend.agent.planning.plan_graph import PlanGraph
from backend.agent.planning.execution_scheduler import ExecutionScheduler, ExecutionContext, ExecutionMode
...
class LongHorizonOrchestrator:
    ...
    async def start_initiative(
        ...
        try:
            ...
            # Step 9: Fire events
            await self._fire_event("initiative_started", {
                "initiative_id": initiative_id,
                "goal": goal,
    ...
    async def execute_initiative(
        ...
        try:
            ...
            if execution_result.get("success", False):
                ...
                await self._fire_event("initiative_completed", {
                    "initiative_id": initiative_id,
                ...
                await self._fire_event("initiative_failed", {
                    "initiative_id": initiative_id,
    ...
    async def pause_initiative(self, initiative_id: str, reason: str = "Manual pause") -> bool:
    ...
    async def resume_initiative(
        ...
        if checkpoint_id:
            try:
                plan_graph, execution_context, checkpoint_metadata = self.checkpoint_engine.restore_checkpoint(
                    checkpoint_id
                )
                ...
                logger.info(f"Restored initiative {initiative_id} from checkpoint {checkpoint_id}")
    ...
    async def _execute_with_monitoring(
        ...
        while True:
            ...
            if config.enable_adaptive_replanning:
                needs_replan, replan_context = self.adaptive_replanner.evaluate_replan_need(
                    plan_graph, execution_context
                )
                
                if needs_replan:
                    trigger_info = replan_context.trigger.value if replan_context and hasattr(replan_context, 'trigger') and replan_context.trigger else "unknown"
                    logger.info(f"Replanning triggered for {initiative_id}: {trigger_info}")
                    
                    # Attempt replanning
                    replan_result = await self._handle_replanning(
                        initiative_id, plan_graph, execution_context, replan_context, config
                    )
            ...
            try:
                ...
                if self.checkpoint_engine.auto_checkpoint_needed(
                    plan_graph, last_checkpoint_time, config.auto_checkpoint_interval_minutes
                ):
                    checkpoint_id = self.checkpoint_engine.create_checkpoint(
                        initiative_id=initiative_id,
                        plan_graph=plan_graph,
    ...
    async def _handle_replanning(
        ...
        # Fire replan event
        await self._fire_event("replan_triggered", {
            "initiative_id": initiative_id,
        ...
        if replan_result.approval_required:
            await self._fire_event("approval_needed", {
                "initiative_id": initiative_id,
            ...
            if config.orchestration_mode == OrchestrationMode.DEVELOPMENT:
                logger.info(f"Auto-approving replan for {initiative_id} (development mode)")
                # Replan is approved by default in development
            else:
                logger.info(f"Replan for {initiative_id} requires approval - pausing execution")
                await self.pause_initiative(initiative_id, "Waiting for replan approval")
    ...
    def _create_enhanced_callback(
        ...
        async def enhanced_callback(event_type: str, event_data: Dict[str, Any]):
            ...
            if event_type == "task_completed":
                progress = self.active_orchestrations[initiative_id]["plan_graph"].get_progress_summary()
                if progress["progress_percent"] >= 25 and progress["progress_percent"] % 25 == 0:
                    await self._fire_event("milestone_reached", {
                        "initiative_id": initiative_id,
    ...
    def _get_execution_mode(self, orchestration_mode: OrchestrationMode) -> ExecutionMode:
    ...
    def _is_execution_complete(self, plan_graph: PlanGraph) -> bool:
        ...
        terminal_tasks = sum(
            progress["status_counts"].get(state, 0) 
    ...
    def get_initiative_status(self, initiative_id: str) -> Optional[Dict[str, Any]]:
        ...
        if initiative_id in self.active_orchestrations:
            ...
            status_data.update({
                "progress": plan_graph.get_progress_summary(),
                "execution_status": self.execution_scheduler.get_execution_status(plan_graph),
        ...
        return [
            {
                **initiative.to_dict(),
                ...
                "orchestration_status": self.active_orchestrations.get(initiative.id, {}).get("status"),
            }
