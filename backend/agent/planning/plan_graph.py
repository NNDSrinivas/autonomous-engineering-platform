...
class PlanGraph:
    ...
    def get_execution_plan(self) -> List[List[str]]:
        ...
        while remaining_tasks:
            ...
            if not ready_in_phase:
                ...
                best_task = min(
                    remaining_tasks, key=lambda t: priority_order[priority_map[t]]
                )
    ...
    def get_critical_path(self) -> List[str]:
        ...
        for task_id in topo_order:
            ...
            if not list(self.graph.predecessors(task_id)):
                ...
                for pred_id in self.graph.predecessors(task_id):
                    pred_finish = (
                        earliest_start[pred_id]
                        + self.nodes[pred_id].task.estimated_hours
                    )
        ...
        critical_tasks = [
            task_id
            for task_id in self.nodes
            if abs(earliest_start[task_id] - latest_start[task_id])
            < 0.1  # Float comparison
        ]
    ...
    def get_blocked_tasks(self) -> List[TaskNode]:
        ...
        return [
            node for node in self.nodes.values() if node.status == TaskStatus.BLOCKED
        ]
    ...
    def can_execute_task(self, task_id: str) -> Tuple[bool, str]:
        ...
        for pred_id in self.graph.predecessors(task_id):
            ...
            if pred_node.status != TaskStatus.COMPLETED:
                return (
                    False,
                    f"Dependency {pred_id} not completed (status: {pred_node.status.value})",
                )
