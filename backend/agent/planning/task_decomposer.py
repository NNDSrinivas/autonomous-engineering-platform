...
class TaskDecomposer:
    ...
    async def decompose_goal(
        ...
        tasks = self._extract_tasks_from_plan(initial_plan, org_id, owner, approval_policy)
    ...
    def create_live_plan(self, initiative: Initiative, decomposition: DecompositionResult) -> Dict[str, Any]:
...
"""
    ...
    def _extract_tasks_from_plan(
        ...
        for i, step in enumerate(steps):
            ...
            approvers = self._get_approvers(step, approval_policy) if approval_required else []
    ...
    def _create_execution_phases(self, tasks: List[DecomposedTask]) -> List[List[str]]:
        ...
        while remaining_tasks:
            ...
            if not ready_tasks:
                ...
                task_priorities = {t.id: priority_order[t.priority] for t in tasks if t.id in remaining_tasks}
                ready_tasks = [min(task_priorities.keys(), key=lambda k: task_priorities[k])]
    ...
    def _identify_critical_path(self, tasks: List[DecomposedTask], phases: List[List[str]]) -> List[str]:
    ...
    def _identify_risks(self, tasks: List[DecomposedTask], context: Dict[str, Any]) -> List[str]:
    ...
    def _identify_assumptions(self, tasks: List[DecomposedTask], context: Dict[str, Any]) -> List[str]:
        ...
        if context.get("timeline"):
            assumptions.append(f"Timeline constraint of {context['timeline']} is flexible")
    ...
    def _infer_task_type(self, title: str, description: str) -> TaskType:
        ...
        if any(keyword in content for keyword in ["analyze", "research", "investigate", "study"]):
            return TaskType.ANALYSIS
        elif any(keyword in content for keyword in ["implement", "code", "develop", "build"]):
            return TaskType.DEVELOPMENT
        elif any(keyword in content for keyword in ["test", "verify", "validate", "qa"]):
            return TaskType.TESTING
        elif any(keyword in content for keyword in ["deploy", "release", "rollout", "launch"]):
            return TaskType.DEPLOYMENT
        elif any(keyword in content for keyword in ["document", "spec", "readme", "wiki"]):
            return TaskType.DOCUMENTATION
        elif any(keyword in content for keyword in ["meet", "approve", "coordinate", "sync"]):
            return TaskType.COORDINATION
        else:
            return TaskType.DEVELOPMENT  # Default
    
    def _estimate_effort(self, title: str, description: str, task_type: TaskType) -> float:
        ...
        if any(keyword in content for keyword in ["complex", "advanced", "integration", "migration"]):
            base *= 2
        elif any(keyword in content for keyword in ["simple", "basic", "straightforward", "quick"]):
            base *= 0.5
        
        return max(1, base)  # At least 1 hour
    
    def _infer_priority(self, title: str, description: str, urgency: str) -> TaskPriority:
        ...
        elif any(keyword in content for keyword in ["critical", "blocker", "urgent", "security"]):
            return TaskPriority.CRITICAL
    ...
    def _extract_dependencies(self, step: PlannedStep, existing_tasks: List[DecomposedTask]) -> List[str]:
    ...
    def _needs_approval(
        ...
        if not approval_policy:
            ...
            return task_type in [TaskType.DEPLOYMENT] or "critical" in (step.description or "").lower()
    ...
    def _get_approvers(self, step: PlannedStep, approval_policy: Optional[Dict[str, Any]]) -> List[str]:
    ...
    def _extract_success_criteria(self, step: PlannedStep) -> List[str]:
        ...
        if "implement" in title.lower():
            return ["Code is implemented and passes unit tests", "Code review is completed"]
    ...
    def _matches_approval_rule(
        ...
        if rule.get("keyword") and rule["keyword"].lower() in (step.description or "").lower():
            return True
