...
class BranchManager:
    ...
    def create_pr_branch(
        ...
        *,
        ...
        """
        try:
            logger.info(f"[BRANCH_MANAGER] Creating PR branch for: {feature_description}")
            ...
            if self._branch_exists(branch_name):
                ...
                branch_name = self._generate_alternative_branch_name(feature_description)
                logger.warning(f"[BRANCH_MANAGER] Branch collision, using alternative: {branch_name}")
    ...
    def _ensure_clean_working_tree(self) -> None:
        ...
        if not self._is_working_tree_clean():
            raise BranchCreationError(
                "Working tree is not clean; cannot create branch"
            )
    ...
    def _detect_base_branch(self) -> str:
        ...
        raise BranchCreationError(
            "Unable to determine base branch"
        )
    ...
    def _generate_alternative_branch_name(self, purpose: str) -> str:
        ...
        safe_purpose = self._slugify(purpose)
        timestamp = datetime.utcnow().strftime("%Y%m%d-%H%M%S")
        microseconds = datetime.utcnow().strftime("%f")[:3]  # First 3 digits of microseconds
