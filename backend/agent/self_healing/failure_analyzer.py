...
class FailureAnalyzer:
    ...
    def analyze(self, ci_payload: Dict[str, Any]) -> List[FailureCause]:
        ...
        try:
            ...
            if not causes:
                causes.append(self._create_unknown_failure(
    ...
    def _analyze_timeout_failures(self, logs: str, status: str) -> Optional[FailureCause]:
        """Analyze for timeout/cancellation issues."""
        timeout_in_logs = any(re.search(pattern, logs, re.IGNORECASE) for pattern in self.TIMEOUT_PATTERNS)
    ...
    def _extract_file_info(self, logs: str, position: int) -> Dict[str, Any]:
        ...
        for pattern in file_patterns:
            ...
            if match:
                ...
                result['line'] = int(match.group(2)) if len(match.groups()) >= 2 else None
    ...
    def _extract_log_context(self, logs: str, position: int, context_size: int = 300) -> str:
