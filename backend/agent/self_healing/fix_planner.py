...
class FixPlan:
    ...
    def is_safe_for_auto_fix(self) -> bool:
        ...
        return (
            self.allowed
            and self.strategy == FixStrategy.AUTO_FIX
            and self.confidence >= 0.7
            and self.risk_level in {"low", "medium"}
        )
...
class FixPlanner:
    ...
    AUTO_FIXABLE_CATEGORIES = {FailureCategory.SYNTAX, FailureCategory.LINT}

    # Categories that might be auto-fixable under certain conditions
    CONDITIONAL_CATEGORIES = {FailureCategory.DEPENDENCY}
    ...
    RETRY_CATEGORIES = {FailureCategory.TIMEOUT}
    ...
    def plan(
        self, causes: List[FailureCause], context: Dict[str, Any] = None
    ) -> FixPlan:
        """
        ...
        try:
            ...
            if attempt_count >= max_total_attempts:
                return self._create_blocked_plan(
                    "Maximum fix attempts exceeded", FixStrategy.HUMAN_ONLY
                )
            ...
            logger.info(
                f"Generated fix plan: {plan.strategy.value} for {primary_cause.category.value}"
            )
            return plan
    ...
    def _plan_for_category(
        self, cause: FailureCause, attempt_count: int, repo_context: Dict[str, Any]
    ) -> FixPlan:
    ...
    def _plan_human_only(self, cause: FailureCause) -> FixPlan:
        ...
        return FixPlan(
            ...
            reason=reason_map.get(
                cause.category,
                f"Category {cause.category.value} requires human intervention",
            ),
            fix_goal=goal_map.get(
                cause.category, "Manual investigation and resolution required"
            ),
            confidence=0.0,
            estimated_effort="high",
            risk_level="high",
    ...
    def _is_simple_dependency_issue(
        self, cause: FailureCause, repo_context: Dict[str, Any]
    ) -> bool:
        """
    ...
    def get_planning_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "auto_fix_rate": (
                self.planning_stats["auto_fix_allowed"]
                / max(1, self.planning_stats["total_planned"])
            ),
        }
