...
async def ground_fix_problems(context: Dict[str, Any]) -> GroundingResult:
    ...
    if any(keyword in user_input for keyword in ci_keywords):
        ...
        try:
            ...
            logger.warning(
                f"CI failure grounding failed, falling back to workspace diagnostics: {e}"
            )
    ...
    for diag in diagnostics_raw:
        if isinstance(diag, dict) and "diagnostics" in diag:
            ...
            for entry in group_diagnostics:
                ...
                logger.info(
                    f"Processing diagnostic: file='{file_path}', message='{message}', severity={severity}"
                )
    ...
    grouped_diagnostics = [
        {"path": file_path, "diagnostics": file_diagnostics}
    ...
    task = FixProblemsTask(
        ...
        metadata={
            ...
            "severity_breakdown": {"errors": len(errors), "warnings": len(warnings)},
        },
    )

    logger.info(
        f"Successfully grounded FIX_PROBLEMS: {diagnostics_count} diagnostics in {len(affected_files)} files"
    )
...
def _should_request_clarification(
    diagnostics: List[Diagnostic], context: Dict[str, Any]
) -> bool:
    """
