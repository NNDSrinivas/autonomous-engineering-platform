...
async def generate_github_actions(
    ...
    # Output path
    output_path = os.path.join(workspace_path, ".github", "workflows", f"{workflow_name.lower().replace(' ', '-')}.yml")
...
def _build_lint_job(project_type: str, package_manager: str) -> Dict[str, Any]:
    ...
    if project_type in ["node", "nextjs", "react", "vue"]:
        job["steps"].extend([
            {"name": "Setup Node.js", "uses": "actions/setup-node@v4", "with": {"node-version": "20", "cache": package_manager}},
            {"name": "Install dependencies", "run": f"{package_manager} {'install' if package_manager != 'npm' else 'ci'}"},
...
def _build_test_job(project_type: str, package_manager: str) -> Dict[str, Any]:
    ...
    if project_type in ["node", "nextjs", "react", "vue"]:
        job["steps"].extend([
            {"name": "Setup Node.js", "uses": "actions/setup-node@v4", "with": {"node-version": "20", "cache": package_manager}},
            {"name": "Install dependencies", "run": f"{package_manager} {'install' if package_manager != 'npm' else 'ci'}"},
            {"name": "Run tests", "run": f"{package_manager} run test -- --coverage"},
            {"name": "Upload coverage", "uses": "codecov/codecov-action@v4", "with": {"fail_ci_if_error": False}},
        ])
    elif project_type == "python":
        job["steps"].extend([
            {"name": "Setup Python", "uses": "actions/setup-python@v5", "with": {"python-version": "3.11"}},
            {"name": "Install dependencies", "run": "pip install pytest pytest-cov pytest-asyncio -r requirements.txt"},
...
def _build_build_job(project_type: str, package_manager: str) -> Dict[str, Any]:
    ...
    if project_type in ["node", "nextjs", "react", "vue"]:
        job["steps"].extend([
            ...
            {"name": "Upload artifact", "uses": "actions/upload-artifact@v4", "with": {"name": "build", "path": "dist/ .next/ build/"}},
        ])
    elif project_type == "python":
        job["steps"].extend([
            {"name": "Setup Python", "uses": "actions/setup-python@v5", "with": {"python-version": "3.11"}},
            ...
            {"name": "Upload artifact", "uses": "actions/upload-artifact@v4", "with": {"name": "dist", "path": "dist/"}},
        ])
    elif project_type == "go":
        job["steps"].extend([
            {"name": "Setup Go", "uses": "actions/setup-go@v5", "with": {"go-version": "1.21"}},
            {"name": "Build", "run": "go build -o bin/ ./..."},
            {"name": "Upload artifact", "uses": "actions/upload-artifact@v4", "with": {"name": "bin", "path": "bin/"}},
        ])
...
def _build_deploy_job(project_type: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "steps": [
            ...
            {"name": "Deploy", "id": "deploy", "run": "echo 'Deploying to production...'"},
        ],
    }
                    ...
                    for k, v in item.items():
                        if first:
                            if isinstance(v, dict):
                                ...
                                lines.append(f"{prefix}  - {k}: {_format_yaml_value(v)}")
                            first = False
                        else:
                            if isinstance(v, dict):
                                lines.append(f"{prefix}    {k}:")
