...
async def loadtest_generate_locust(
    ...
    if not endpoints:
        endpoints = [
            ...
            {"method": "GET", "path": "/api/health", "name": "health_check", "weight": 1},
        ]
...
async def loadtest_analyze_results(
    ...
    try:
        ...
        if error_rate > 0.01:
            ...
            recommendations.append("Investigate failing requests - check logs for errors")
        ...
        return {
            ...
            "insights": insights or ["Performance looks good - no major issues detected"],
...
async def loadtest_establish_baseline(
    ...
    smoke_result = await loadtest_generate_k6(
        ...
        test_type="smoke",
        endpoints=endpoints,
        output_path=os.path.join(workspace_path, "baseline_smoke.js") if workspace_path else None,
    )
...
LOAD_TESTING_TOOLS = {
    "loadtest_generate_k6": {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "test_type": {"type": "string", "enum": ["smoke", "load", "stress", "spike", "soak"], "description": "Type of load test"},
                "endpoints": {"type": "array", "description": "Endpoints to test [{method, path, name, body}]"},
    ...
    "loadtest_generate_locust": {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "users": {"type": "integer", "description": "Number of users to simulate"},
                "spawn_rate": {"type": "integer", "description": "Users spawned per second"},
                "output_path": {"type": "string", "description": "Path to save script"},
            },
            "required": ["target_url"],
        },
    ...
    "loadtest_run": {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "script_path": {"type": "string", "description": "Path to load test script"},
                "tool": {"type": "string", "enum": ["k6", "locust"], "description": "Load testing tool"},
    ...
    "loadtest_analyze_results": {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "results_path": {"type": "string", "description": "Path to results file"},
            },
    ...
    "loadtest_establish_baseline": {
        ...
        "parameters": {
            ...
            "properties": {
                "target_url": {"type": "string", "description": "Base URL to test"},
                "endpoints": {"type": "array", "description": "Endpoints to test"},
                "workspace_path": {"type": "string", "description": "Where to save baseline"},
            },
            "required": ["target_url"],
        },
    },
