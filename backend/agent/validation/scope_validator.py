...
class ScopeValidator:
    ...
    def validate(self, changes: Iterable[CodeChange]) -> ValidationResult:
        ...
        for change in changes_list:
            file_path = getattr(change, "file_path", None) or getattr(
                change, "path", None
            )
            if not file_path:
                continue
        ...
        if issues:
            return ValidationResult(status=ValidationStatus.FAILED, issues=issues)

        return ValidationResult(status=ValidationStatus.PASSED, issues=[])
    ...
    def _validate_file_path(self, file_path: str) -> List[ValidationIssue]:
        ...
        try:
            ...
            for forbidden in self._forbidden_paths:
                if (
                    str(relative_path).startswith(forbidden)
                    or forbidden in path_obj.parts
                ):
                    issues.append(
                        ValidationIssue(
                            validator="ScopeValidator",
