...
async def run_workflow_step(
    state: WorkflowState, context: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    try:
        logger.info(
            f"Executing workflow step: {state.current_step} for {state.issue_id}"
        )
        ...
        if step == "analysis":
            result = await step_analysis(
                ...
                enriched_context=(
                    state.enriched_context if state.enriched_context is not None else {}
                ),
            ...
            result = await step_propose_diffs(state=state, user_id=context["user_id"])

        elif step == "apply_diffs":
            result = await step_apply_diffs(state=state, user_id=context["user_id"])

        elif step == "run_tests":
            result = await step_run_tests(
                state=state, user_id=context["user_id"], cwd=context["workspace_root"]
            )
            ...
            result = await step_create_pr(state=state, user_id=context["user_id"])

        elif step == "update_jira":
            result = await step_update_jira(state=state, user_id=context["user_id"])
        ...
        if "approve" in result.get("actions", []) or "approve all" in result.get(
            "actions", []
        ):
            state.set_waiting_approval({"step": step, "result": result})

        return result

    except Exception as e:
        logger.error(
            f"Error executing workflow step {state.current_step}: {e}", exc_info=True
        )
...
async def create_workflow(
    issue_id: str, user_id: str, issue: Dict[str, Any], enriched_context: Dict[str, Any]
) -> WorkflowState:
    """
...
async def cancel_workflow(
    issue_id: str, reason: Optional[str] = None
) -> Dict[str, Any]:
    """
