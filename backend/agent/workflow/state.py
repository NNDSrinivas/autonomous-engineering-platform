...
class WorkflowState:
    ...
    def next_step(self) -> str:
        ...
        try:
            ...
            if current_idx < len(self.STEP_ORDER) - 1:
                ...
                self.updated_at = datetime.utcnow()
                logger.info(f"Workflow {self.issue_id} advanced to step: {self.current_step}")
            else:
    ...
    def set_waiting_approval(self, approval_data: Dict[str, Any]):
        ...
        self.updated_at = datetime.utcnow()
        logger.info(f"Workflow {self.issue_id} waiting for approval at step {self.current_step}")
