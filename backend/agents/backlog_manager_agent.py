...
class BacklogManagerAgent:
    ...
    async def analyze_backlog(self, backlog_items: List[BacklogItem]) -> BacklogAnalysis:
        """
    ...
    async def rank_backlog(
        ...
        try:
            ...
            for score_data in ranking_data:
                ...
                item = next((item for item in backlog_items if item.id == score_data["item_id"]), None)
                if item:
                    ...
                    item.priority = priority_map.get(score_data["recommended_priority"], 3)
    ...
    async def create_backlog_item(
        ...
        # Generate unique ID
        item_id = hashlib.md5(f"{title}_{datetime.now().isoformat()}".encode()).hexdigest()[:12]
        ...
        if auto_prioritize:
            ...
            try:
                response = await self.llm.run(prompt=estimation_prompt, use_smart_auto=True)
        ...
        if merge_strategy == "smart":
            ...
            try:
                ...
                merged_item = BacklogItem(
                    ...
                    story_points=merged_data.get("story_points", max(item.story_points or 0 for item in items)),
                    priority=min(item.priority for item in items),  # Highest priority (lowest number)
                    tags=list(set(sum([item.tags for item in items], []))),  # Merge all tags
                    dependencies=list(set(sum([item.dependencies for item in items], []))),
                ...
                merged_item.description += "\n\n" + "\n".join([f"Merged from: {item.title}" for item in items[1:]])
    ...
    async def cleanup_stale_items(self, max_age_days: Optional[int] = None) -> Dict[str, Any]:
        """
    ...
    def _calculate_priority_distribution(self, items: List[BacklogItem]) -> Dict[str, int]:
    ...
    def _calculate_technical_debt_percentage(self, items: List[BacklogItem]) -> float:
        ...
        tech_debt_items = sum(1 for item in items 
                             if "tech" in item.title.lower() or "debt" in item.title.lower() 
                             or "refactor" in item.title.lower())
        return (tech_debt_items / len(items)) * 100
    
    async def _find_duplicate_items(self, items: List[BacklogItem]) -> List[Tuple[str, str]]:
        ...
        for i, item1 in enumerate(items):
            for item2 in items[i+1:]:
                ...
                similarity = self._calculate_text_similarity(item1.title + " " + item1.description,
    ...
    async def _generate_backlog_recommendations(
        ...
        if tech_debt_pct > self.technical_debt_threshold * 100:
            recommendations.append(f"High technical debt detected ({tech_debt_pct:.1f}%). Consider dedicating 20% of sprint capacity to tech debt.")
        
        if duplicate_count > 0:
            recommendations.append(f"Found {duplicate_count} potential duplicate items. Review and merge to reduce backlog noise.")
        
        if stale_count > 0:
            recommendations.append(f"Found {stale_count} stale items. Run cleanup to maintain backlog hygiene.")
        
        if len(items) > self.max_backlog_size:
            recommendations.append(f"Backlog size ({len(items)}) exceeds recommended maximum ({self.max_backlog_size}). Consider archiving low-priority items.")
        
        return recommendations
    
    def _fallback_ranking(self, items: List[BacklogItem]) -> List[Tuple[BacklogItem, PriorityScore]]:
    ...
    async def _save_backlog_item(self, item: BacklogItem) -> None:
        ...
        try:
            ...
            await self.db.execute(query, [
                item.id, item.title, item.description, item.story_points, item.priority,
                json.dumps(item.tags), json.dumps(item.dependencies), item.assigned_to,
                item.estimated_hours, item.complexity, item.business_value, item.technical_risk,
                (item.created_at.isoformat() if item.created_at else 'unknown'), datetime.now().isoformat()
            ])
    ...
    async def _find_stale_items_by_date(self, cutoff_date: datetime) -> List[Dict[str, Any]]:
