...
class ExecutionAgent:
    ...
    def __init__(self):
        ...
        self.allowed_commands = {
            'npm', 'yarn', 'pip', 'python', 'node', 'git', 'make',
            'mvn', 'gradle', 'cargo', 'go', 'dotnet', 'pytest',
    ...
    async def execute_step(
        ...
        try:
            ...
            backup_info = None
            if self.backup_before_changes and step.action_type in ['modify_file', 'refactor', 'apply_patch']:
                backup_info = await self._create_backup(step.file_targets, workspace_root)
            ...
            if result.success:
                validation_result = await self._validate_execution(step, workspace_root, result)
        ...
        try:
            ...
            if refactor_type == 'extract_method':
                ...
                result = await self.ast_service.move_class(
                    source_file=os.path.join(workspace_root, step.metadata.get('source_file')),
                    target_file=os.path.join(workspace_root, step.metadata.get('target_file')),
    ...
    async def _handle_review(
        ...
        try:
            ...
            if review_type == 'quality':
                results = await self._review_code_quality(step.file_targets, workspace_root)
            
            elif review_type == 'security':
                results = await self._review_security(step.file_targets, workspace_root)
            
            elif review_type == 'performance':
                results = await self._review_performance(step.file_targets, workspace_root)
    ...
    async def _replace_file_section(self, file_path: str, metadata: Dict[str, Any]) -> bool:
        """
    ...
    async def _create_backup(self, file_targets: List[str], workspace_root: str) -> Dict[str, Any]:
        """
    ...
    async def _search_text_in_files(self, query: str, file_paths: List[str], workspace_root: str) -> List[Dict[str, Any]]:
        """Search for text in files"""
        return []
    
    async def _search_functions(self, function_name: str, file_paths: List[str], workspace_root: str) -> List[Dict[str, Any]]:
        """Search for function definitions"""
        return []
    
    async def _search_classes(self, class_name: str, file_paths: List[str], workspace_root: str) -> List[Dict[str, Any]]:
        """Search for class definitions"""
        return []
    
    # Review helper methods
    async def _review_code_quality(self, file_paths: List[str], workspace_root: str) -> Dict[str, Any]:
        """Review code quality"""
        return {}
    
    async def _review_security(self, file_paths: List[str], workspace_root: str) -> Dict[str, Any]:
        """Review security issues"""
        return {}
    
    async def _review_performance(self, file_paths: List[str], workspace_root: str) -> Dict[str, Any]:
        """Review performance issues"""
        return {}
    
    async def _review_style(self, file_paths: List[str], workspace_root: str) -> Dict[str, Any]:
        """Review code style"""
        return {}