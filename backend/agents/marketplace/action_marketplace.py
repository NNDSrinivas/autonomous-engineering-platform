...
class ActionSandbox:
    ...
    def __init__(
        self, workspace_root: str, sandbox_config: Optional[Dict[str, Any]] = None
    ):
        ...
        # Sandbox limits
        self.limits = {
            "max_execution_time": self.config.get(
        ...
        try:
            ...
            if ActionPermission.FILESYSTEM_WRITE in action_metadata.permissions:
                ...
                if (
                    ...
                    < self.limits["max_disk_mb"] * 1024 * 1024
                ):
                    shutil.copytree(
                        self.workspace_root, workspace_link, dirs_exist_ok=True
                    )
                else:
                    workspace_link.symlink_to(self.workspace_root)
    ...
    async def _execute_with_monitoring(
        ...
        try:
            ...
            wrapped_code = self._wrap_action_code(
                action_code, action_metadata, parameters, context
            )
            ...
            try:
                ...
                if process.returncode == 0:
                    ...
                    result["errors"].append(
                        f"Action failed with return code {process.returncode}"
                    )
                    result["errors"].append(stderr.decode("utf-8"))

                result["logs"].append(
                    f"Action completed with return code {process.returncode}"
                )

            except asyncio.TimeoutError:
                process.kill()
                result["errors"].append(
                    f"Action timed out after {self.limits['max_execution_time']} seconds"
                )
...
"""
    ...
    async def _install_dependencies(
        self, execution_dir: Path, dependencies: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for dependency in dependencies[:10]:  # Limit to 10 deps
                ...
                if install_process.returncode != 0:
                    result["errors"].append(
                        f"Failed to install dependency: {dependency}"
                    )
                    return result
...
class ActionRegistry:
    ...
    def search_actions(
        ...
        for action_id, metadata_dict in self.registry_data.get("actions", {}).items():
            try:
                ...
                # Text search
                searchable_text = (
                    f"{metadata.name} {metadata.description} {metadata.author}".lower()
                )
    ...
    async def _download_action(
        self, action_id: str, metadata: ActionMetadata
    ) -> Dict[str, Any]:
        """
        ...
        Returns:
            Download result
        """
        result = {"success": False, "installation_path": None, "error": None}
    ...
    def get_installed_actions(self) -> List[ActionMetadata]:
        ...
        for action_id, install_info in self.installed_actions.items():
            try:
                ...
                self.logger.warning(
                    f"Invalid installed action metadata for {action_id}: {e}"
                )
    ...
    def uninstall_action(self, action_id: str) -> Dict[str, Any]:
        ...
        """
        result = {"success": False, "action_id": action_id, "error": None}
...
class ActionMarketplace:
    ...
    def __init__(self, workspace_root: str, registry_path: Optional[str] = None):
        ...
        self.registry_path = registry_path or str(
            self.workspace_root / ".navi_marketplace"
        )
    ...
    def list_categories(self) -> List[Dict[str, Any]]:
        ...
        for category in ActionCategory:
            ...
            count = len(
                [
                    action
                    for action in self.registry.registry_data.get(
                        "actions", {}
                    ).values()
    ...
    async def recommend_actions(
        ...
        if workspace_analysis.get("deployment_ready"):
            deployers = self.registry.search_actions(
