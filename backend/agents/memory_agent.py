...
class MemoryAgent:
    ...
    async def search_memory(
        ...
        events = await self._search_events_by_keywords(user_id, query, event_types or [], limit)
    ...
    async def cleanup_old_memories(self, user_id: Optional[str] = None):
        ...
        for event_type, retention_period in self.retention_policies.items():
            ...
            await self._cleanup_events_before_date(user_id or "system", event_type, event_cutoff)
    ...
    async def _analyze_execution_patterns(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_plan_size': sum(len(e.get('plan', {}).get('steps', [])) for e in executions) / len(executions)
        }
    ...
    async def _calculate_importance(self, event_type: str, content: Dict[str, Any]) -> float:
        """
    ...
    async def _generate_repo_suggestions(self, repo_context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
    ...
    async def _process_feedback(self, user_id: str, execution_id: str, feedback: Dict[str, Any]):
        """
    ...
    async def _cleanup_events_before_date(self, user_id: str, event_type: str, cutoff_date: datetime):
        """
