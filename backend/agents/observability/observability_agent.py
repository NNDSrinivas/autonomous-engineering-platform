    ...
    def _initialize_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "database_connection_error": {
                "pattern": re.compile(r"database.*connection.*(?:failed|error|timeout)", re.IGNORECASE),
                "severity": AlertSeverity.HIGH,
                "category": "database",
                "description": "Database connection issue detected"
            },
            "memory_leak": {
                "pattern": re.compile(r"(?:memory|heap).*(?:leak|exhausted|out of memory)", re.IGNORECASE),
            ...
            "authentication_failure": {
                "pattern": re.compile(r"auth(?:entication|orization).*(?:failed|denied|invalid)", re.IGNORECASE),
            ...
            "sql_injection_attempt": {
                "pattern": re.compile(r"(?:union|select|insert|update|delete).*(?:from|where).*['\"];", re.IGNORECASE),
                "severity": AlertSeverity.CRITICAL,
                "category": "security",
                "description": "Potential SQL injection attempt"
            },
            "build_failure": {
                "pattern": re.compile(r"build.*(?:failed|error)|compilation.*error|npm.*err", re.IGNORECASE),
                "severity": AlertSeverity.HIGH,
                "category": "build",
                "description": "Build or compilation failure"
            },
            "test_failure": {
                "pattern": re.compile(r"test.*(?:failed|error)|assertion.*(?:failed|error)", re.IGNORECASE),
                "severity": AlertSeverity.MEDIUM,
                "category": "testing",
                "description": "Test execution failure"
            },
            "network_timeout": {
                "pattern": re.compile(r"network.*timeout|connection.*timeout|request.*timeout", re.IGNORECASE),
                "severity": AlertSeverity.MEDIUM,
                "category": "network",
                "description": "Network or connection timeout"
            },
            "disk_space_low": {
                "pattern": re.compile(r"disk.*(?:full|space|low)|no.*space.*left", re.IGNORECASE),
                "severity": AlertSeverity.HIGH,
                "category": "resources",
                "description": "Low disk space detected"
            },
            "javascript_error": {
                "pattern": re.compile(r"(?:uncaught|unhandled).*(?:error|exception)|TypeError|ReferenceError", re.IGNORECASE),
                "severity": AlertSeverity.MEDIUM,
                "category": "frontend",
                "description": "JavaScript runtime error"
            },
            "python_exception": {
                "pattern": re.compile(r"Traceback.*|(?:Error|Exception):.*|File.*line.*", re.IGNORECASE),
                "severity": AlertSeverity.MEDIUM,
                "category": "backend",
...
class AnomalyDetector:
    ...
    def detect_anomalies(self, metric_name: str, threshold_std: float = 2.0) -> List[Dict[str, Any]]:
        ...
        if metric_name not in self.metric_windows or len(self.metric_windows[metric_name]) < 10:
            return []
        ...
        anomalies = []
        for i, (timestamp, value) in enumerate(zip(timestamps[-10:], values[-10:])):  # Check last 10 values
            ...
            if z_score > threshold_std:
                anomalies.append({
                    "timestamp": timestamp,
                    "value": value,
                    "expected_range": (mean - threshold_std * stdev, mean + threshold_std * stdev),
                    "z_score": z_score,
...
class ObservabilityAgent:
    ...
    async def analyze_log_line(self, log_line: str, source: str = "unknown", 
                              context: Optional[Dict[str, Any]] = None) -> Optional[MonitoringAlert]:
        """
        ...
        try:
            ...
            if pattern_matches:
                ...
                anomaly = Anomaly(
                    ...
                    raw_data={"log_entry": asdict(log_entry), "patterns": pattern_matches},
    ...
    async def analyze_performance_metric(self, metric_name: str, value: float, 
                                       unit: str = "", source: str = "system",
                                       tags: Optional[Dict[str, str]] = None) -> Optional[MonitoringAlert]:
        ...
        try:
            ...
            anomalies = self.anomaly_detector.detect_anomalies(
                metric_name, 
                self.config["alert_threshold_std"]
            )
    ...
    async def analyze_build_output(self, build_output: str, build_type: str = "general") -> List[MonitoringAlert]:
        ...
        for i, line in enumerate(lines):
            alert = await self.analyze_log_line(line, f"build_{build_type}", {
    ...
    async def analyze_test_output(self, test_output: str, test_framework: str = "general") -> List[MonitoringAlert]:
        ...
        for i, line in enumerate(lines):
            alert = await self.analyze_log_line(line, f"test_{test_framework}", {
    ...
    async def get_system_health_summary(self) -> Dict[str, Any]:
        ...
        alert_summary = {
            "critical_last_hour": len([a for a in recent_alerts if a.severity == AlertSeverity.CRITICAL]),
            "high_last_hour": len([a for a in recent_alerts if a.severity == AlertSeverity.HIGH]),
            "medium_last_hour": len([a for a in recent_alerts if a.severity == AlertSeverity.MEDIUM]),
            "low_last_hour": len([a for a in recent_alerts if a.severity == AlertSeverity.LOW]),
        ...
        for metric_name in set(m.metric_name for m in list(self.metrics_buffer)[-100:]):
            recent_values = [m.value for m in list(self.metrics_buffer) if m.metric_name == metric_name and m.timestamp > last_hour]
            if recent_values:
                performance_summary[metric_name] = {
                    "current": recent_values[-1] if recent_values else None,
                    "average_hour": statistics.mean(recent_values) if recent_values else None,
                    "min_hour": min(recent_values) if recent_values else None,
                    "max_hour": max(recent_values) if recent_values else None
                }
        
        # Error rate analysis
        recent_logs = [log for log in list(self.log_buffer) if log.timestamp > last_hour]
        error_rate = len([log for log in recent_logs if log.level in [LogLevel.ERROR, LogLevel.CRITICAL]]) / max(len(recent_logs), 1)
        ...
        return {
            ...
            "system_health_score": self._calculate_health_score(alert_summary, error_rate),
            "recommendations": await self._generate_health_recommendations(alert_summary, error_rate)
        }
    
    async def register_alert_callback(self, callback: Callable[[MonitoringAlert], None]):
    ...
    def _parse_log_entry(self, log_line: str, source: str, context: Dict[str, Any]) -> LogEntry:
    ...
    async def _attempt_auto_remediation(self, alert: MonitoringAlert, log_entry: LogEntry):
    ...
    def _calculate_health_score(self, alert_summary: Dict[str, Any], error_rate: float) -> float:
    ...
    async def _generate_health_recommendations(self, alert_summary: Dict[str, Any], 
        ...
        if alert_summary["total_daily"] > 50:
            recommendations.append("High alert volume - consider tuning alert thresholds")
...
class ObservabilityService:
    ...
    async def analyze_log_stream(self, log_lines: List[str], source: str = "application") -> List[MonitoringAlert]:
