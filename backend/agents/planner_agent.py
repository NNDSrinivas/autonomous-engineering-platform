...
class PlannerAgent:
    ...
    async def generate_plan(
        ...
        complexity_score = await self._analyze_instruction_complexity(instruction, repo_map)
        
        # Generate the plan using LLM with structured prompting
        plan_steps = await self._generate_plan_steps(instruction, repo_map, user_context)
    ...
    def _estimate_complexity_fallback(self, instruction: str, repo_map: Dict[str, Any]) -> float:
        """
        ...
        high_complexity_keywords = [
            'refactor', 'architecture', 'redesign', 'migrate', 'convert',
            'performance', 'optimization', 'security', 'testing', 'ci/cd'
        ]
        
        # Medium complexity indicators
        medium_complexity_keywords = [
            'feature', 'component', 'service', 'api', 'database',
            'integration', 'update', 'enhance', 'improve'
        ]
    ...
    async def _enrich_file_targets(
        ...
        for step in steps:
            if not step.file_targets and step.action_type in ['modify_file', 'refactor', 'search_code']:
                ...
                if 'test' in step.description.lower():
                    ...
                    # Add config files
                    config_files = [f for f in hotspots if any(ext in f for ext in ['.json', '.yaml', '.yml', '.toml', '.ini'])]
    ...
    async def replan(
        ...
        try:
            ...
            remaining_steps = [
                ...
                if not any(r.get('step_id') == step.id and r.get('success') for r in execution_results)
            ]
