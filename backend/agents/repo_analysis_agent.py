...
class RepoAnalysisAgent:
    ...
    async def _analyze_dependencies(self, file_info: List[Dict[str, Any]], root_path: str) -> Dict[str, List[str]]:
        """
    ...
    async def _extract_file_dependencies(self, file_info: Dict[str, Any], root_path: str) -> List[str]:
        """
    ...
    async def _analyze_javascript_file(self, file_path: str, root_path: str) -> List[str]:
        """
    ...
    async def _analyze_typescript_file(self, file_path: str, root_path: str) -> List[str]:
        """
    ...
    async def _analyze_complexity(self, file_info: List[Dict[str, Any]]) -> Dict[str, float]:
        """
        ...
        try:
            ...
            for line in non_empty_lines:
                for structure in control_structures:
                    if f' {structure} ' in line or line.strip().startswith(f'{structure} '):
                        control_count += 1
    ...
    async def _detect_hotspots(
        ...
        return list(dict.fromkeys(hotspots))  # Preserves order while removing duplicates
    ...
    def _detect_circular_dependencies(self, dependencies: Dict[str, List[str]]) -> List[List[str]]:
        ...
        def has_path(graph: Dict[str, List[str]], start: str, end: str, visited: Set[str]) -> bool:
            if start == end:
                return True
    ...
    async def _analyze_quality(self, file_info: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if source_files:
            quality_metrics['test_coverage_estimate'] = len(test_files) / len(source_files)
        ...
        quality_metrics['has_linting'] = any(
            name in file_names for name in [
                ...
                'pylint.cfg', '.pylintrc', 'flake8.cfg',
                'tslint.json'
            ]
        )
        ...
        quality_metrics['has_ci_cd'] = any(
            '.github' in f['relative_path'] or 
            '.gitlab-ci' in f['name'] or
            'jenkins' in f['name'].lower() or
            'azure-pipelines' in f['name']
            for f in file_info
        )
        
        return quality_metrics
    
    async def _analyze_security(self, file_info: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    async def _analyze_performance(self, file_info: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        performance_patterns = [
            'nested loops', 'recursive calls', 'database queries in loops',
            'large object creation', 'memory leaks'
        ]
