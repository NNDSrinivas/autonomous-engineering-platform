...
class PatchValidator:
    ...
    def validate_patch(self, patch_content: str, target_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not target_path.exists():
                validation_result['validation_errors'].append(f"Target file does not exist: {target_file}")
            ...
            if not syntax_valid:
                validation_result['validation_errors'].append("Patched code has syntax errors")
                return validation_result
            ...
            validation_result['is_safe'] = (
                ...
                validation_result['semantic_valid'] and
    ...
    def _apply_patch_to_string(self, target_path: Path, patch_content: str) -> Optional[str]:
        """
    ...
    def _validate_semantics(self, original_path: Path, patched_content: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            removed_functions = self._find_removed_functions(original_content, patched_content)
            ...
            line_change_ratio = abs(len(patched_lines) - len(original_lines)) / max(len(original_lines), 1)
            if line_change_ratio > 0.3:
                result['warnings'].append(f"Large structural change: {line_change_ratio:.2%} line difference")
    ...
    def _validate_security_improvement(self, original: str, patched: str) -> bool:
        ...
        security_improvements = [
            ...
            (r'password\s*=\s*["\'][^"\']+["\']', r'password\s*=.*input|environ|config')
        ]
        
        improvements_found = 0
        
        for vulnerable_pattern, secure_pattern in security_improvements:
            original_vulnerable = len(re.findall(vulnerable_pattern, original, re.IGNORECASE))
            patched_vulnerable = len(re.findall(vulnerable_pattern, patched, re.IGNORECASE))
...
class PatchApplicator:
    ...
    def apply_patch(self, 
        ...
        try:
            ...
            if not target_path.exists():
                application_result['error'] = f"Target file does not exist: {target_file}"
                return application_result
            ...
            patched_content = validator._apply_patch_to_string(target_path, patch_content)
    ...
    def rollback_patch(self, rollback_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not backup_files or not metadata_files:
                rollback_result['error'] = f"Backup files not found for rollback_id: {rollback_id}"
