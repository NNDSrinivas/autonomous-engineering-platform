...
class SecurityAgent:
    ...
    async def comprehensive_scan(self, 
        ...
        try:
            ...
            # Generate summary
            scan_results['summary'] = self._generate_scan_summary(scan_results['findings'])
            
            # Generate patches if requested
            if generate_patches and scan_results['findings']:
                patches = await self._generate_security_patches(scan_results['findings'])
            ...
            scan_results['scan_duration_seconds'] = (datetime.utcnow() - scan_start).total_seconds()
            
            # Record in memory
            await self._record_scan_in_memory(scan_results)
            
            self.logger.info(f"Security scan {scan_id} completed. Found {len(scan_results['findings'])} issues.")
            return scan_results
                ...
                try:
                    ...
                    for result_item in bandit_data.get('results', []):
                        finding = SecurityFinding(
                            ...
                            file_path=str(Path(result_item.get('filename', '')).relative_to(self.workspace_root)),
                ...
                try:
                    ...
                    for result_item in semgrep_data.get('results', []):
                        ...
                        finding = SecurityFinding(
                            ...
                            file_path=str(Path(result_item.get('path', '')).relative_to(self.workspace_root)),
            ...
            try:
                ...
                for package_name, vuln_data in vulnerabilities.items():
                    ...
                    finding = SecurityFinding(
                        ...
                        description=vuln_data.get('title', f'Vulnerability in {package_name}'),
                ...
                try:
                    ...
                    for vuln in audit_data:
                        ...
                        finding = SecurityFinding(
                            ...
                            description=vuln.get('description', f'Vulnerability in {package_name}'),
    ...
    async def _generate_security_patches(self, findings: List[SecurityFinding]) -> List[Dict[str, Any]]:
        """
        ...
        for finding in patchable_findings:
            try:
                ...
                self.logger.warning(f"Failed to generate patch for {finding.finding_id}: {e}")
        
        return patches
    
    async def _generate_single_security_patch(self, finding: SecurityFinding) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.warning(f"Patch generation failed for {finding.finding_id}: {e}")
            return None
    ...
    def _generate_scan_summary(self, findings: List[SecurityFinding]) -> Dict[str, Any]:
        ...
        summary['by_cwe'] = dict(sorted(cwe_counts.items(), key=lambda x: x[1], reverse=True)[:10])
        
        # Calculate risk score (0-100)
        risk_score = 0.0
        for finding in findings:
            severity_weight = self.severity_scores.get(finding.severity, 1.0)
            confidence_weight = {'high': 1.0, 'medium': 0.7, 'low': 0.4}.get(finding.confidence, 0.5)
            risk_score += severity_weight * confidence_weight
        
        summary['risk_score'] = min(100.0, risk_score)
        summary['critical_issues'] = summary['by_severity'].get('critical', 0) + summary['by_severity'].get('high', 0)
        summary['patchable_issues'] = len([f for f in findings if f.confidence in ['high', 'medium']])
    ...
    def _get_scannable_files(self) -> List[Path]:
        ...
        scannable_extensions = {
            ...
            '.c', '.cpp', '.h', '.hpp', '.cs', '.vb', '.sql', '.xml', '.json',
            '.yaml', '.yml', '.sh', '.bat', '.ps1', '.config'
        }
        ...
        def should_scan_file(file_path: Path) -> bool:
            return (file_path.suffix in scannable_extensions and 
                   file_path.stat().st_size < 1_000_000)  # Skip files > 1MB
        ...
        try:
            for item in self.workspace_root.rglob('*'):
                if item.is_file() and should_scan_file(item):
                    ...
                    if all(should_scan_dir(parent) for parent in item.parents 
                          if self.workspace_root in parent.parents or parent == self.workspace_root):
    ...
    async def _record_scan_in_memory(self, scan_results: Dict[str, Any]):
        ...
        try:
            ...
            self.logger.debug(f"Recorded security scan {scan_results['scan_id']} in memory")
