...
class SelfHealingAgent:
    ...
    def __init__(
        ...
        self.logger.info(
            f"SelfHealingAgent initialized for workspace: {workspace_root}"
        )

    async def detect_and_heal(
        self, trigger_source: str = "manual", context: Optional[Dict[str, Any]] = None
    ) -> HealingResult:
        """
        ...
        failure_context = FailureContext(
            ...
            error_message=(
                context.get("error_message", "Unknown failure")
                if context
        ...
        healing_result = HealingResult(
            ...
            metadata={"trigger_source": trigger_source, "context": context or {}},
        )
        ...
        try:
            ...
            failure_context = max(
                failures, key=lambda f: self._get_severity_score(f.severity)
            )
            ...
            healing_result.success = (
                application_results["success"]
                and validation_result["tests_passed"]
                and validation_result["issue_resolved"]
            )
            ...
            if healing_result.success:
                ...
                if self.config["auto_rollback_on_failure"]:
                    rollback_success = await self._perform_rollback(
                        application_results.get("snapshot_id")
                    )
            ...
            healing_result.healing_time_seconds = (
                datetime.utcnow() - healing_start
            ).total_seconds()
        ...
        self.logger.info(
            f"Healing cycle completed: {healing_id}, Success: {healing_result.success}"
        )
        return healing_result
    ...
    async def _detect_failures(
        ...
        try:
            ...
            if context and context.get("error_logs"):
                runtime_failures = await self._detect_runtime_errors(
                    context["error_logs"]
                )
    ...
    async def _detect_runtime_errors(
    ...
    async def _generate_healing_patches(
        self, failure_context: FailureContext, diagnosis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if failure_context.failure_type == FailureType.SYNTAX_ERROR:
                patches = await self._generate_syntax_fix_patches(
                    failure_context, diagnosis
                )
            elif failure_context.failure_type == FailureType.SECURITY_VULNERABILITY:
                patches = await self._generate_security_fix_patches(
                    failure_context, diagnosis
                )
            elif failure_context.failure_type == FailureType.TEST_FAILURE:
                patches = await self._generate_test_fix_patches(
                    failure_context, diagnosis
                )
            else:
                patches = await self._generate_generic_fix_patches(
                    failure_context, diagnosis
                )
    ...
    async def _generate_syntax_fix_patches(
        self, failure_context: FailureContext, diagnosis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
            ...
            try:
                ...
                if full_path.exists():
                    ...
                    fixed_content = await call_llm(
                        message=fix_prompt, context={}, model="gpt-4", mode="code_fix"
                    )
    ...
    async def _generate_security_fix_patches(
        self, failure_context: FailureContext, diagnosis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
            ...
            security_patches = await self.security_agent._generate_security_patches(
                [
                    SecurityFinding(
                        ...
                        description=failure_context.stack_trace
                        or failure_context.error_message,
                        file_path=(
                            failure_context.affected_files[0]
                            if failure_context.affected_files
                            else "unknown"
                        ),
    ...
    async def _generate_test_fix_patches(
        self, failure_context: FailureContext, diagnosis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            fix_suggestions = await call_llm(
                message=fix_prompt, context={}, model="gpt-4", mode="test_fix"
            )
    ...
    async def _generate_generic_fix_patches(
        self, failure_context: FailureContext, diagnosis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    async def _apply_patches_safely(
        ...
        try:
            ...
            snapshot = self.sandbox.snapshot_manager.create_snapshot(
    ...
    def _calculate_healing_confidence(
        self,
        failure_context: FailureContext,
    ...
    async def _record_healing_in_memory(self, healing_result: HealingResult):
        ...
        try:
            ...
            if healing_result.failure_context:
                event_content += (
                    f" - {healing_result.failure_context.failure_type.value}"
                )
            ...
            await self.memory.record_event(
                ...
                metadata={
                    ...
                    "failure_type": (
                        healing_result.failure_context.failure_type.value
                        if healing_result.failure_context
                        else None
                    ),
                },
    ...
    def get_healing_status(self) -> Dict[str, Any]:
        ...
        recent_healings = [
            ...
            if datetime.utcnow()
            - datetime.fromisoformat(h.metadata.get("timestamp", "2024-01-01T00:00:00"))
            < timedelta(hours=24)
        ]
