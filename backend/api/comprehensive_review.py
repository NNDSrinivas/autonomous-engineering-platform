...
async def comprehensive_analysis(
    ...
    try:
        ...
        logger.info(
            f"Starting comprehensive analysis for {repo_path} with {analysis_depth} depth"
        )
        ...
        logger.info(
            f"Comprehensive analysis complete - analyzed {analysis.files_analyzed} files"
        )
        ...
        try:
            ...
            try:
                review_service = RealReviewService(
                    repo_path, analysis_depth=analysis_depth
                )
            except ValueError as e:
            ...
            for i, change in enumerate(
                changes[: review_service.config["max_files"]], 1
            ):
                ...
                try:
                    ...
                    file_analysis = (
                        await review_service.analyze_file_change_comprehensive(change)
                    )
            ...
            if len(file_analyses) > 1 and review_service.config.get(
                "enable_architecture_analysis", False
            ):
                yield "event: live-progress\ndata: Analyzing cross-file architecture patterns…\n\n"
                architecture_insights = (
                    await review_service._analyze_architecture_patterns(
                        file_analyses, repo_context
                    )
                )
            else:
                architecture_insights = []
            ...
            final_analysis = {
                ...
                "total_issues": len(
                    [issue for entry in review_entries for issue in entry.issues]
                ),
                "review_entries": [
                    {
                        "file": entry.file,
...
async def get_repository_summary(
    ...
    try:
        ...
        return JSONResponse(content={"success": True, "repository_summary": summary})

    except Exception as e:
        logger.error(f"Repository summary failed: {e}")
        raise HTTPException(
            status_code=500, detail=f"Failed to get repository summary: {str(e)}"
        )
