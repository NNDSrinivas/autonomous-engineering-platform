...
class IngestEvent(BaseModel):
    ...
    """
    source: str = Field(..., description="Source system: jira, slack, github, zoom, teams, jenkins, confluence, etc.")
    event_type: str = Field(..., description="Type of event: issue_updated, comment_added, message_sent, pr_merged, etc.")
    external_id: str = Field(..., description="External identifier: SCRUM-1, commit sha, slack timestamp, etc.")
    ...
    content: Optional[str] = Field(None, description="Full body content, transcript, or message text")
    ...
    tags: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata and tags")
    importance: Optional[int] = Field(default=3, ge=1, le=5, description="Importance level 1-5")
