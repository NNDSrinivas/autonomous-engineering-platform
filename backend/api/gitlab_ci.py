...
def _resolve_connector(
    ...
    if connector_name and connectors_service.connectors_available(db):
        conn = connectors_service.get_connector(
            db, user_id=user_id, provider="gitlab", name=connector_name
        )
        if conn:
    ...
    if not token:
        raise HTTPException(
            status_code=500, detail="Configure a GitLab connector or set GITLAB_TOKEN"
        )
...
class GitLabDispatch(BaseModel):
    ...
    connector_name: Optional[str] = Field(
        None, description="Saved connector name (default)"
    )
        ...
        async with await _client(resolved["base_url"], resolved["token"]) as client:
            resp = await client.post(
                ...
                json={
                    "ref": req.ref,
                    "variables": [
                        {"key": k, "value": v} for k, v in (req.variables or {}).items()
                    ],
                },
            )
    ...
    try:
        async with await _client(resolved["base_url"], resolved["token"]) as client:
            resp = await client.get(
                f"/projects/{req.project_id}/pipelines/{req.pipeline_id}"
            )
