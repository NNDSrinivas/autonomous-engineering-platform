...
async def classify_intent(req: IntentRequest) -> IntentResponse:
    ...
    if not client:
        ...
        if any(
            word in text
            for word in ["jira", "ticket", "issue", "task", "story", "sprint"]
        ):
            if any(
                phrase in text
                for phrase in ["my tasks", "what tickets", "show", "list"]
            ):
                return IntentResponse(intent="jira_list")
            elif any(
                phrase in text for phrase in ["priority", "first", "next", "should I"]
            ):
        ...
        if any(
            word in text
            for word in ["code", "function", "bug", "error", "refactor", "test"]
        ):
