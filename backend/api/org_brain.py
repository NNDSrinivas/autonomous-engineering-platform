...
async def query_organizational_brain(
    request: OrgBrainQueryRequest, mg: MemoryGraphService = Depends(get_memory_service)
):
    """
...
@router.post(
    "/nodes", response_model=MemoryNodeResponse, status_code=status.HTTP_201_CREATED
)
async def create_node(
    node: MemoryNodeCreate, mg: MemoryGraphService = Depends(get_memory_service)
):
    """Create a new memory node."""
    try:
        node_id = await mg.add_node(
            node_type=node.node_type, text=node.text, title=node.title, meta=node.meta
        )
...
async def get_node(node_id: int, mg: MemoryGraphService = Depends(get_memory_service)):
...
async def delete_node(
    node_id: int, mg: MemoryGraphService = Depends(get_memory_service)
):
...
@router.post(
    "/edges", response_model=MemoryEdgeResponse, status_code=status.HTTP_201_CREATED
)
async def create_edge(
    edge: MemoryEdgeCreate, mg: MemoryGraphService = Depends(get_memory_service)
):
...
async def navigate_graph(
    ...
    try:
        ...
        related_nodes, edges = mg.get_related_nodes(
            node_id=request.node_id, edge_types=request.edge_types, depth=request.depth
        )
    ...
    try:
        ...
        issue_dict = {
            ...
            "fields": {
                ...
                "assignee": (
                    {"displayName": request.assignee} if request.assignee else None
                ),
...
async def ingest_slack_message(
    ...
    try:
        ...
        node_id = await ingestor.ingest_message(
            channel_id=request.channel_id, message=message_dict
        )
...
async def ingest_github_pr(
    request: GitHubPRIngestRequest, mg: MemoryGraphService = Depends(get_memory_service)
):
