            ...
            for i, change in enumerate(changes, 1):
                ...
                try:
                    ...
                    review_data = {
                        'file': entry.file,
                        'hunk': entry.diff,
                        'severity': 'high' if any(issue.severity == 'error' for issue in entry.issues) else 'medium' if any(issue.severity == 'warning' for issue in entry.issues) else 'low',
                        'title': f"Found {len(entry.issues)} issues" if entry.issues else "No issues found",
                        'body': "\\n".join([f"{issue.severity.upper()}: {issue.title} - {issue.message}" for issue in entry.issues]) if entry.issues else "Code looks good!",
