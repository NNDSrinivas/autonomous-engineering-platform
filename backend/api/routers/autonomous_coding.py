...
from backend.autonomous.enhanced_coding_engine import EnhancedAutonomousCodingEngine, CodingTask
...
async def create_task_from_jira(
    request: CreateTaskFromJiraRequest,
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create autonomous task: {str(e)}"
        )
...
async def execute_step(
    ...
    try:
        ...
        logger.info(f"Executed step {request.step_id} for task {request.task_id}: {result['status']}")
...
async def get_user_daily_context(
    request: Request,
    db: Session = Depends(get_db)
):
    """
