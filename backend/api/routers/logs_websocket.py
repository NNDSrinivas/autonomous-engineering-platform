...
async def _stream_docker_logs(
    ...
    try:
        ...
        if ps_result.returncode != 0:
            await websocket.send_json(
                {"type": "error", "message": "Docker is not running or not installed"}
            )
            return
        ...
        if service:
            matching_containers = [
                c for c in running_containers if service.lower() in c.lower()
            ]
            ...
            if not running_containers:
                await websocket.send_json(
                    {"type": "error", "message": "No running containers found"}
                )
                return
        ...
        await websocket.send_json(
            {"type": "error", "message": f"Docker log streaming failed: {str(e)}"}
        )
...
async def _stream_k8s_logs(
    ...
    try:
        ...
        if pods_result.returncode != 0:
            await websocket.send_json(
                {"type": "error", "message": "kubectl not available or not configured"}
            )
            return

        all_pods = [
            p.strip() for p in pods_result.stdout.strip().split("\n") if p.strip()
        ]
        ...
        if service:
            ...
            if not all_pods:
                await websocket.send_json({"type": "error", "message": "No pods found"})
                return
            pod = all_pods[0]
        ...
        await websocket.send_json(
            {"type": "stream_started", "message": f"Streaming logs from pod: {pod}"}
        )
        ...
        await websocket.send_json(
            {"type": "error", "message": f"Kubernetes log streaming failed: {str(e)}"}
        )
...
async def _stream_file_logs(
    ...
    try:
        ...
        await websocket.send_json(
            {"type": "error", "message": f"File log streaming failed: {str(e)}"}
        )
