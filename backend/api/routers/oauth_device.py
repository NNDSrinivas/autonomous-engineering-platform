...
class DeviceCodeStartRequest(BaseModel):
    client_id: Optional[str] = Field(default="aep-vscode-extension", description="Client identifier")
...
class DeviceCodeStartResponse(BaseModel):
    ...
    verification_uri_complete: Optional[str] = Field(description="Complete URI with user_code")
...
async def poll_device_code(request: DeviceCodePollRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to poll device code: {str(e)}")
