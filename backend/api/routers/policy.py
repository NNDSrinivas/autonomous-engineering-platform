...
def upsert_policy(
    ...
    Returns:
        Success indicator
    """
    org_id: str = (request.headers.get("X-Org-Id") if request else None) or "default"
    user_id: str = (request.headers.get("X-User-Id") if request else None) or "unknown"
...
def check_policy_action(
    ...
    def check_action(policy: dict, action: dict) -> dict:
        ...
        return {"allowed": True, "reason": "Policy check disabled"}
