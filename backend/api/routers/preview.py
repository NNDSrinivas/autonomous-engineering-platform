...
async def get_preview(
    ...
    - Content-Security-Policy with restrictions:
      ...
      * style-src 'unsafe-inline' - Allows inline styles
        ⚠️  WARNING: This enables CSS-based data exfiltration via background-image URLs
        This is an ACCEPTED RISK for Phase 1 trusted previews (NAVI-generated content)
        Phase 2 will implement nonce-based CSP for untrusted content
      * connect-src 'none' - NO fetch/XHR (but doesn't prevent CSS exfiltration)
      * form-action 'none' - NO form submissions
      * base-uri 'none' - NO <base> tag injection
    - Cross-Origin-Resource-Policy: same-site
    - Cache-Control: no-store

    ⚠️  IMPORTANT: Only use this endpoint for TRUSTED content generated by NAVI.
    Do NOT use for user-uploaded or untrusted HTML due to CSS exfiltration risk.
    """
    # Get PreviewService singleton from app state
    preview_service = getattr(request.app.state, "preview_service", None)
    if not preview_service:
