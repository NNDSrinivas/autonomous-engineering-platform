...
async def get_tasks(
    ...
    try:
        ...
        count_result = db.execute(text(count_query), {k: v for k, v in params.items() if k not in ["limit", "offset"]}).mappings().first()
    ...
    try:
        ...
        db.execute(
            ...
            """),
            {
                ...
                "status": request.priority.value if hasattr(request, 'status') else TaskStatus.TODO.value,
...
async def update_task(
    ...
    try:
        ...
        if request.title or request.description:
            await _index_task(db, org_id, task_id, request.title or "", request.description or "")
...
async def get_projects(
    ...
    try:
        ...
        projects = []
        for row in result.mappings():
            progress = (row["completed_tasks"] / row["task_count"] * 100) if row["task_count"] > 0 else 0
...
async def get_project(
    ...
    try:
        ...
        progress = (result["completed_tasks"] / result["task_count"] * 100) if result["task_count"] > 0 else 0
...
async def get_analytics(
    ...
    try:
        ...
        # Calculate timeframe
        days_back = {"week": 7, "month": 30, "quarter": 90, "year": 365}.get(timeframe, 30)
        since_date = datetime.now() - timedelta(days=days_back)
        
        # Get comprehensive analytics
        analytics = {
            "productivity_metrics": await _get_productivity_metrics(db, org_id, since_date, project_key, team_member),
            "completion_trends": await _get_completion_trends(db, org_id, since_date, project_key),
            "bottleneck_analysis": await _get_bottleneck_analysis(db, org_id, since_date),
...
async def _generate_tasks_summary(db: Session, org_id: str, user_id: Optional[str]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "completion_rate": (stats["done"] / stats["total"] * 100) if stats and stats["total"] > 0 else 0
        }
...
async def _index_task(db: Session, org_id: str, task_id: str, title: str, description: str):
    """Index task for search"""
    try:
        content = f"{title} {description}"
        upsert_memory_object(
            db, org_id, "task", task_id, title, "", "en", {}, content
        )
    except Exception as e:
        logger.error(f"Error indexing task: {e}")


async def _get_dashboard_metrics(db: Session, org_id: str, user_id: Optional[str], since_date: datetime) -> Dict[str, Any]:
    ...
    try:
        metrics = db.execute(
            text("""
                ...
                WHERE org_id = :org_id
            """),
            {"org_id": org_id, "user_id": user_id, "since_date": since_date, "now": datetime.now()}
        ).mappings().first()
...
async def _get_recent_activity(db: Session, org_id: str, user_id: Optional[str], limit: int) -> List[Dict[str, Any]]:
...
async def _get_upcoming_deadlines(db: Session, org_id: str, user_id: Optional[str], days: int) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for row in deadlines.mappings():
            deadline_list.append({
                ...
                "days_remaining": (row["due_date"] - datetime.now()).days if row["due_date"] else None
            })
...
async def _get_velocity_trends(db: Session, org_id: str, timeframe: str) -> Dict[str, Any]:
    """Get team velocity trends"""
    try:
        # Simplified velocity calculation
        days_back = {"day": 7, "week": 28, "month": 90, "quarter": 365}.get(timeframe, 28)
        since_date = datetime.now() - timedelta(days=days_back)
...
async def _get_productivity_metrics(db: Session, org_id: str, since_date: datetime, project_key: Optional[str], team_member: Optional[str]) -> Dict[str, Any]:
...
async def _get_completion_trends(db: Session, org_id: str, since_date: datetime, project_key: Optional[str]) -> Dict[str, Any]:
...
async def _get_bottleneck_analysis(db: Session, org_id: str, since_date: datetime) -> Dict[str, Any]:
...
async def _get_team_performance(db: Session, org_id: str, since_date: datetime) -> Dict[str, Any]:
...
async def _get_project_health(db: Session, org_id: str, project_key: Optional[str]) -> Dict[str, Any]:
...
async def _get_forecasting_data(db: Session, org_id: str, since_date: datetime) -> Dict[str, Any]:
...
async def _get_quality_metrics(db: Session, org_id: str, since_date: datetime) -> Dict[str, Any]:
    """Get quality metrics"""
    return {
        "defect_rate": 0.05,
