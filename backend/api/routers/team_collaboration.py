...
async def get_team_context(
    ...
    try:
        ...
        suggestions = await _generate_collaboration_suggestions(
            db, org_id, team_members
        )
        ...
        return TeamContextResponse(
            ...
            collaboration_score=_calculate_collaboration_score(
                team_members, active_projects
            ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get team context: {str(e)}"
        )
...
async def get_team_activity(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get team activity: {str(e)}"
        )
...
async def get_team_insights(
    ...
    try:
        ...
        insights = {
            "productivity_trends": await _analyze_productivity_trends(
                db, org_id, timeframe
            ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get team insights: {str(e)}"
        )
...
async def update_presence(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update presence: {str(e)}"
        )
...
async def _get_active_projects(
    db: Session, org_id: str, project_filter: Optional[str]
) -> List[Dict[str, Any]]:
...
async def _get_current_sprint(
    db: Session, org_id: str, project: Optional[str]
...
async def _generate_collaboration_suggestions(
    db: Session, org_id: str, team_members: List[TeamMember]
...
async def _identify_knowledge_gaps(db: Session, org_id: str) -> List[str]:
    ...
    try:
        ...
        for row in result.mappings():
            gaps.append(
                f"{row['component']} (only {row['assignee_count']} team member(s) working on {row['issue_count']} issues)"
            )
        ...
        for row in reopened_result.mappings():
            gaps.append(
                f"{row['issue_type']} issues frequently reopened ({row['reopened_count']} cases)"
            )
...
def _calculate_collaboration_score(
    team_members: List[TeamMember], active_projects: List[Dict[str, Any]]
) -> float:
    """Calculate team collaboration score"""
    try:
        # Simple scoring based on activity and project involvement
        active_members = len(
            [m for m in team_members if m.status in ["active", "busy"]]
        )
...
async def _calculate_team_pulse(db: Session, org_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Calculate metrics
        activity_rate = (
            activity_result["recent_activity"] / activity_result["total_issues"]
        )
        completion_rate = (
            activity_result["completed_issues"] / activity_result["total_issues"]
        )
        collaboration_index = min(
            activity_result["active_members"] / 5.0, 1.0
        )  # Normalize to team size
        ...
        overall_health = (
            completion_rate * 0.5 + activity_rate * 0.3 + collaboration_index * 0.2
        )
...
async def _store_team_activity(
    db: Session, org_id: str, activity: Dict[str, Any]
) -> str:
...
async def _index_team_activity(
    db: Session, org_id: str, activity_id: str, activity: Dict[str, Any]
):
