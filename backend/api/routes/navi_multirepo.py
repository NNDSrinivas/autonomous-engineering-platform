...
class ImpactAnalysisRequest(BaseModel):
    ...
    changed_files: Optional[List[str]] = Field(
        None, description="List of files being changed"
    )
    change_type: Optional[str] = Field(
        "code_change", description="Type of change (code_change, api_change, etc.)"
    )
...
class ArchitecturalGuidanceRequest(BaseModel):
    ...
    problem_description: str = Field(
        ..., description="Description of the architectural problem"
    )
    workspace_root: str = Field(..., description="Root path of the workspace")
...
class CoordinatedChangeRequest(BaseModel):
    ...
    workspace_root: str = Field(..., description="Root path of the workspace")
    change_requests: List[Dict[str, Any]] = Field(
        ..., description="Individual repository changes"
    )
...
async def analyze_cross_repo_impact(request: ImpactAnalysisRequest):
    ...
    try:
        logger.info(
            f"Analyzing cross-repo impact: {request.change_description[:100]}..."
        )
        ...
        return ImpactAnalysisResponse(analysis_available=False, error=str(e))
...
async def get_architectural_guidance(request: ArchitecturalGuidanceRequest):
    ...
    try:
        logger.info(
            f"Requesting architectural guidance: {request.problem_description[:100]}..."
        )
        ...
        return ArchitecturalGuidanceResponse(guidance_available=False, error=str(e))
...
async def get_system_health(workspace_root: str):
    ...
    try:
        ...
        return SystemHealthResponse(insights_available=False, error=str(e))
...
async def enhance_intent_with_system_context(request: EnhanceIntentRequest):
    ...
    try:
        ...
        enhanced_intent = await enhance_navi_intent_with_system_context(
            intent=request.intent, workspace_root=request.workspace_root
        )
...
async def coordinate_multi_repo_change(
    request: CoordinatedChangeRequest, background_tasks: BackgroundTasks
):
    """
    ...
    try:
        ...
        change_plan = await integration.plan_coordinated_change(
            ...
            impact_analysis={
                "affected_repositories": [
                    cr.get("repo_name") for cr in request.change_requests
                ]
            },
        )
        ...
        if not change_plan.get("plan_available", False):
            raise HTTPException(
                status_code=400,
                detail=change_plan.get("reason", "Failed to create plan"),
            )
...
async def get_integration_status(workspace_root: str):
    ...
    try:
        ...
        return {
            ...
            "last_health_check": (
                system_context.last_health_check.isoformat()
                if system_context.last_health_check
                else None
            ),
            "capabilities": [
        ...
        return {"integration_enabled": False, "error": str(e), "status": "error"}
...
async def discover_repositories(workspace_root: str, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        background_tasks.add_task(
            _discover_repositories_background, integration, workspace_root
        )
