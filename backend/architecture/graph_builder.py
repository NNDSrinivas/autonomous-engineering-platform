    ...
    def analyze_file(self, file_path: Path) -> Tuple[List[GraphNode], List[GraphEdge]]:
        ...
        try:
            ...
            file_node = GraphNode(
                ...
                metadata={'size_bytes': len(content), 'lines': len(content.splitlines())}
            )
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        ...
        func_node = GraphNode(
            ...
            metadata={
                ...
                'is_method': len(self.current_scope) > 0 and self.current_scope[-1] != node.name
            }
        )
        self.nodes.append(func_node)
        
        # Create containment edge
        parent_id = self._get_current_scope_id() if self.current_scope else self.file_node_id
        edge = GraphEdge(
            source_id=parent_id,
            target_id=func_id,
            edge_type=EdgeType.CONTAINS,
...
class JavaScriptAnalyzer:
    ...
    def analyze_file(self, file_path: Path) -> Tuple[List[GraphNode], List[GraphEdge]]:
        ...
        try:
            ...
            # Create file node
            file_node_id = f"file_{hashlib.md5(str(file_path).encode()).hexdigest()[:12]}"
            file_node = GraphNode(
                node_id=file_node_id,
                name=file_path.name,
                node_type=NodeType.FILE,
            ...
            js_nodes, js_edges = self._analyze_js_content(content, file_path, file_node_id)
    ...
    def _analyze_js_content(self, content: str, file_path: Path, file_node_id: str) -> Tuple[List[GraphNode], List[GraphEdge]]:
        ...
        for pattern in import_patterns:
            for match in re.finditer(pattern, content):
                ...
                dep_node = GraphNode(
                    ...
                    name=module_name,
                    node_type=NodeType.EXTERNAL_DEPENDENCY,
                    metadata={'import_type': 'es6' if 'import' in match.group(0) else 'commonjs'}
                )
...
class ArchitectureAnalyzer:
    ...
    async def analyze_architecture(self, 
                                 ...
                                 include_external_deps: bool = True) -> ArchitectureGraph:
        """
        ...
        try:
            ...
            if analysis_level in [AnalysisLevel.COMPREHENSIVE, AnalysisLevel.DEEP]:
                all_nodes, all_edges = await self._enhance_analysis(all_nodes, all_edges)
    ...
    def _get_analyzer_for_file(self, file_path: Path) -> Optional[Union[PythonAnalyzer, JavaScriptAnalyzer]]:
        """Get appropriate analyzer for file."""
        return self.analyzers.get(file_path.suffix)
    
    async def _enhance_analysis(self, 
                              nodes: Dict[str, GraphNode], 
                              edges: List[GraphEdge]) -> Tuple[Dict[str, GraphNode], List[GraphEdge]]:
    ...
    def _filter_external_deps(self, 
                            nodes: Dict[str, GraphNode], 
                            edges: List[GraphEdge]) -> Tuple[Dict[str, GraphNode], List[GraphEdge]]:
    ...
    def detect_patterns(self, arch_graph: ArchitectureGraph) -> Dict[str, Any]:
        ...
        if len(G.nodes()) > 0:
            ...
            patterns['god_classes'] = sorted(god_classes, key=lambda x: x['in_degree'], reverse=True)[:5]
        ...
        try:
            await self.memory.record_event(
                ...
                metadata={
                    ...
                    'analysis_duration': graph.metadata.get('analysis_duration_seconds', 0)
                }
            )
        except Exception as e:
    ...
    def generate_hierarchical_data(arch_graph: ArchitectureGraph) -> Dict[str, Any]:
        ...
        for edge in arch_graph.edges:
            if edge.edge_type == EdgeType.CONTAINS:
                ...
                if source_node and target_node and source_node.node_type == NodeType.FILE:
