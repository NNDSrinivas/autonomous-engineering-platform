...
class PlanEvent(Base):
    ...
    seq: Mapped[int] = mapped_column(Integer, index=True, nullable=False)  # monotonic per plan
...
class AuditLog(Base):
    ...
    actor_sub: Mapped[str | None] = mapped_column(String(128), index=True, nullable=True)
    actor_email: Mapped[str | None] = mapped_column(String(255), nullable=True)
    route: Mapped[str] = mapped_column(String(255), nullable=False)
    method: Mapped[str] = mapped_column(String(16), nullable=False)
    event_type: Mapped[str] = mapped_column(String(64), nullable=False)
    resource_id: Mapped[str | None] = mapped_column(String(128), nullable=True)  # e.g., plan_id
    payload: Mapped[dict] = mapped_column(JSON, nullable=False)
