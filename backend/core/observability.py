...
class ROIMetrics:
    ...
    def __post_init__(self):
        ...
        if self.navi_investment > 0:
            ...
            monthly_savings = self.net_savings / max(1, (self.period_end - self.period_start).days / 30)
            monthly_cost = self.navi_investment / max(1, (self.period_end - self.period_start).days / 30)
...
class MetricsCollector:
    ...
    def _load_default_thresholds(self) -> Dict[MetricType, Dict[str, float]]:
        ...
        return {
            ...
            MetricType.MEAN_TIME_TO_RECOVERY: {"warning": 4.0, "critical": 12.0},  # hours
    ...
    async def record_initiative_completion(self, 
                                         initiative_id: str,
                                         success: bool,
                                         duration_hours: float,
                                         complexity_score: int = 5) -> None:
    ...
    async def record_pr_metrics(self, 
                               pr_id: str,
                               created_at: datetime,
...
class MetricsRepository(TenantRepository):
    ...
    async def get_metrics(self,
                         ...
                         end_time: datetime,
                         tags: Optional[Dict[str, str]] = None,
                         limit: int = 10000) -> List[MetricPoint]:
    ...
    async def get_metric_summary(self,
        ...
        previous_metrics = await self.get_metrics(metric_type, prev_start_time, start_time)
        ...
        confidence_score = min(1.0, len(current_values) / 100)  # More data = higher confidence
...
class ROIAnalyzer:
    ...
    async def calculate_comprehensive_roi(self,
                                        ...
                                        navi_users: int = 10,
                                        custom_navi_cost: Optional[float] = None) -> ROIMetrics:
        ...
        navi_cost = custom_navi_cost or (self.default_navi_cost_per_user_monthly * navi_users * (period_days / 30))
        ...
        productivity_gains = await self._calculate_productivity_gains(start_time, end_time)
        
        # Calculate quality improvements
        quality_metrics = await self._calculate_quality_metrics(start_time, end_time)
        
        # Business impact assessment
        business_impact = await self._assess_business_impact(metrics_data, productivity_gains)
        ...
        return ROIMetrics(
            ...
            feature_delivery_acceleration=business_impact["feature_delivery_acceleration"],
    ...
    async def _collect_roi_metrics(self, start_time: datetime, end_time: datetime) -> Dict[str, float]:
        ...
        return {
            ...
            "initiatives_count": len(await self.metrics_repo.get_metrics(
                MetricType.INITIATIVES_COMPLETED, start_time, end_time
            )),
            "auto_fixes_count": len(await self.metrics_repo.get_metrics(
                MetricType.AUTO_FIXES_APPLIED, start_time, end_time
            ))
        }
    
    async def _calculate_productivity_gains(self, start_time: datetime, end_time: datetime) -> Dict[str, float]:
    ...
    async def _calculate_quality_metrics(self, start_time: datetime, end_time: datetime) -> Dict[str, int]:
    ...
    async def _assess_business_impact(self, metrics_data: Dict, productivity_gains: Dict) -> Dict[str, float]:
...
class DashboardGenerator:
    ...
    async def generate_executive_dashboard(self, period_days: int = 90) -> Dict[str, Any]:
    ...
    async def generate_engineering_dashboard(self, period_days: int = 30) -> Dict[str, Any]:
