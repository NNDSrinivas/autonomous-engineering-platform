    ...
    def from_dict(cls, data: Dict[str, Any]) -> "OrganizationConfig":
        ...
        return cls(
            ...
            allowed_integrations=(
                data.get("allowed_integrations", "").split(",")
                if data.get("allowed_integrations")
                else []
            ),
...
class UserMembership:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "UserMembership":
        return cls(
            ...
            permissions=(
                data.get("permissions", "").split(",")
                if data.get("permissions")
                else []
            ),
            team_ids=(
                data.get("team_ids", "").split(",") if data.get("team_ids") else []
            ),
            joined_at=(
...
class UserMembershipRepository(TenantRepository):
    ...
    @require_role(TenantRole.ORG_ADMIN)
    async def update_user_roles(self, user_id: str, roles: List[TenantRole]) -> bool:
...
class OrganizationService:
    ...
    async def create_organization_with_admin(
        self, org_config: OrganizationConfig, admin_user_id: str
    ) -> OrganizationConfig:
        ...
        async def add_admin():
            return await self.membership_repo.add_user(
                admin_user_id, [TenantRole.ORG_ADMIN], ["*"]
            )
