...
class RedisJobQueue(JobQueue):
    ...
    async def enqueue(self, job: Job) -> bool:
        ...
        try:
            ...
            logger.info(f"Enqueued job {job.id} of type {job.type} for org {job.context.org_id}")
            return True
...
class BackgroundWorker:
    ...
    async def _process_job(self, job: Job):
        ...
        try:
            ...
            logger.info(f"Job {job.id} completed successfully in {job.context.duration()}")
            ...
            if job.context.retry_count < job.context.max_retries:
                ...
                job.scheduled_for = datetime.utcnow() + timedelta(minutes=5 * job.context.retry_count)
...
class JobScheduler:
    ...
    async def schedule_initiative_execution(self, initiative_id: str, 
                                          goal: str) -> str:
    ...
    async def schedule_ci_repair(self, build_id: str, failure_data: Dict[str, Any]) -> str:
    ...
    async def get_job_status(self, job_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "started_at": job.context.started_at.isoformat() if job.context.started_at else None,
            "completed_at": job.context.completed_at.isoformat() if job.context.completed_at else None,
