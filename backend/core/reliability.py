...
class StatelessExecutionEngine:
    ...
    async def execute_with_checkpoints(
        ...
        existing_checkpoint = await self.checkpoint_repo.get_latest_checkpoint(
            execution_id
        )

        if (
            existing_checkpoint
            and existing_checkpoint.state != ExecutionState.COMPLETED
        ):
            logger.info(
                f"Resuming execution {execution_id} from step {existing_checkpoint.step_index}"
            )
    ...
    async def resume_execution(self, execution_id: str) -> Optional[Dict[str, Any]]:
        ...
        logger.info(
            f"Resuming execution {execution_id} from step {checkpoint.step_index}"
        )
...
class CircuitBreaker:
    ...
    def _is_circuit_open(self) -> bool:
        ...
        if (
            ...
            and datetime.utcnow() >= self.state.next_attempt_time
        ):
...
class BackupManager:
    ...
    async def create_backup(
        ...
        logger.info(
            f"Created {backup_type.value} backup {backup_id} ({metadata.size_bytes} bytes)"
        )
    ...
    async def _perform_backup(
        self, data_sources: List[str], backup_type: BackupType
    ) -> Dict[str, Any]:
    ...
    async def _store_backup(
        self, backup_id: str, data: Dict[str, Any], encrypt: bool
    ) -> str:
    ...
    async def _perform_restore(
        self, backup_data: Dict[str, Any], target_location: str
    ) -> bool:
...
class HealthMonitor:
    ...
    def register_self_healing_action(
        self, condition: str, action_func: Callable[[], Awaitable[bool]]
    ):
    ...
    async def _trigger_self_healing(self, check_name: str, health_result: HealthCheck):
        ...
        logger.warning(
            f"Triggering self-healing for {check_name}: {health_result.message}"
        )

        # Look for matching self-healing actions
        for condition, action in self.self_healing_actions.items():
            if (
                condition in check_name.lower()
                or condition in health_result.message.lower()
            ):
                try:
                    success = await action()
                    if success:
                        logger.info(
                            f"Self-healing action '{condition}' succeeded for {check_name}"
                        )
                    else:
                        logger.warning(
                            f"Self-healing action '{condition}' failed for {check_name}"
                        )
                except Exception as e:
