...
class SecretStore(ABC):
    ...
    @abstractmethod
    async def store_secret(self, org_id: str, key: str, value: str, 
...
class LocalEncryptedSecretStore(SecretStore):
    ...
    async def store_secret(self, org_id: str, key: str, value: str,
        ...
        try:
            ...
            TenantAuditLogger.log_access("secrets", "store", {"key": key, "integration": metadata.integration_type.value})
            return True
...
class SecretsManager:
    ...
    async def store_oauth_credentials(self, integration: IntegrationType,
                                    credentials: OAuthCredentials) -> bool:
        ...
        return await self.secret_store.store_secret(
            tenant.org_id, 
            f"oauth_{integration.value}",
            credentials_json,
            metadata
        )
    
    async def get_oauth_credentials(self, integration: IntegrationType) -> Optional[OAuthCredentials]:
        ...
        try:
            ...
            logger.error(f"Failed to parse OAuth credentials for {integration.value}: {e}")
            return None
        ...
        for key in secret_keys:
            if key.startswith("oauth_") or key.startswith("apikey_"):
                ...
                integrations.append({
                    ...
                    "auth_method": "oauth" if key.startswith("oauth_") else "api_key",
                    "configured_at": "unknown"  # Would fetch from metadata
                })
        
        return integrations
    
    def get_oauth_authorization_url(self, integration: IntegrationType,
...
def create_secrets_manager(provider: SecretProvider, **config) -> SecretsManager:
    ...
    if provider == SecretProvider.LOCAL_ENCRYPTED:
        ...
        if not vault_url or not vault_token:
            raise ValueError("vault_url and vault_token are required for Vault secrets provider")
