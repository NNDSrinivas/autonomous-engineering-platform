...
class TenantQueryBuilder:
    ...
    async def find_many(
        ...
        TenantAuditLogger.log_access(
            table, "read_many", {"filters": filters, "limit": limit}
        )
    ...
    async def update(
        ...
        TenantAuditLogger.log_access(
            table, "update", {"filters": filters, "updates_keys": list(updates.keys())}
        )
...
class TenantRepository:
    ...
    async def create(
        self, data: Dict[str, Any], table_name: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    async def create_or_update(
        self, data: Dict[str, Any], record_id: Optional[str] = None
    ) -> Dict[str, Any]:
...
def get_tenant_db() -> TenantDatabase:
    ...
    if not tenant_db:
        raise RuntimeError(
            "Tenant database not initialized. Call init_tenant_database() first."
        )
    return tenant_db
