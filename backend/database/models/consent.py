...
class ConsentPreference(Base):
    ...
    preference_type = Column(String(20), nullable=False)  # 'exact_command' or 'command_type'
    command_pattern = Column(Text, nullable=False)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    created_by_task_id = Column(String(50), nullable=True)

    __table_args__ = (
        UniqueConstraint('user_id', 'org_id', 'preference_type', 'command_pattern',
...
class ConsentAuditLog(Base):
    ...
    decision = Column(String(20), nullable=False)  # 'allow_once', 'allow_always_exact', 'allow_always_type', 'deny', 'alternative'
    ...
    __table_args__ = (
        Index('idx_consent_audit_user', 'user_id', 'requested_at', postgresql_using='btree'),
    )
