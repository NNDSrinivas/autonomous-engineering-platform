...
class ReasoningGraph:
    ...
    def calculate_confidence_metrics(self) -> Dict[str, float]:
        ...
        decision_confidences = [
            self.nodes[node_id].confidence_score 
            for node_id in self.decision_nodes
        ]
        ...
        if all_evidence:
            evidence_strength = sum(e.reliability_score for e in all_evidence) / len(all_evidence)
    ...
    def finalize_graph(self) -> None:
        ...
        if self.overall_confidence > 0.8 and confidence_metrics["evidence_strength"] > 0.7:
            self.risk_level = "low"
        elif self.overall_confidence > 0.6 and confidence_metrics["evidence_strength"] > 0.5:
...
class ExplainableAISystem:
    ...
    async def start_reasoning_session(
        ...
        logging.info(f"Started explainable reasoning session {session_id} for user {user_id}")
    ...
    async def finalize_reasoning_session(self, session_id: str) -> ReasoningGraph:
        ...
        await self.memory.store_memory(
            ...
            tags=[f"user_{graph.context.user_id}", f"type_{graph.context.request_type}"]
        )
        ...
        logging.info(f"Finalized reasoning session {session_id} with confidence {graph.overall_confidence}")
    ...
    async def explain_decision(
        ...
        relevant_alternatives = [
            alt for alt in graph.alternatives
            if any(e.evidence_id in [ev.evidence_id for ev in decision_node.evidence] 
                  for e in alt.evidence)
        ]
        ...
        explanation = {
            ...
            "risk_assessment": {
                ...
                "confidence_level": self._get_confidence_level(decision_node.confidence_score).value,
    ...
    async def generate_reasoning_summary(self, session_id: str) -> str:
        ...
        if graph.decision_nodes:
            ...
            for decision_id in graph.decision_nodes:
                ...
                summary_parts.append(f"- {decision.label} (confidence: {decision.confidence_score:.2f})")
        ...
        if graph.alternatives:
            ...
            for alt in graph.alternatives[:3]:  # Top 3
                summary_parts.append(f"- {alt.description} → Rejected: {alt.rejection_reason}")
            summary_parts.append("")
        
        # Add evidence summary
        total_evidence = sum(len(node.evidence) for node in graph.nodes.values())
        summary_parts.append(f"**Evidence Analyzed**: {total_evidence} pieces of evidence from various sources")
...
async def example_reasoning_session():
    ...
    await explainer.connect_reasoning_steps(session_id, goal_node, decision_node, "leads_to", 0.9)
    await explainer.connect_reasoning_steps(session_id, constraint_node, decision_node, "constrains", 0.8)
    ...
    await explainer.add_considered_alternative(
        ...
        cons=["Encryption key must be managed", "Still visible to developers", "Key rotation complexity"],
        risk_score=0.7,
