...
async def get_featured_extensions():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve featured extensions"
        )
...
async def search_extensions(request: ExtensionSearchRequest):
    ...
    try:
        ...
        for ext in all_extensions:
            if (
                not request.query
                or request.query.lower() in ext["name"].lower()
...
async def get_extension_details(extension_id: str):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to retrieve extension details"
        )
...
async def install_extension(
    request: InstallExtensionRequest, tenant=Depends(require_tenant)
):
    ...
    try:
        ...
        await metrics_collector.record_metric(
            ...
            1.0,
            "count",
            tags={"extension_id": request.extension_id, "org_id": tenant.org_id},
        )
...
async def list_capabilities():
    ...
    try:
        ...
        for capability_name, provider_extensions in capabilities_map.items():
            ...
            capabilities = (
                get_extension_runtime().capability_registry.find_capabilities(
                    capability_name
                )
            )
            if capabilities:
...
async def execute_capability(
    request: ExecuteCapabilityRequest, tenant=Depends(require_tenant)
):
...
async def publish_extension(
    ...
    try:
        ...
        background_tasks.add_task(_mock_extension_review_workflow, manifest.id)
    ...
    try:
        ...
        response = SecurityReportResponse(
            ...
            certificate=(
                ...
                if security_report.certificate
                else None
            ),
        ...
        await metrics_collector.increment(
            ...
            tags={
                ...
                "score": str(security_report.overall_score),
            },
        )
...
async def get_security_report(
    extension_id: str, tenant=Depends(require_tenant), db: Session = Depends(get_db)
):
    """Get security report for an extension"""
    try:
        security_service = create_security_service(db)
        security_report = await security_service.get_security_report(
            extension_id, tenant.id
        )
...
@router.get(
    "/security/certificate/{extension_id}", response_model=ExtensionCertificateResponse
)
async def get_extension_certificate(
    ...
    try:
        security_service = create_security_service(db)
        certificate = await security_service.get_extension_certificate(
            extension_id, tenant.id
        )
...
async def update_security_policies(
    ...
    try:
        security_service = create_security_service(db)
        success = await security_service.update_security_policies(
            tenant.id, request.policies
        )
        ...
        if success:
            ...
            raise HTTPException(
                status_code=500, detail="Failed to update security policies"
            )
...
async def get_security_policies(
    tenant=Depends(require_tenant), db: Session = Depends(get_db)
):
...
async def get_security_dashboard(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to get security dashboard data"
        )
...
async def revoke_certificate(
    ...
    try:
        security_service = create_security_service(db)
        success = await security_service.revoke_certificate(
            certificate_id, tenant.id, reason
        )
...
async def execute_ci_fixer(request: CIFixerRequest, tenant=Depends(require_tenant)):
    ...
    try:
        ...
        # Mock extension context that would be passed to TypeScript extension
        {
            ...
            "navi": {"apiUrl": "http://localhost:8787", "version": "1.0.0"},
        }
