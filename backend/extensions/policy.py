...
class PolicyResult:
    ...
    def __init__(self, action: PolicyAction, reason: str, details: Optional[Dict] = None):
        self.action = action
        self.reason = reason
...
class PolicyEngine:
    ...
    def evaluate_installation(self, manifest: ExtensionManifest) -> PolicyResult:
        ...
        forbidden_perms = set(manifest.permissions) & self.org_policy.forbidden_permissions
        if forbidden_perms:
            return PolicyResult(
                PolicyAction.DENY,
        ...
        requires_approval = (
            set(manifest.permissions) & self.org_policy.requires_approval or
            set(manifest.permissions) & self.immutable_rules["critical_permissions"]
        )
        ...
        if manifest.trust == TrustLevel.ORG_APPROVED and ExtensionPermission.NETWORK_ACCESS in manifest.permissions:
            return PolicyResult(
                PolicyAction.WARN,
    ...
    def evaluate_runtime_permission(
        ...
        if context.get("environment") == "production":
            if permission in {ExtensionPermission.DEPLOY, ExtensionPermission.EXECUTE_COMMANDS}:
                return PolicyResult(
                    PolicyAction.DENY,
    ...
    def isolate_extension_environment(self, extension_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "blocked_modules": [
                ...
                "requests", "httplib", "eval", "exec", "__import__"
            ],
            "max_memory_mb": 256,
