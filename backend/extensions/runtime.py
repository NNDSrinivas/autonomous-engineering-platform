...
class ExtensionManifest:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ExtensionManifest":
        ...
        return cls(
            ...
            extension_type=ExtensionType(data.get("extension_type", ExtensionType.SKILL.value)),
            ...
            trust_level=ExtensionTrust(data.get("trust_level", ExtensionTrust.COMMUNITY.value)),
            created_at=created_at,
            updated_at=updated_at,
            state=ExtensionLifecycleState(data.get("state", ExtensionLifecycleState.DRAFT.value)),
...
class PermissionManager:
    ...
    async def validate_permissions(self, manifest: ExtensionManifest, tenant: TenantContext) -> Dict[str, bool]:
    ...
    async def _check_permission_approval(self, permission: str, extension_id: str, org_id: str) -> bool:
...
class CapabilityRegistry:
    ...
    def __init__(self):
        ...
        self.extension_capabilities: Dict[str, List[str]] = {}  # extension_id -> capability names
    ...
    def register_capability(self, extension_id: str, capability: ExtensionCapability):
        ...
        logger.info(f"Registered capability '{capability.name}' from extension '{extension_id}'")
    ...
    def unregister_extension(self, extension_id: str):
        ...
        for cap_name in capability_names:
            ...
            self.capabilities[cap_name] = [
                cap for cap in self.capabilities[cap_name]
...
class ExtensionSandbox:
    ...
    async def __aenter__(self):
        ...
        self.sandbox_dir = Path(tempfile.mkdtemp(prefix=f"navi_ext_{self.tenant_context.org_id}_"))
        return self
    ...
    async def execute_extension(self, 
        ...
        try:
            ...
            permissions = await permission_manager.validate_permissions(manifest, self.tenant_context)
            
            denied_permissions = [p for p, allowed in permissions.items() if not allowed]
            ...
            result = await self._execute_in_sandbox(manifest, capability_name, exec_context)
...
class ExtensionRuntime:
    ...
    async def install_extension(self, manifest: ExtensionManifest) -> bool:
        ...
        try:
            ...
            # Check permissions
            permission_manager = PermissionManager()
            permissions = await permission_manager.validate_permissions(manifest, tenant)
            ...
            logger.info(f"Extension {manifest.id} installed successfully for org {tenant.org_id}")
            return True
    ...
    async def execute_capability(self, 
                                capability_name: str,
                                inputs: Dict[str, Any],
                                preferred_extension: Optional[str] = None) -> Dict[str, Any]:
        ...
        if preferred_extension:
            chosen_capability = next(
                (cap for cap in capabilities if cap.extension_id == preferred_extension),
                None
            )
    ...
    async def list_available_capabilities(self) -> Dict[str, List[str]]:
        ...
        result = {}
        for capability_name, capabilities in self.capability_registry.capabilities.items():
    ...
    async def get_extension_info(self, extension_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "capabilities": self.capability_registry.get_extension_capabilities(extension_id),
...
class ExtensionRepository(TenantRepository):
    ...
    async def list_extensions(self, 
                             ...
                             extension_type: Optional[ExtensionType] = None) -> List[ExtensionManifest]:
