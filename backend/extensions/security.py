...
try:
    ...
    oid = None
    logging.warning(
        "cryptography package not available - certificate features disabled"
    )
...
class CertificateAuthority:
    ...
    def generate_ca_credentials(
        self, common_name: str = "NAVI Extension CA"
    ) -> Tuple[bytes, bytes]:
    ...
    def sign_extension(
        self, extension_manifest: Dict[str, Any], extension_content: bytes
        ...
        subject_name = Name(
            [
                ...
                NameAttribute(
                    oid.NameOID.ORGANIZATION_NAME, author.get("name", "Unknown")
                ),
...
class VulnerabilityScanner:
    ...
    def __init__(self):
        ...
        self.dangerous_patterns = [
            ...
            # Network access
            (
                r"urllib\.request|requests\.",
                ThreatType.SUSPICIOUS_BEHAVIOR,
                SecurityRisk.LOW,
                "Network requests",
            ),
            (
                r"socket\.|httplib\.|http\.client",
                ThreatType.SUSPICIOUS_BEHAVIOR,
                SecurityRisk.MEDIUM,
    ...
    def scan_extension(
        self, extension_manifest: Dict[str, Any], extension_files: Dict[str, str]
    ...
    def _is_vulnerable_version(
        self, current_version: str, version_spec: str, vulnerable_spec: str
    ) -> bool:
        ...
        try:
            current_parts = [
                int(x) for x in current_version.replace("v", "").split(".")
            ]
            vuln_parts = [
                int(x)
                for x in vulnerable_spec.replace("<", "")
                .replace(">", "")
                .replace("=", "")
                .split(".")
            ]
...
class ExtensionSecurityManager:
    ...
    def assess_extension_security(
        ...
        findings = self.vulnerability_scanner.scan_extension(
            extension_manifest, extension_files
        )

        # Determine security level
        security_level = self._determine_security_level(
            extension_manifest, certificate, findings
        )
        ...
        recommendations = self._generate_recommendations(
            findings, certificate, policy_compliance
        )
    ...
    def _determine_security_level(
        ...
        critical_findings = [
            f for f in findings if f.risk_level == SecurityRisk.CRITICAL
        ]
    ...
    def _check_policy_compliance(
        ...
        compliance["signature_required"] = not self.security_policies[
            "require_signature"
        ] or (certificate is not None)

        # Self-signed certificates
        compliance["self_signed_allowed"] = self.security_policies[
            "allow_self_signed"
        ] or (certificate is None or not certificate.is_self_signed)

        # Security score threshold
        score = self._calculate_security_score(findings, certificate)
        compliance["min_security_score"] = (
            score >= self.security_policies["min_security_score"]
        )
        ...
        compliance["high_risk_permissions"] = (
            not self.security_policies["block_high_risk_permissions"]
            or not has_high_risk
        )
    ...
    def _generate_recommendations(
        ...
        if not certificate:
            recommendations.append(
                "Consider signing the extension with a digital certificate for enhanced trust"
            )
        elif certificate.is_self_signed:
            recommendations.append(
                "Use a certificate from a trusted Certificate Authority instead of self-signed"
            )
        ...
        for policy, compliant in policy_compliance.items():
            if not compliant:
                if policy == "signature_required":
                    recommendations.append(
                        "Add digital signature to meet security policy requirements"
                    )
                elif policy == "min_security_score":
                    recommendations.append(
                        "Improve security score by addressing findings"
                    )
...
def create_security_manager(
    ca_key_path: Optional[str] = None, ca_cert_path: Optional[str] = None
