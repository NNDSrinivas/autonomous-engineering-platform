...
class ExtensionSecurityService:
    ...
    def __init__(self, db_session: Session, ca_key_path: Optional[str] = None, ca_cert_path: Optional[str] = None):
        self.db = db_session
    ...
    async def get_extension_certificate(
        ...
        try:
            cert_model = self.db.query(CertificateModel).filter(
    ...
    async def update_security_policies(
        ...
        try:
            ...
            for policy_name, policy_value in updated_policies.items():
                policy_model = self.db.query(SecurityPolicyModel).filter(
    ...
    async def get_security_policies(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            policy_models = self.db.query(SecurityPolicyModel).filter(
                SecurityPolicyModel.tenant_id == tenant_id
            ).all()
    ...
    async def get_security_dashboard_data(self, tenant_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_reports = self.db.query(SecurityReportModel).filter(
                SecurityReportModel.tenant_id == tenant_id
            ).order_by(SecurityReportModel.created_at.desc()).limit(50).all()
            ...
            avg_security_score = total_score / len(recent_reports) if recent_reports else 0.0
            ...
            cert_count = self.db.query(CertificateModel).filter(
                CertificateModel.tenant_id == tenant_id
            ).count()
            
            revoked_count = self.db.query(CertificateModel).filter(
                CertificateModel.tenant_id == tenant_id,
            ...
            return {
                ...
                "compliance_rate": (
                    sum(1 for r in recent_reports if self._coerce_bool(r.policy_compliant))
                    / len(recent_reports)
                    * 100
                    if recent_reports
                    else 0
    ...
    async def _extract_extension_files(self, extension_content: bytes) -> Dict[str, str]:
            ...
            with tempfile.NamedTemporaryFile(delete=False) as temp_file:
                ...
                with zipfile.ZipFile(temp_file.name, 'r') as zip_file:
                    for file_info in zip_file.filelist:
                        if not file_info.is_dir() and file_info.filename.endswith(('.py', '.js', '.ts', '.json', '.yaml', '.yml')):
                            try:
    ...
    async def _store_security_report(self, report: SecurityReport, tenant_id: str):
        ...
        try:
            ...
            critical_count = len([f for f in report.findings if f.risk_level == SecurityRisk.CRITICAL])
            high_count = len([f for f in report.findings if f.risk_level == SecurityRisk.HIGH])
    ...
    async def _store_certificate(self, certificate: ExtensionCertificate, tenant_id: str):
        """Store certificate in database"""
    ...
    async def _evaluate_approval(self, report: SecurityReport, tenant_id: str) -> bool:
        ...
        try:
            ...
            if policies.get("block_critical_vulnerabilities", True):
                critical_findings = [f for f in report.findings if f.risk_level == SecurityRisk.CRITICAL]
    ...
    def _serialize_security_report(self, report: SecurityReport) -> Dict[str, Any]:
        ...
        report_dict = {
            ...
            "certificate": self._serialize_certificate(report.certificate) if report.certificate else None,
    ...
    def _serialize_certificate(self, certificate: ExtensionCertificate) -> Dict[str, Any]:
    ...
    def _deserialize_security_report(self, report_data: Dict[str, Any]) -> SecurityReport:
    ...
    def _deserialize_certificate(self, cert_data: Dict[str, Any]) -> ExtensionCertificate:
