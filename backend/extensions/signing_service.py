...
class ExtensionSigningService:
    ...
    def load_core_key(self, private_key_b64: str) -> None:
        ...
        self._core_private_key = SigningKey(private_key_b64.encode(), encoder=Base64Encoder)
        
        # Add core public key to trusted set
        public_key = self._core_private_key.verify_key.encode(encoder=Base64Encoder).decode()
    ...
    def load_vendor_key(self, private_key_b64: str) -> None:
        ...
        self._vendor_private_key = SigningKey(private_key_b64.encode(), encoder=Base64Encoder)
        
        # Add vendor public key to trusted set
        public_key = self._vendor_private_key.verify_key.encode(encoder=Base64Encoder).decode()
    ...
    def _sign_manifest(self, manifest: ExtensionManifest, trust_level: TrustLevel) -> Tuple[str, str]:
...
class ExtensionVerificationService:
    ...
    def verify_extension_bundle(self, bundle_bytes: bytes) -> ExtensionBundle:
        ...
        if actual_hash != expected_hash:
            raise ValueError(f"Extension bundle hash mismatch: expected {expected_hash}, got {actual_hash}")
    ...
    def _verify_signature(self, manifest: ExtensionManifest, signature: str, public_key: str) -> None:
        ...
        if public_key not in trusted_keys:
            raise ValueError(f"Untrusted public key for {trust_level}: {public_key[:16]}...")
...
class OrganizationPolicy:
    ...
    def __init__(self):
        ...
        self.requires_approval = {ExtensionPermission.CI_ACCESS, ExtensionPermission.EXECUTE_COMMANDS}
        self.blocked_authors = set()
        self.allowed_trust_levels = {TrustLevel.CORE, TrustLevel.VERIFIED, TrustLevel.ORG_APPROVED}
    ...
    def validate_extension(self, manifest: ExtensionManifest) -> None:
        ...
        if manifest.trust not in self.allowed_trust_levels:
            raise PermissionError(f"Trust level {manifest.trust} not allowed by org policy")
        ...
        if requires_approval:
            logger.warning(f"Extension {manifest.id} requires approval for: {requires_approval}")
