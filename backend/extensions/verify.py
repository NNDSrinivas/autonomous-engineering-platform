...
class ExtensionVerifier:
    ...
    def verify_and_validate_extension(
        ...
        try:
            ...
            logger.info(f"Extension {bundle.manifest.id} passed cryptographic verification")
            
            # Step 2: Policy validation (ENFORCEMENT)
            policy_result = self._validate_against_policy(bundle.manifest, user_id, org_id)
    ...
    def _validate_against_policy(
        ...
        self.audit_logger.log_installation_attempt(manifest, policy_result, user_id, org_id)
    ...
    def _perform_security_checks(self, bundle: ExtensionBundle) -> None:
        ...
        if len(manifest.permissions) > 5:
            logger.warning(f"Extension {manifest.id} requests {len(manifest.permissions)} permissions")
...
class RuntimePermissionChecker:
    ...
    def check_runtime_permission(
        ...
        if extension_id not in self.installed_extensions:
            logger.error(f"Runtime permission check for unknown extension: {extension_id}")
            return False
...
def check_extension_permission(
    ...
    return checker.check_runtime_permission(extension_id, permission, operation_context, user_id)