...
class ContinuousFleetIntelligenceEngine:
    ...
    async def ingest_project_signal(
        ...
        signal_id = f"signal_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{project_id[:8]}"
    ...
    async def update_fleet_intelligence(self) -> FleetIntelligence:
        ...
        active_projects = len([
            consent for consent in self.learning_consents.values()
    ...
    async def _collect_pattern_signals(self, pattern_type: PatternType) -> List[ProjectSignal]:
