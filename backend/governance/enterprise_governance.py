...
class EnterpriseGovernanceFramework:
        ...
        if permission in user.permissions:
            ...
            if policy_check["allowed"]:
                ...
                return {
                    ...
                    "requires_approval": policy_check.get(
                        "can_request_approval", False
                    ),
                }

        # Check if approval workflow exists for this permission
        approval_workflow = await self._find_applicable_workflow(
    ...
    async def request_approval(
        ...
        if not workflow_id:
            workflow = await self._find_workflow_for_request(
                request_type, request_details, user
            )
            if not workflow:
    ...
    async def record_ai_decision(
        ...
        human_review_required: bool = (
            confidence_score < self.governance_config["ai_decision_review_threshold"]
            or (
                impact_assessment is not None
                and impact_assessment.get("risk_level") == "high"
            )
            or decision_type
            in ["security_action", "data_deletion", "user_access_change"]
        )
        ...
        await self._log_audit_event(
            ...
            details={
                ...
                "human_review_required": human_review_required,
                "impact_level": (
                    impact_assessment.get("risk_level", "unknown")
            ...
            "audit_summary": {
                ...
                "user_actions": len(
                    [
                        ...
                        if log_entry.event_type == AuditEventType.USER_ACTION
                    ]
                ),
                "system_actions": len(
                    [
                        log_entry
                        for log_entry in relevant_logs
                        if log_entry.event_type == AuditEventType.SYSTEM_ACTION
                    ]
                ),
                "ai_decisions": len(
                    [
                        log_entry
                        for log_entry in relevant_logs
                        if log_entry.event_type == AuditEventType.AI_DECISION
                    ]
                ),
                "security_events": len(
                    [
                        log_entry
                        for log_entry in relevant_logs
                        if log_entry.event_type == AuditEventType.SECURITY_EVENT
                    ]
                ),
            },
    ...
    def _initialize_default_roles(self):
        ...
        role_permissions = {
            ...
            Role.ARCHITECT: {
                ...
                Permission.EXECUTE_MIGRATIONS,
                Permission.VIEW_AI_DECISIONS,
                Permission.CONFIGURE_AI,
            },
            Role.DEVOPS_ENGINEER: {
                Permission.READ_CODE,
                Permission.DEPLOY_CHANGES,
    ...
    async def _notify_approvers(self, approval_request, approvers):
        ...
        logging.info(
            f"Notifying approvers {approvers} of request {approval_request.request_id}"
        )
    ...
    async def _create_ai_decision_review_request(self, ai_decision):
        ...
        logging.info(
            f"Creating review request for AI decision {ai_decision.decision_id}"
        )
    ...
    async def _calculate_risk_score(
        self, event_type, action, resource, details, user_id
    ):
    ...
    async def _trigger_security_alert(self, audit_entry):
        ...
        logging.warning(
            f"High-risk security event detected: {audit_entry.action} by {audit_entry.user_id}"
        )
