...
class KpiEngine:
    ...
    async def compute_velocity(
        ...
        if sprint_ids:
            ...
            sprint_data = await self._get_team_sprints_since(
                team_id or "default", since_date
            )
        ...
        for sprint in sprint_data:
            ...
            normalized_velocity = completed_points * (
                14 / sprint_duration
        ...
        current_velocity = (
            statistics.mean(velocities[-3:])
            if len(velocities) >= 3
            else statistics.mean(velocities)
        )
        previous_velocity = (
            statistics.mean(velocities[:-3]) if len(velocities) >= 6 else velocities[0]
        )
        ...
        return {
            ...
            "change_percentage": (
                ((current_velocity - previous_velocity) / previous_velocity * 100)
    ...
    async def compute_mttr(
        ...
        if not incident_data:
            ...
            since_date = datetime.now() - timedelta(days=days_back)
            incident_data = await self._get_incidents_since(
                team_id or "default", since_date
            )
        ...
        # Calculate MTTR
        repair_times = [
            incident.get("duration_hours", 0)
            for incident in incident_data
            if incident.get("duration_hours")
        ]
        ...
        historical_mttr = await self._get_historical_metric(
            MetricType.MTTR, team_id, days_back=60
        )
        previous_mttr = (
            historical_mttr[-1]["value"] if historical_mttr else current_mttr
        )
        ...
        return {
            ...
            "change_percentage": (
                ((current_mttr - previous_mttr) / previous_mttr * 100)
                if previous_mttr > 0
                else 0
            ),
            "trend": (
    ...
    async def compute_pr_throughput(
        ...
        review_times = [
            pr.get("review_time_hours", 0)
            for pr in merged_prs
            if pr.get("review_time_hours")
        ]
    ...
    async def generate_team_performance_report(
        ...
        coverage_trend = await self._get_metric_trend(
            MetricType.TEST_COVERAGE, team_id, days_back
        )

        # Get technical debt trend
        tech_debt_trend = await self._get_metric_trend(
            MetricType.TECHNICAL_DEBT, team_id, days_back
        )
    ...
    async def _generate_performance_insights(
        self, team_id: str, metrics_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "recommendations": [
                    {
                        "area": "process",
                        "action": "Continue tracking metrics",
                        "priority": "medium",
                    }
                ],
                "risks": [],
    ...
    def _calculate_overall_score(self, metrics: Dict[str, Any]) -> float:
        ...
        if velocity.get("velocity", 0) > 0:
            velocity_score = min(
                100, (velocity["velocity"] / self.velocity_target) * 100
            )
        ...
        if mttr.get("mttr_hours", 0) > 0:
            mttr_score = max(
                0, 100 - (mttr["mttr_hours"] / self.mttr_target_hours) * 50
            )
        ...
        if pr_throughput.get("prs_per_week", 0) > 0:
            pr_score = min(
                100, (pr_throughput["prs_per_week"] / self.pr_throughput_target) * 100
            )
            scores.append(pr_score)

        return statistics.mean(scores) if scores else 50.0

    def _create_kpi_trend(
    ...
    async def _get_metric_trend(
        self, metric_type: MetricType, team_id: str, days_back: int
    ) -> KPITrend:
    ...
    async def _get_team_sprints_since(
        self, team_id: str, since_date: datetime
    ) -> List[Dict[str, Any]]:
        """Get team sprints since date."""
        return []  # Placeholder

    async def _get_incidents_since(
        self, team_id: str, since_date: datetime
    ) -> List[Dict[str, Any]]:
        """Get incidents since date."""
        return []  # Placeholder

    async def _get_prs_since(
        self, team_id: str, since_date: datetime
    ) -> List[Dict[str, Any]]:
        """Get PRs since date."""
        return []  # Placeholder

    async def _get_bugs_since(
        self, team_id: str, since_date: datetime
    ) -> List[Dict[str, Any]]:
        """Get bugs since date."""
        return []  # Placeholder
