...
async def install_extension(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Extension verification failed: {e}"
        )
...
async def uninstall_extension(
    ...
    logger.info(
        f"Uninstalling extension {extension_id} for user {current_user.user_id}"
    )
...
async def get_extension_details(extension_id: str, db: Session = Depends(get_db)):
...
async def upload_extension(
    ...
    if not file.filename or not file.filename.endswith(".navi-ext"):
        raise HTTPException(
            status_code=400, detail="Invalid file format. Expected .navi-ext file"
        )
