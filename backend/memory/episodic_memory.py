...
class EpisodicMemory:
    ...
    async def record_user_preference(
        self, preference_type: str, preference_value: Any, context: str
    ) -> int:
        """
        ...
        return await self.record_event(
            MemoryEventType.USER_PREFERENCE, content, metadata
        )
    ...
    async def record_success_pattern(
        ...
        return await self.record_event(
            MemoryEventType.SUCCESS_PATTERN, content, metadata
        )
    ...
    async def retrieve_context(
        ...
        self.logger.debug(
            f"Retrieved {len(enhanced_results)} relevant memories for query: {query[:50]}"
        )
        return enhanced_results

    async def get_similar_bug_fixes(
        ...
        return await self.retrieve_context(
            query=error_message, event_types=[MemoryEventType.BUG_FIX], max_items=limit
        )
    ...
    async def get_user_preferences(self, context: str = "") -> List[Dict[str, Any]]:
        ...
        return await self.retrieve_context(
            query=query, event_types=[MemoryEventType.USER_PREFERENCE], max_items=20
        )
