...
class MemoryLayer:
    ...
    async def store_memory(
        ...
        # Generate unique ID
        memory_id = hashlib.md5(f"{title}_{content}_{datetime.now().isoformat()}".encode()).hexdigest()[:16]
        ...
        if similar_memories:
            ...
            return await self._consolidate_memories(similar_memories[0], {
                "title": title,
                "content": content,
    ...
    async def recall_memories(
        ...
        similar_docs = self.vector_store.search(
            ...
            filters={
                "memory_type": [mt.value for mt in query.memory_types] if query.memory_types else None,
    ...
    async def generate_insights(
        ...
        try:
            ...
            return [MemoryInsight(
                ...
                confidence=0.7,
                actionable_recommendations=["Review stored memories for relevant patterns"]
            )]
    ...
    async def consolidate_memories(
        ...
        if len(memories) < 2:
            return {"consolidated": 0, "message": "Insufficient memories for consolidation"}
        ...
        try:
            response = await self.llm.run(prompt=consolidation_prompt, use_smart_auto=True)
            ...
            return {
                ...
                "groups_processed": len(consolidation_plan.get("consolidation_groups", [])),
    ...
    async def cleanup_old_memories(
        ...
        for memory in old_memories:
            ...
            # - Recently accessed
            # - Frequently accessed
            if (memory.importance in [MemoryImportance.CRITICAL, MemoryImportance.HIGH] or
                memory.last_accessed > cutoff_date or
    ...
    async def _enhance_memory_content(
        ...
        try:
            response = await self.llm.run(prompt=enhancement_prompt, use_smart_auto=True)
    ...
    async def _consolidate_memories(
        ...
        existing_memory.content += f"\n\n--- Additional Information ---\n{new_data['content']}"
        existing_memory.tags = list(set(existing_memory.tags + new_data.get('tags', [])))
        existing_memory.related_files = list(set(existing_memory.related_files + new_data.get('related_files', [])))
    ...
    async def _save_memory(self, memory: Memory) -> None:
        ...
        try:
            ...
            await self.db.execute(query, [
                memory.id, memory.memory_type.value, memory.title, memory.content,
                memory.importance.value, json.dumps(memory.tags), json.dumps(memory.related_files),
                json.dumps(memory.related_memories), json.dumps(memory.context, default=str),
                memory.created_at.isoformat(), memory.last_accessed.isoformat(),
                memory.access_count, memory.confidence_score, memory.user_id, memory.team_id
            ])
