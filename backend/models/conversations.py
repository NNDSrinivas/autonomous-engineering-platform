...
class ConversationMessage(Base):
    ...
    replies = relationship("ConversationReply", back_populates="parent", cascade="all, delete-orphan")
...
class ConversationReply(Base):
    ...
    id = Column(BigInteger, primary_key=True, autoincrement=True)
    org_id = Column(String(255), nullable=False, index=True)
    parent_id = Column(BigInteger, ForeignKey("conversation_message.id", ondelete="CASCADE"), nullable=False)
    message_ts = Column(String(255), nullable=True, index=True)
    user = Column(String(255), nullable=True)
    text = Column(Text, nullable=False)
    meta_json = Column(JSONB, default={}, nullable=False)
