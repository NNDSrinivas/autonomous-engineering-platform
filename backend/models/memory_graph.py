...
from sqlalchemy import Column, BigInteger, String, Text, Float, Integer, ForeignKey, TIMESTAMP
...
class MemoryNode(Base):
    ...
    node_type = Column(String(50), nullable=False, index=True)  # jira_issue, slack_msg, pr, code, etc
    ...
    # Relationships
    chunks = relationship("MemoryChunk", back_populates="node", cascade="all, delete-orphan")
...
class MemoryChunk(Base):
    ...
    id = Column(BigInteger, primary_key=True, index=True, autoincrement=True)
    node_id = Column(BigInteger, ForeignKey("memory_node.id", ondelete="CASCADE"), nullable=False)
...
class MemoryEdge(Base):
    ...
    from_id = Column(BigInteger, ForeignKey("memory_node.id", ondelete="CASCADE"), nullable=False, index=True)
    to_id = Column(BigInteger, ForeignKey("memory_node.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    # Relationships
    from_node = relationship("MemoryNode", foreign_keys=[from_id], back_populates="outgoing_edges")
    to_node = relationship("MemoryNode", foreign_keys=[to_id], back_populates="incoming_edges")
