...
class PlanStep(BaseModel):
    ...
    def _create_fallback_step(cls, step_data: Dict, counter: int) -> 'PlanStep':
        ...
        return cls(
            ...
            file_targets=step_data.get('file_targets', '').split(',') if step_data.get('file_targets') else [],
            description=step_data.get('description', ''),
            required_context=step_data.get('required_context', '').split(',') if step_data.get('required_context') else [],
            depends_on=step_data.get('depends_on', '').split(',') if step_data.get('depends_on') else [],
...
class Plan(BaseModel):
    ...
    def get_executable_steps(self) -> List[PlanStep]:
        ...
        """
        executed_step_ids = {step.id for step in self.steps if getattr(step, 'executed', False)}
        ...
        for step in self.steps:
            ...
            deps_satisfied = all(dep_id in executed_step_ids for dep_id in step.depends_on)
            if deps_satisfied:
    ...
    def get_progress(self) -> float:
        ...
        """
        executed_count = sum(1 for step in self.steps if getattr(step, 'executed', False))
...
class MemoryEvent(BaseModel):
    ...
    id: str
    user_id: str
    event_type: str  # "instruction", "execution_result", "error", "feedback", "context_update"
