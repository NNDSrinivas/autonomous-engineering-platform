...
async def explain_repo(workspace_root: Optional[str]) -> str:
    ...
    if any(s.path.match("app/navi/page.*") for s in samples):
        route_summaries.append(
            "- **NAVI page (`app/navi/page.*)`)** – Application page for NAVI-related features and functionality."
        )
    if any(s.path.match("app/aep/page.*") for s in samples):
        route_summaries.append(
            "- **AEP page (`app/aep/page.*)`)** – Application page for AEP-related features and functionality."
        )
    if any(s.path.match("app/pricing/page.*") for s in samples):
        route_summaries.append(
            "- **Pricing (`app/pricing/page.*)`)** – Pricing information and subscription details."
        )
