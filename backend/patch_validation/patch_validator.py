...
class EnvironmentManager:
    ...
    async def rollback_to_snapshot(
        self, patch_id: str, snapshot: SystemSnapshot
    ) -> bool:
...
class ValidationEngine:
    ...
    async def validate_syntax(
        self, env_path: str, files_changed: List[str]
                ...
                if file_path.endswith(".py"):
                    ...
                    if proc.returncode != 0:
                        ...
                        result.error_message = (
                            f"Python syntax error in {file_path}: {proc.stderr}"
                        )
                    ...
                    if shutil.which("node"):
                        ...
                        if proc.returncode != 0:
                            result.status = ValidationStatus.FAILED
                            result.error_message = (
                                f"JS/TS syntax error in {file_path}: {proc.stderr}"
                            )
                            output_lines.append(result.error_message)
                        else:
                            output_lines.append(f"✓ {file_path} syntax OK")
    ...
    async def validate_static_analysis(
        self, env_path: str, files_changed: List[str]
    ) -> ValidationStepResult:
        ...
        try:
            ...
            if analysis_result.get("status") == "success":
                ...
                for file_path, file_results in analysis_result.get(
                    "results", {}
                ).items():
                        ...
                        for issue in analysis.issues:
                            if issue.severity.value in ["error", "critical"]:
                                critical_issues.append(
                                    f"{file_path}:{issue.line_number}: {issue.message}"
                                )

                if critical_issues:
                    result.status = ValidationStatus.FAILED
                    result.error_message = (
                        f"Critical static analysis issues found: {len(critical_issues)}"
                    )
                    result.output = "\n".join(critical_issues[:10])  # Limit output
                else:
                    result.status = ValidationStatus.PASSED
                    result.output = (
                        f"Static analysis passed. {len(files_changed)} files analyzed."
                    )
    ...
    async def validate_linting(
        ...
        try:
            python_files = [f for f in files_changed if f.endswith(".py")]
            js_files = [
                f for f in files_changed if f.endswith((".js", ".ts", ".jsx", ".tsx"))
            ]
    ...
    async def validate_tests(
        self, env_path: str, test_type: str = "unit"
    ) -> ValidationStepResult:
        ...
        step = (
            ValidationStep.UNIT_TESTS
            if test_type == "unit"
            else ValidationStep.INTEGRATION_TESTS
        )

        result = ValidationStepResult(
            step=step, status=ValidationStatus.RUNNING, start_time=start_time
        )
...
class ContinuousPatchValidator:
    ...
    async def validate_patch(
        self, patch: PatchInfo, validation_steps: Optional[List[ValidationStep]] = None
    ) -> ValidationResult:
        """
        ...
        if not validation_steps:
            validation_steps = (
                self.config["required_steps"] + self.config["optional_steps"]
            )
        ...
        try:
            ...
            env_path = await self.environment_manager.create_isolated_environment(
                patch.id
            )
            ...
            # Step 3: Apply patch
            patch_applied = await self.environment_manager.apply_patch(
                patch.id, patch.patch_content
            )
            ...
            failed_steps = [
                r
                for r in validation_result.step_results
            ...
            if failed_steps or error_steps:
                ...
                if failed_steps:
                    ...
                    validation_result.rollback_reason = self._step_to_rollback_reason(
                        failed_step.step
                    )
                else:
                    validation_result.rollback_reason = RollbackReason.UNKNOWN_ERROR
                ...
                if self.config["auto_rollback_on_failure"]:
                    rollback_success = (
                        await self.environment_manager.rollback_to_snapshot(
                            patch.id, snapshot
                        )
                    )
            ...
            await self.episodic_memory.record_event(
                ...
                metadata={
                    "success": validation_result.overall_status
    ...
    async def get_validation_dashboard(self) -> Dict[str, Any]:
        ...
        avg_duration = sum(v.duration_seconds for v in recent_validations) / max(
            len(recent_validations), 1
        )
    ...
    async def _run_validation_pipeline(
        self, env_path: str, patch: PatchInfo, validation_steps: List[ValidationStep]
        ...
        for step in validation_steps:
            ...
            if self.validation_engine.config.get("fail_fast", True):
                failed_required = any(
                    ...
                    and r.step in self.config["required_steps"]
            ...
            if step == ValidationStep.SYNTAX_CHECK:
                result = await self.validation_engine.validate_syntax(
                    env_path, patch.files_changed
                )
            elif step == ValidationStep.STATIC_ANALYSIS:
                result = await self.validation_engine.validate_static_analysis(
                    env_path, patch.files_changed
                )
            elif step == ValidationStep.TYPE_CHECK:
                result = await self.validation_engine.validate_type_check(
                    env_path, patch.files_changed
                )
            elif step == ValidationStep.LINT_CHECK:
                result = await self.validation_engine.validate_linting(
                    env_path, patch.files_changed
                )
                ...
                result = await self.validation_engine.validate_tests(
                    env_path, "integration"
                )
            else:
...
class PatchValidationService:
    ...
    async def validate_patch_content(
        self, patch_content: str, description: str = "Manual patch"
    ) -> Dict[str, Any]:
