...
def check_action(policy: Dict, action: Dict) -> Dict:
    ...
    if action.get("command"):
        ...
        def token_matches_pattern(tok: str, pat: str) -> bool:
            ...
            if os.path.basename(tok) == pat:
                return True
            # Glob pattern match (e.g. 'git*' matches 'git-commit')
            if fnmatch.fnmatch(os.path.basename(tok), pat):
                return True
            return False
        ...
        if denied:
            ...
            if allow_cmds:
                allowed = any(
                    token_matches_pattern(tok, a) for a in allow_cmds for tok in tokens
                )
                if not allowed:
    ...
    if action.get("branch") and any(
        fnmatch.fnmatch(action["branch"], pat) for pat in branches_protected
    ):
...
def check_budget(policy: Dict, phase: str, current_usage: Dict[str, float]) -> Dict:
    """
    ...
    if phase_budgets_str:
        try:
            budgets = (
                json.loads(phase_budgets_str)
                if isinstance(phase_budgets_str, str)
                else phase_budgets_str
            )
        except json.JSONDecodeError:
            budgets = {}
    ...
    if token_limit and current_usage.get("tokens", 0) > token_limit:
        reasons.append(
            f"Token budget exceeded for {phase}: {current_usage['tokens']} > {token_limit}"
        )

    if usd_limit and current_usage.get("usd", 0) > usd_limit:
        reasons.append(
            f"Cost budget exceeded for {phase}: ${current_usage['usd']:.2f} > ${usd_limit:.2f}"
        )
