...
class DraftPRRequest(BaseModel):
    ...
    repo_full_name: str = Field(..., description="GitHub repository in format 'owner/repo'")
    ...
    ticket_key: Optional[str] = Field(None, description="Optional JIRA ticket key for auto-linking")
    dry_run: bool = Field(True, description="If true, return preview without creating PR")
...
class JiraCommentRequest(BaseModel):
    ...
    transition: Optional[str] = Field(None, description="Optional status transition name")
    dry_run: bool = Field(True, description="If true, return preview without posting comment")
...
class JiraCommentResponse(BaseModel):
    ...
    status: str = Field(..., description="Operation status: 'success' or 'partial_success' (errors use HTTPException)")