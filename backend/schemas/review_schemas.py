...
class FixRequest(BaseModel):
    ...
    severity: SeverityLevel = Field(SeverityLevel.INFO, description="Severity of the issue")
...
class FixResponse(BaseModel):
    ...
    status: Literal["success", "failed"] = Field(..., description="Status of the fix operation")
    ...
    metadata: Dict[str, Any] = Field(default_factory=dict, description="Additional metadata about the fix")
...
class ReviewEntry(BaseModel):
    ...
    issues: List[str] = Field(default_factory=list, description="List of issues found in this file")
    severity: SeverityLevel = Field(SeverityLevel.INFO, description="Overall severity for this file")
    suggestions: List[str] = Field(default_factory=list, description="Improvement suggestions")
    fix_id: Optional[str] = Field(None, description="ID for auto-fix if available")
    line_numbers: List[int] = Field(default_factory=list, description="Line numbers with issues")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "severity": "warning",
                "suggestions": ["Add initial value to useState", "Define prop interface"],
                "fix_id": "fix_12345",
                "line_numbers": [16, 25]
            }
        }
...
class ReviewResponse(BaseModel):
    ...
    entries: List[ReviewEntry] = Field(..., description="Individual file review entries")
    overall_severity: SeverityLevel = Field(SeverityLevel.INFO, description="Highest severity across all files")
    total_issues: int = Field(0, description="Total number of issues found")
    fixable_issues: int = Field(0, description="Number of issues that can be auto-fixed")
