...
async def run_auto_fix(fix_id: str) -> Dict[str, Any]:
    ...
    logger.info(f"Successfully generated patch for {fix_id} with {risk_assessment.overall_risk.value} risk level")
...
def _build_workspace_context(primary_file: str, max_files: int = 5) -> str:
    ...
    try:
        ...
        while workspace_root.parent != workspace_root:
            if (workspace_root / ".git").exists() or (workspace_root / "package.json").exists():
                break
...
def _analyze_patch_safety(patch: str, original_content: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "estimated_risk": "low" if safety_score >= 0.7 else "medium" if safety_score >= 0.4 else "high"
    }
