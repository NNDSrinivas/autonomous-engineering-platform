...
class BatchEmbeddingService:
    ...
    async def embed_batch(self, texts: List[str]) -> List[List[float]]:
        ...
        logger.info(f"[EMBED] Cache hit: {len(texts) - len(uncached_texts)}/{len(texts)}, generating {len(uncached_texts)} new embeddings")
    ...
    async def _generate_embeddings_individual(self, texts: List[str]) -> List[List[float]]:
        ...
        for text in texts:
            try:
                ...
                logger.error(f"[EMBED] Individual embedding failed for text: {text[:50]}... Error: {e}")
