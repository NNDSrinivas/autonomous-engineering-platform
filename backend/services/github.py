...
class GitHubService:
    ...
    def save_connection(db: Session, access_token: str) -> GhConnection:
        conn = GhConnection(id=GitHubService._id(), access_token=access_token, scopes=["repo","read:org"])
    ...
    def upsert_repo(db: Session, conn_id: str, repo: dict):
        ...
        if row:
            ...
            row = GhRepo(id=GitHubService._id(), **payload); db.add(row)
        db.commit(); return row

    @staticmethod
    def upsert_file(db: Session, repo_id: str, path: str, sha: str | None, lang: str | None, size: int | None):
        row = GhFile(id=GitHubService._id(), repo_id=repo_id, path=path, sha=sha, lang=lang, size_bytes=size, updated=dt.datetime.now(dt.timezone.utc))
    ...
    def upsert_issuepr(db: Session, repo_id: str, number: int, type_: str, title: str, body: str | None, state: str, author: str | None, url: str, updated: dt.datetime | None):
        ...
        row = GhIssuePr(id=GitHubService._id(), repo_id=repo_id, number=number, type=type_, title=title, body=safe_body, state=state, author=author, url=url, updated=updated)
        db.add(row); db.commit()
