...
def __getattr__(name):
    ...
    if name in ("EmbeddingService", "get_embedding_service"):
        from backend.services.memory.embedding_service import (
            EmbeddingService,
            get_embedding_service,
        )
        ...
        from backend.services.memory.user_memory import (
            UserMemoryService,
            get_user_memory_service,
        )
        ...
        return (
            UserMemoryService
            if name == "UserMemoryService"
            else get_user_memory_service
        )
    elif name in ("OrgMemoryService", "get_org_memory_service"):
        from backend.services.memory.org_memory import (
            OrgMemoryService,
            get_org_memory_service,
        )

        return (
            OrgMemoryService if name == "OrgMemoryService" else get_org_memory_service
        )
    elif name in ("ConversationMemoryService", "get_conversation_memory_service"):
        from backend.services.memory.conversation_memory import (
            ConversationMemoryService,
            get_conversation_memory_service,
        )
        ...
        return (
            ConversationMemoryService
            if name == "ConversationMemoryService"
            else get_conversation_memory_service
        )
    elif name in ("CodebaseMemoryService", "get_codebase_memory_service"):
        from backend.services.memory.codebase_memory import (
            CodebaseMemoryService,
            get_codebase_memory_service,
        )
        ...
        return (
            CodebaseMemoryService
            if name == "CodebaseMemoryService"
            else get_codebase_memory_service
        )
        ...
        return (
            ...
            else get_semantic_search_service
        )
