...
class MemoryGraphService:
    ...
    async def add_node(
        ...
        try:
            ...
            logger.info(
                f"Created memory node: id={node.id}, type={node_type}, title={title}"
            )
            ...
            self.db.commit()
            logger.info(
                f"Successfully created node {node.id} with {len(chunks)} chunks"
            )
    ...
    def get_node(self, node_id: int) -> Optional[MemoryNode]:
        ...
        return (
            ...
            .filter(MemoryNode.id == node_id, MemoryNode.org_id == self.org_id)
            .first()
        )

    def get_related_nodes(
        self, node_id: int, edge_types: Optional[List[str]] = None, depth: int = 1
        ...
        try:
            ...
            query = self.db.query(MemoryEdge).filter(
                MemoryEdge.from_id == node_id, MemoryEdge.org_id == self.org_id
            )
            ...
            # Get the nodes
            nodes = (
                self.db.query(MemoryNode).filter(MemoryNode.id.in_(related_ids)).all()
                if related_ids
                else []
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            node_count = (
                self.db.query(MemoryNode)
                .filter(MemoryNode.org_id == self.org_id)
                .count()
            )

            edge_count = (
                self.db.query(MemoryEdge)
                .filter(MemoryEdge.org_id == self.org_id)
                .count()
            )
