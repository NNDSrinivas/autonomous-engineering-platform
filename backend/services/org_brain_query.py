...
Please provide a comprehensive answer based on the context above."""
    ...
    def _build_context(self, nodes: List[Dict[str, Any]], edges: List[Dict[str, Any]]) -> str:
        """
...
Provide a concise 2-3 sentence summary."""
    ...
    async def find_connections(self, node_id_a: int, node_id_b: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "node_a": {"id": node_a.id, "title": node_a.title, "type": node_a.node_type},
                "node_b": {"id": node_b.id, "title": node_b.title, "type": node_b.node_type},
                "common_nodes": len(common_ids),
                "connection_strength": len(common_ids) / max(len(ids_a), len(ids_b)) if ids_a or ids_b else 0
            }
