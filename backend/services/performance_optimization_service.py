...
class PerformanceOptimizationService:
    ...
    async def scan_codebase_streaming(
        ...
        logger.info(
            f"Starting streaming scan of {total_files} files in {workspace_path}"
        )
        ...
        async def process_file_batch(
            ...
            results = await asyncio.gather(
                *[process_single_file(f) for f in file_batch], return_exceptions=True
            )
            # Filter out exceptions and return only successful results
            return [
                result
                for result in results
                if isinstance(result, FileMetrics) or result is None
            ]
        ...
        for i in range(0, len(prioritized_files), batch_size):
            ...
            for j, result in enumerate(results):
                ...
                estimated_remaining_ms = (
                    (remaining_files / throughput * 1000) if throughput > 0 else 0
                )
        ...
        logger.info(
            f"Completed streaming scan: {processed} processed, {skipped} skipped in {elapsed_ms:.2f}ms"
        )

    async def get_high_priority_files(
        self, workspace_path: str, limit: int = 20
    ) -> List[FileMetrics]:
        """
        ...
        async for file_metrics, _ in self.scan_codebase_streaming(
            workspace_path, max_files=limit * 3
        ):
    ...
    def optimize_context_window(
                ...
                if file_metrics.priority_score > 0.8 and len(selected_files) < 3:
                    ...
                    if max_lines > 10:  # Minimum useful content
                        ...
                        partial_metrics = FileMetrics(
                            ...
                            priority_score=file_metrics.priority_score
                            * 0.7,  # Reduced for partial
        ...
        logger.info(
            f"Context optimization: selected {len(selected_files)} files using ~{total_tokens} tokens"
        )
    ...
    async def _analyze_file_performance(self, file_path: str) -> Optional[FileMetrics]:
        ...
        try:
            ...
            priority = self._calculate_priority_score(
                file_path, stat.st_size, complexity
            )
    ...
    def _discover_files(
        self, workspace_path: str, file_extensions: Optional[List[str]] = None
    ) -> List[str]:
    ...
    def _calculate_priority_score(
        self, file_path: str, size_bytes: int, complexity: float
    ) -> float:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        if self.file_cache:
            avg_analysis_time = (
                sum(f.analysis_time_ms for f in self.file_cache.values()) / cache_size
            )
