...
DEFAULT_DB_PATH = os.environ.get(
    "NAVI_PLANS_DB", os.path.expanduser("~/.navi/plans.db")
)
...
def init_database(db_path: Optional[str] = None) -> None:
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        # Create indexes
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_plans_workspace ON plans(workspace_path)"
        )
        ...
        cursor.execute(
            "CREATE INDEX IF NOT EXISTS idx_logs_plan ON execution_logs(plan_id)"
        )
...
class PlanStore:
    ...
    @classmethod
    def load_plan(
        cls, plan_id: str, db_path: Optional[str] = None
        ...
        with get_connection(db_path) as conn:
            ...
            plan = {
                ...
                "detected_technologies": json.loads(
                    row["detected_technologies"] or "[]"
                ),
            ...
            # Load images
            cursor.execute("SELECT * FROM plan_images WHERE plan_id = ?", (plan_id,))
            plan["images"] = [
                {
                    "id": img["id"],
            ...
            plan["unanswered_questions"] = len(
                [q for q in plan["questions"] if not q["answered"]]
            )
            plan["completed_tasks"] = len(
...
class CheckpointManager:
    ...
    def restore_from_checkpoint(
        ...
        with get_connection(db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT state FROM plan_checkpoints WHERE id = ?", (checkpoint_id,)
            )
