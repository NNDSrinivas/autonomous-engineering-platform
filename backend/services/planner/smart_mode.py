...
CRITICAL_FILES = {
    "auth", "login", "security", "jwt", "session", "password", "token",
    "db", "database", "schema", "migrations", "model", "entity",
    "middleware", "api", "server", "payments", "billing", "checkout",
    "config", "settings", "env", "docker", "kubernetes", "deploy"
}

AUTOSAFE_FILE_TYPES = {".md", ".json", ".yml", ".yaml", ".css", ".scss", ".txt", ".gitignore"}

HIGH_RISK_PATTERNS = [
    r"DROP\s+TABLE", r"DELETE\s+FROM", r"TRUNCATE",
    r"rm\s+-rf", r"sudo", r"chmod\s+777",
    r"eval\s*\(", r"exec\s*\(", r"__import__",
    r"process\.env", r"os\.system", r"shell_exec"
]

FRAMEWORK_CRITICAL_FILES = {
    # React/Next.js
    "package.json", "next.config.js", "webpack.config.js",
    # Python/Django
...
class SmartModePlanner:
    ...
    def _assess_file_risk(self, changed_files: List[str]) -> dict:
        ...
        for file_path in changed_files:
            ...
            if any(prod in filepath_lower for prod in ["prod", "production", "deploy", "release"]):
                score += 0.2
                reasons.append(f"Production file: {file_path}")
            
            # Test files (generally safer)
            if any(test in filepath_lower for test in ["test", "spec", "__tests__", "testing"]):
                score -= 0.05
    ...
    def _assess_content_risk(self, diff_content: str) -> dict:
        ...
        sensitive_patterns = [
            r"DROP", r"DELETE", r"REMOVE", r"TRUNCATE",
    ...
    def should_auto_apply(self, risk: RiskAssessment) -> bool:
        ...
        return (
            risk.recommended_mode == "auto" and 
            risk.confidence > 0.8 and 
            risk.score < 0.3
        )
