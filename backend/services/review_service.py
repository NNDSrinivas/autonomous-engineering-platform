...
class RealReviewService:
    ...
    def __init__(
        ...
        logger.info(
            f"[REAL REVIEW] Initialized for {self.repo_path} with depth={analysis_depth}"
        )
            ...
            for line in result.stdout.strip().split("\n"):
                ...
                if status.strip() in ["M", "A", "D", "MM", "AM", "??"]:
                    change_data = {
                        ...
                        "content": (
                            self._get_file_content(filepath) if status != "D" else ""
                        ),
                    }
    ...
    async def analyze_file_change(self, change: Dict[str, Any]) -> ReviewEntry:
        ...
        return ReviewEntry(
            ...
            summary=(
                f"Found {len(issues)} issues in {filepath}"
                if issues
                else f"No issues found in {filepath}"
            ),
            status=change.get("status"),
        )

    def analyze_file_change_comprehensive(
    ...
    async def _analyze_with_llm(
        self, file_path: str, diff: str, content: str
    ) -> List[ReviewIssue]:
...
async def analyze_hunk_with_ai(
    file_path: str, hunk: str, context: str = ""
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if review_entry.get("title") and review_entry["title"] != "No Issues Found":
            try:
                ...
                issue_desc = review_entry.get(
                    "body", review_entry.get("title", "Code issue found")
                )
...
async def generate_review_stream(
    ...
    try:
        ...
        yield {
            "type": "live-progress",
            "data": "🔍 Scanning repository intelligently...",
        }
        ...
        if not full_diff.strip() and untracked_files:
            yield {
                "type": "live-progress",
                "data": f"📄 No git changes found, analyzing {len(untracked_files)} untracked files...",
            }
            ...
            for untracked_file in untracked_files:
                try:
                    ...
                    if (
                        ...
                        and os.path.getsize(file_path) < 100000
                    ):  # < 100KB
                        with open(
                    ...
                    logger.warning(
                        f"Could not read untracked file {untracked_file}: {e}"
                    )
        ...
        if not full_diff.strip():
            yield {
                "type": "live-progress",
                "data": "💡 No changes detected. Scanning all code files for issues...",
            }
            await asyncio.sleep(0.3)
            ...
            if code_files:
                yield {
                    "type": "live-progress",
                    "data": f"🔎 Scanning {len(code_files)} code files for potential issues...",
                }
                ...
                for file_path in code_files[:max_scan_files]:  # Scan up to max
                    try:
                        with open(
                            ...
                            if (
                                "import pdb" in content
                                or "breakpoint()" in content
                                or "debugger;" in content
                            ):
                                issues.append("Contains debug statements")
                            if "any" in content.lower() and file_path.endswith(
                                (".py", ".ts", ".tsx")
                            ):
                ...
                yield {
                    "type": "done",
                    "data": f"Quick scan complete - analyzed {files_scanned} files, found {issues_found} potential issues",
                }
                return
            else:
                yield {
                ...
                yield {
                    "type": "done",
                    "data": "Repository analysis complete - no issues found",
                }
                return
        ...
        yield {
            "type": "live-progress",
            "data": f"Analyzing {total_hunks} file changes with AI…",
        }
        await asyncio.sleep(0.3)
        ...
        for batch_start in range(0, total_hunks, BATCH_SIZE):
            ...
            progress_pct = (
                int((completed / total_hunks) * 100) if total_hunks > 0 else 0
            )
            yield {
                "type": "live-progress",
                "data": f"Processing files {batch_start + 1}-{batch_end} of {total_hunks} ({progress_pct}%)",
            }
