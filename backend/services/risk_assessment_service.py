...
class RiskAssessmentService:
    ...
    def __init__(self):
        self.critical_patterns = {
            ...
            'auth_changes': [
                r'password', r'token', r'secret', r'auth', r'login', r'session',
                r'jwt', r'oauth', r'permissions', r'roles', r'security'
            ],
            'database_operations': [
                r'DELETE\s+FROM', r'DROP\s+TABLE', r'ALTER\s+TABLE', r'CREATE\s+INDEX',
                r'TRUNCATE', r'\.execute\(', r'\.query\(', r'raw_sql'
            ],
            'system_calls': [
                r'os\.system', r'subprocess', r'exec', r'eval', r'__import__',
                r'open\(.*[\'"]w', r'file\.write', r'fs\.write'
            ],
            'network_operations': [
                r'requests\.', r'urllib', r'socket', r'http\.client',
                r'fetch\(', r'axios\.'
            ],
            'core_infrastructure': [
                r'config', r'settings', r'environment', r'deployment',
                r'docker', r'kubernetes', r'nginx', r'apache'
            ]
        }
        
        self.medium_risk_patterns = {
            'api_changes': [
                r'@app\.route', r'@api\.', r'endpoint', r'handler',
                r'def\s+\w+_api', r'class\s+\w+API'
            ],
            'data_structures': [
                r'class\s+\w+', r'def\s+__init__', r'@dataclass',
                r'schema', r'model', r'entity'
            ],
            'business_logic': [
                r'calculate', r'process', r'validate', r'transform',
                r'algorithm', r'logic', r'rule'
            ]
        }
        ...
        self.low_risk_patterns = {
            'formatting': [
                r'^\s*#', r'"""', r"'''", r'//\s*', r'/\*', r'\*/',
                r'console\.log', r'print\(', r'logger\.'
            ],
            'simple_changes': [
                r'variable\s*=', r'const\s+\w+', r'let\s+\w+',
                r'import\s+', r'from\s+.*\s+import'
            ]
        }
    ...
    def assess_patch(self, patch_content: str, file_paths: List[str]) -> RiskAssessment:
        ...
        try:
            ...
            confidence_score = self._calculate_confidence(factors, len(added_lines) + len(removed_lines))
            
            # Generate impact analysis
            impact_analysis = self._generate_impact_analysis(factors, file_paths, len(added_lines))
            
            # Generate recommendations
            recommendations = self._generate_recommendations(factors, overall_risk)
            
            # Determine review requirements
            requires_review = overall_risk in [RiskLevel.HIGH, RiskLevel.CRITICAL] or confidence_score < 0.7
            safe_to_auto_apply = overall_risk == RiskLevel.MINIMAL and confidence_score > 0.9
            ...
            return RiskAssessment(
                ...
                risk_score=0.8,
                factors=[RiskFactor("analysis_error", 0.8, f"Assessment failed: {str(e)}", "system", [])],
    ...
    def _analyze_file_paths(self, file_paths: List[str]) -> List[RiskFactor]:
        ...
        for file_path in file_paths:
            ...
            # Critical files
            if any(critical in path_lower for critical in [
                'config', 'settings', '.env', 'security', 'auth', 'password',
                'docker', 'deploy', 'infra', 'migration', 'schema'
            ]):
            ...
            # Core system files
            elif any(core in path_lower for core in [
                'main', 'app', 'server', 'api', 'core', 'base', 'models'
            ]):
                factors.append(RiskFactor(
    ...
    def _analyze_code_changes(self, added_lines: List[str], removed_lines: List[str]) -> List[RiskFactor]:
    ...
    def _analyze_change_magnitude(self, added_lines: List[str], removed_lines: List[str]) -> List[RiskFactor]:
        ...
        if total_changes > 100:
            factors.append(RiskFactor(
                ...
                category="magnitude",
                examples=[f"{len(added_lines)} additions, {len(removed_lines)} deletions"]
            ))
        ...
        if len(removed_lines) > 0:
            ...
            if deletion_ratio > 0.5:
                factors.append(RiskFactor(
                    ...
                    category="magnitude",
                    examples=[f"{len(removed_lines)} deletions out of {total_changes} total changes"]
                ))
    ...
    def _analyze_dependencies(self, patch_content: str) -> List[RiskFactor]:
        ...
        import_changes = re.findall(r'^[+-].*(?:import|require|from)\s+[\w\.\-]+', patch_content, re.MULTILINE)
    ...
    def _calculate_confidence(self, factors: List[RiskFactor], total_lines: int) -> float:
    ...
    def _generate_impact_analysis(self, factors: List[RiskFactor], file_paths: List[str], additions: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "highest_risk_factor": max(factors, key=lambda f: f.score).factor if factors else None,
    ...
    def _generate_recommendations(self, factors: List[RiskFactor], risk_level: RiskLevel) -> List[str]:
        ...
        for factor in factors:
            if "security" in factor.category or "auth" in factor.factor:
                recommendations.append("🔒 Security review required for authentication/authorization changes")
            elif "database" in factor.factor:
                recommendations.append("💾 Database backup recommended before schema changes")
