...
class ChangeDetector:
    ...
    def __init__(self):
        ...
        self.function_hashes: Dict[str, Dict[str, str]] = {}  # file_path -> function_name -> hash
        self.class_hashes: Dict[str, Dict[str, str]] = {}    # file_path -> class_name -> hash
    
    def detect_file_changes(self, file_path: str, new_content: str, 
                           old_content: Optional[str] = None) -> List[CodeChange]:
                ...
                if line.startswith('@@'):
                    ...
                    if match:
                        ...
                        changes.append(CodeChange(
                            ...
                            old_content='\n'.join(old_lines[old_start-1:old_start+old_count-1]) if old_count > 0 else None,
                            new_content='\n'.join(new_lines[new_start-1:new_start+new_count-1]) if new_count > 0 else None
                        ))
        
        return changes
    
    def detect_function_changes(self, file_path: str, new_content: str, 
                              old_content: Optional[str] = None) -> List[CodeChange]:
        ...
        try:
            ...
            new_functions = {node.name: node for node in ast.walk(new_tree) if isinstance(node, ast.FunctionDef)}
            
            old_functions = {}
            if old_content:
                try:
                    old_tree = ast.parse(old_content)
                    old_functions = {node.name: node for node in ast.walk(old_tree) if isinstance(node, ast.FunctionDef)}
                except Exception:
                    pass
            ...
            for func_name, func_node in new_functions.items():
                ...
                if func_name not in old_functions:
                    ...
                    changes.append(CodeChange(
                        ...
                        new_content=self._extract_function_content(func_node, new_content)
                    ))
                else:
                    # Check if function changed
                    old_hash = CodeHasher.hash_function(old_functions[func_name], old_content or '')
                    ...
                    if new_hash != old_hash:
                        changes.append(CodeChange(
                            ...
                            line_end=func_node.end_lineno or func_node.lineno,
                            function_name=func_name,
                            old_content=self._extract_function_content(old_functions[func_name], old_content or ''),
                            new_content=self._extract_function_content(func_node, new_content)
                        ))
            ...
            for func_name in old_functions:
                if func_name not in new_functions:
                    ...
                    changes.append(CodeChange(
                        ...
                        function_name=func_name,
                        old_content=self._extract_function_content(old_node, old_content or '')
                    ))
    ...
    def _extract_function_content(self, func_node: ast.FunctionDef, source_code: str) -> str:
...
class PythonAnalyzer:
    ...
    async def analyze_security(self, code: str, file_path: str) -> List[AnalysisIssue]:
        ...
        security_patterns = [
            ...
            (r"subprocess\.call\s*\([^)]*shell=True", "Shell injection risk with shell=True", "security_shell"),
            (r"pickle\.loads?\s*\(", "Pickle deserialization can execute arbitrary code", "security_pickle"),
            (r"yaml\.load\s*\(", "YAML load() can execute arbitrary code", "security_yaml"),
            (r"os\.system\s*\(", "os.system() is vulnerable to shell injection", "security_os_system")
        ]
...
class JavaScriptAnalyzer:
    ...
    async def analyze_security(self, code: str, file_path: str) -> List[AnalysisIssue]:
        ...
        security_patterns = [
            (r"eval\s*\(", "Use of eval() can lead to code injection", "security_eval"),
            (r"innerHTML\s*=", "innerHTML assignment can lead to XSS", "security_innerhtml"),
            (r"document\.write\s*\(", "document.write can lead to XSS", "security_document_write"),
            (r"dangerouslySetInnerHTML", "Dangerous HTML injection risk", "security_dangerous_html"),
            (r"window\.location\s*=", "Unvalidated redirect vulnerability", "security_redirect")
        ]
...
class IncrementalStaticAnalyzer:
    ...
    def __init__(self, workspace_path: str):
        ...
        # Cache management
        self.analysis_cache: Dict[str, Dict[str, AnalysisResult]] = {}  # file_hash -> analysis_type -> result
        self.dependency_graph: Dict[str, Set[str]] = {}  # file_path -> dependent_files
        self.file_signatures: Dict[str, Dict[str, str]] = {}  # file_path -> function_name -> signature_hash
    ...
    async def analyze_changes(self, changed_files: List[str], 
                            analysis_types: Optional[List[AnalysisType]] = None) -> Dict[str, List[AnalysisResult]]:
        """
        ...
        if not analysis_types:
            analysis_types = [AnalysisType.SYNTAX, AnalysisType.SECURITY, AnalysisType.QUALITY]
        ...
        try:
            ...
            for file_path in analysis_targets:
                ...
                for analysis_type in analysis_types:
                    ...
                    result = await self._analyze_file(file_path, analysis_type, detailed_changes.get(file_path, []))
    ...
    async def analyze_function(self, file_path: str, function_name: str, 
            ...
            if file_path.endswith('.py'):
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.FunctionDef) and node.name == function_name:
                        function_content = self.change_detector._extract_function_content(node, content)
                        ...
                        results = []
                        for analysis_type in analysis_types:
                            result = await self._analyze_code_region(function_content, file_path, analysis_type, [change])
    ...
    async def get_analysis_summary(self, file_paths: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "issues_by_type": {analysis_type.value: 0 for analysis_type in AnalysisType},
    ...
    async def _detect_detailed_changes(self, changed_files: List[str]) -> Dict[str, List[CodeChange]]:
        ...
        for file_path in changed_files:
            try:
                ...
                if file_path.endswith('.py'):
                    changes = self.change_detector.detect_function_changes(file_path, new_content, old_content)
                else:
                    changes = self.change_detector.detect_file_changes(file_path, new_content, old_content)
    ...
    async def _compute_analysis_targets(self, detailed_changes: Dict[str, List[CodeChange]]) -> Set[str]:
    ...
    async def _analyze_file(self, file_path: str, analysis_type: AnalysisType, 
                           changes: List[CodeChange]) -> AnalysisResult:
        ...
        try:
            ...
            if changes and analysis_type != AnalysisType.DEPENDENCIES:
                return await self._analyze_changed_regions(content, file_path, analysis_type, changes)
            else:
    ...
    async def _analyze_changed_regions(self, content: str, file_path: str, 
                                     analysis_type: AnalysisType, changes: List[CodeChange]) -> AnalysisResult:
        ...
        for change in changes:
            if change.new_content:
                region_issues = await self._analyze_code_region(change.new_content, file_path, analysis_type, [change])
    ...
    async def _analyze_full_file(self, content: str, file_path: str, analysis_type: AnalysisType) -> AnalysisResult:
        """Analyze the entire file."""
        
        return await self._analyze_code_region(content, file_path, analysis_type, [])
    
    async def _analyze_code_region(self, code: str, file_path: str, analysis_type: AnalysisType, 
                                  changes: List[CodeChange]) -> AnalysisResult:
        ...
        if file_path.endswith('.py'):
            ...
            if analysis_type == AnalysisType.SYNTAX:
                ...
                issues = await self.javascript_analyzer.analyze_security(code, file_path)
        
        # Quality analysis using LLM for any language
        if analysis_type == AnalysisType.QUALITY and self.config["enable_quality_analysis"]:
            quality_issues = await self._analyze_quality_with_llm(code, file_path, changes)
    ...
    async def _analyze_quality_with_llm(self, code: str, file_path: str, 
                                      changes: List[CodeChange]) -> List[AnalysisIssue]:
        ...
        try:
            ...
            issues_data = json.loads(getattr(response, 'response', '[]') if hasattr(response, 'response') else '[]')
    ...
    async def _update_caches(self, detailed_changes: Dict[str, List[CodeChange]], 
            ...
            for result in file_results:
                try:
                    ...
                    if result.dependencies_checked:
                        self.dependency_graph[file_path] = set(result.dependencies_checked)
    ...
    async def _cleanup_cache(self):
        ...
        if len(self.analysis_cache) > self.config["max_cache_size"]:
            ...
            files_to_remove = list(self.analysis_cache.keys())[:-self.config["max_cache_size"]//2]
            
            for file_path in files_to_remove:
                del self.analysis_cache[file_path]
    
    def _calculate_cache_hit_rate(self, results: Dict[str, List[AnalysisResult]]) -> float:
        """Calculate cache hit rate for this analysis session."""
        
        total_analyses = sum(len(file_results) for file_results in results.values())
        cache_hits = sum(sum(1 for result in file_results if result.cache_hit) 
...
class IncrementalAnalysisService:
    ...
    async def analyze_git_changes(self, commit_range: str = "HEAD~1..HEAD") -> Dict[str, Any]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                supported_files = [f for f in changed_files 
                ...
                return {
                    ...
                    "summary": await self.analyzer.get_analysis_summary(supported_files)
                }
            else:
                return {
                    "status": "error",
    ...
    async def analyze_function_changes(self, file_path: str, function_name: str) -> Dict[str, Any]:
