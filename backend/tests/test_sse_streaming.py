...
class TestSSEStreaming:
    ...
    def test_smart_review_stream_events(self, client):
        ...
        with patch(
            "backend.services.planner.smart_mode.SmartModePlanner"
        ) as mock_planner:
            ...
            with client.stream(
                "POST", "/api/smart/review/stream", json=payload
            ) as response:
                assert response.status_code == 200
        ...
        def mock_review_with_progress(*args, **kwargs):
            ...
            return [
                Mock(
                    ...
                    issues=[
                        Mock(
                            type="console_log", line=5, description="Remove console.log"
                        )
                    ],
                )
            ]
...
class TestSSEEventFormats:
    ...
    def test_complex_event_data(self):
        ...
        complex_data = {
            ...
            "metadata": {"timestamp": "2025-12-17T10:30:00Z", "version": "1.0.0"},
        }
