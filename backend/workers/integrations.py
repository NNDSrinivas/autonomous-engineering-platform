...
from ..models.integrations import JiraConnection, JiraProjectConfig, JiraIssue, GhConnection, GhRepo
            ...
            async with httpx.AsyncClient(timeout=30) as client:
                for key in cfg.project_keys:
                    ...
                    while True:
                        ...
                        r = await client.get(f"{conn.cloud_base_url}/rest/api/3/search",
...
def github_index(connection_id: str, repo_full_name: str):
    ...
    try:
        ...
        async def fetch():
            async with httpx.AsyncClient(timeout=30) as client:
                # Repo meta
                r = await client.get(f"https://api.github.com/repos/{repo_full_name}", headers=headers); r.raise_for_status()
                repo = r.json(); repo_row = ghsvc.upsert_repo(db, conn.id, repo)
                ...
                # Issues (last 90d)
                r = await client.get(f"https://api.github.com/repos/{repo_full_name}/issues", headers=headers, params={"state":"all","per_page":100})
                if r.status_code == 200:
                    for it in r.json():
                        typ = "pr" if "pull_request" in it else "issue"
                        upd = it.get("updated_at"); upd = dt.datetime.fromisoformat(upd.replace("Z","+00:00")) if upd else None
                        ghsvc.upsert_issuepr(db, repo_row.id, it["number"], typ, it.get("title",""), it.get("body",""), it.get("state","open"), (it.get("user") or {}).get("login"), it.get("html_url",""), upd)
        import anyio; anyio.run(fetch)
        db.commit()
    except Exception:
        pass
