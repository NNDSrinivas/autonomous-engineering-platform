            ...
            for attempt in range(self.retry_count + 1):
                try:
                    ...
                    if attempt == self.retry_count:
                        ...
                        self.logger.warning(f"{error_msg}, retrying ({attempt + 1}/{self.retry_count})")
...
class PipelineWorkflow:
    ...
    async def execute(self, 
        ...
        self.context = PipelineContext(
            ...
            metadata={'execution_mode': mode.value, 'started_at': execution_start.isoformat()}
        )
        ...
        try:
            ...
            execution_result['execution_time_seconds'] = (datetime.utcnow() - execution_start).total_seconds()
            
            # Record in memory
            await self._record_execution_in_memory(execution_result)
        
        self.logger.info(f"Pipeline execution completed: {self.pipeline_id}, Success: {execution_result['success']}")
    ...
    async def _execute_sequential(self) -> Dict[str, StepResult]:
        ...
        for step_id in execution_order:
            ...
            if hasattr(result, 'status') and result.status == StepStatus.FAILED and self.config.get('fail_fast', False):
    ...
    async def _execute_parallel(self) -> Dict[str, StepResult]:
        ...
        while len(executed) < len(self.steps):
            ...
            for step, result in zip(ready_steps, step_results):
                ...
                # Check fail fast
                if step_result.status == StepStatus.FAILED and self.config.get('fail_fast', False):
                    return results
    ...
    async def _record_execution_in_memory(self, execution_result: Dict[str, Any]):
        ...
        try:
            ...
            failed_steps = [
                step_id for step_id, result in execution_result['step_results'].items()
...
class PipelineBuilder:
    ...
    def add_step(self, 
                ...
                condition: Optional[Callable[[PipelineContext], bool]] = None,
                retry_count: int = 0,
                timeout_seconds: int = 300) -> 'PipelineBuilder':
        """
    ...
    def add_parallel_group(self, 
                          ...
                          group_id_prefix: str = "parallel") -> 'PipelineBuilder':
        """
    ...
    def build(self, 
             memory: Optional[EpisodicMemory] = None,
             execution_mode: ExecutionMode = ExecutionMode.SEQUENTIAL) -> PipelineWorkflow:
        """
...
class PipelineTemplates:
    ...
    def create_security_audit_pipeline(workspace_root: str) -> PipelineWorkflow:
        ...
        async def generate_report(context: PipelineContext):
            ...
            report = {
                'secrets_found': secrets_result.get('secrets_found', 0) if isinstance(secrets_result, dict) else 0,
                'vulnerabilities': len(deps_result.get('vulnerabilities', [])) if isinstance(deps_result, dict) else 0,
                'timestamp': datetime.utcnow().isoformat()
            }
            
            return report
        
        builder = PipelineBuilder("security_audit", "Security Audit Pipeline", workspace_root)
        ...
        builder.add_step("generate_report", "Generate Report", generate_report, 
    ...
    def create_code_quality_pipeline(workspace_root: str) -> PipelineWorkflow:
        ...
        async def generate_quality_report(context: PipelineContext):
            ...
            return {
                'quality_score': 85,
                'lint_errors': lint_result.get('lint_errors', 0) if isinstance(lint_result, dict) else 0,
                'test_coverage': test_result.get('coverage', 0) if isinstance(test_result, dict) else 0,
                'complexity': complexity_result.get('cyclomatic_complexity', 0) if isinstance(complexity_result, dict) else 0
            }
        
        builder = PipelineBuilder("code_quality", "Code Quality Pipeline", workspace_root)
        ...
        builder.add_step("generate_quality_report", "Generate Quality Report", 
                        generate_quality_report, depends_on=["lint_code", "run_tests", "analyze_complexity"])
