...
async def test_cache_thread_safety():
    ...
    assert success_count == 10, f"Expected all operations to succeed, got {success_count}/10"
...
def test_error_handling():
    ...
    try:
        ...
        assert deps._LOG_THROTTLE_SECONDS == 300, f"Expected 300, got {deps._LOG_THROTTLE_SECONDS}"
        print("✓ Invalid LOG_THROTTLE_SECONDS handled correctly")
        
        # Test with valid value
        os.environ['LOG_THROTTLE_SECONDS'] = '60'
        importlib.reload(deps)
        assert deps._LOG_THROTTLE_SECONDS == 60, f"Expected 60, got {deps._LOG_THROTTLE_SECONDS}"
...
def test_log_throttling_cleanup():
    ...
    old_time = time.time() - (3 * _LOG_THROTTLE_SECONDS)  # Older than 2x throttle period
    ...
    assert 'recent_message' in deps._log_timestamps, "Recent entries should be preserved"
