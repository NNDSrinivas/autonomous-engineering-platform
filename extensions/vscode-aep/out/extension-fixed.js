        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            try {
                switch (msg.type) {
                    ...
                    case 'openConnectors': {
                        vscode.window.showInformationMessage('Connect your repos & tools – coming soon!');
                        break;
                    }
                    case 'openSettings': {
                        // Open the AEP NAVI settings section
                        vscode.commands.executeCommand('workbench.action.openSettings', '@ext:navralabs.aep-professional navi backend');
                        break;
                    }
                    case 'modelChanged': {
...
async function callNaviBackend(opts) {
    ...
    const backendUrl = config.get('navi.backendUrl') ||
