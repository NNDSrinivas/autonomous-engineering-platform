    ...
    handleToolbar(action, webview) {
        switch (action) {
            ...
            case 'new':
                ...
                break;
            case 'focus':
                vscode.window.showInformationMessage('Focus on current file/selection will be wired up soon.');
                break;
            case 'settings':
                vscode.commands.executeCommand('workbench.action.openSettings', 'aep');
                break;
            case 'connect':
                vscode.window.showInformationMessage('Connections hub (Slack, Jira, GitHub, MCP servers, etc.) will live here.');
                break;
        }
    }
    getHtml(webview) {
        ...
        const mascotUri = webview.asWebviewUri(vscode.Uri.joinPath(mediaRoot, 'navi-fox.svg'));
