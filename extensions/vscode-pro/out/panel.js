...
class AepPanel {
    ...
    constructor(ctx) {
        this.ctx = ctx;
        this.jiraClient = new services_1.JiraClient(ctx);
        this.githubClient = new services_1.GithubClient();
        this.history = new history_1.HistoryStore(ctx);
        this.panel = vscode.window.createWebviewPanel('aepPro', 'AEP Professional', vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        this.panel.webview.onDidReceiveMessage(async (msg) => {
            try {
                switch (msg.type) {
                    case 'signin':
                        await (0, auth_1.signIn)(this.ctx);
    ...
    async addMcp(name) {
        const mcps = this.ctx.globalState.get('aep.mcp', []);
    ...
    async sendInitialState() {
        ...
        const mcp = this.ctx.globalState.get('aep.mcp', []);
    ...
    async render() {
        ...
        const model = this.ctx.globalState.get('aep.model', 'gpt-4o');
        const mcp = this.ctx.globalState.get('aep.mcp', []);
        const isSignedIn = await (0, auth_1.getAccessToken)(this.ctx) !== undefined;
    ...
    getHtml(model, mcp, isSignedIn, jiraConnected, githubConnected, issues, prs, sessions) {
    ...
    <body>
      <div class="wrap">
        <div class="row">
          ...
          <select id="model">
            ${['gpt-4o', 'gpt-4o-mini', 'claude-3-5-sonnet', 'gemini-1.5-pro'].map(m => `<option value="${m}" ${m === model ? 'selected' : ''}>${m}</option>`).join('')}
          </select>
          ${signInButton}
