...
export class HomePanel implements vscode.WebviewViewProvider {
    public static readonly viewType = 'aep.professional.home';
    ...
    private state(): AepState {
        const s = this.ctx.globalState.get<AepState>('aep.state');
        return s ?? {
            model: 'gpt-4o',
            apiKeySet: false,
            signedIn: false,
    ...
    private async save(partial: Partial<AepState>) {
        const next = { ...this.state(), ...partial };
        await this.ctx.globalState.update('aep.state', next);
    }

    private html(state: AepState) {
    ...
    private resourceUri(rel: string) {
        return this.view?.webview.asWebviewUri(
            vscode.Uri.joinPath(this.ctx.extensionUri, rel)
        );
    }
}