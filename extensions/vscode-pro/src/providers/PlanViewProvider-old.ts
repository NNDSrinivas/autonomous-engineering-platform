...
export interface Plan {
    ...
    description: string;
    steps: PlanStep[];
    status: 'draft' | 'reviewing' | 'executing' | 'completed' | 'cancelled';
}

export class PlanViewProvider implements vscode.WebviewViewProvider {
    public static readonly viewType = 'aep.planView';
    ...
    public async approveCurrentStep(): Promise<void> {
    ...
    public async rejectCurrentStep(): Promise<void> {
        // This would need to be enhanced to track which step is "current"
        // For now, we'll just trigger a refresh to show available actions
        await this.refreshPlan();
