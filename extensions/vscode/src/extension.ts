...
import { greet, fetchContextPack, proposePlan } from '../../../agent-core/dist/runtime.js';
import { checkPolicy } from '../../../agent-core/dist/policy.js';
import { applyEdits, runCommand } from '../../../agent-core/dist/tools.js';
...
<meta charset="utf-8">
<style>
  body { font: 12px/1.4 system-ui, -apple-system, Segoe UI, Roboto, sans-serif; padding: 12px; }
  .card { border:1px solid #ddd; border-radius:8px; padding:12px; margin-bottom:8px; background:#fff; }
  .row { display:flex; gap:8px; align-items:center; margin:6px 0; }
  .btn { border:1px solid #ccc; border-radius:6px; padding:6px 10px; background:#f7f7f7; cursor:pointer;}
  .btn.primary { background:#0ea5e9; color:#fff; border-color:#0ea5e9;}
...
<body>
  ...
  <script>
    const vscode = acquireVsCodeApi();
    function log(html){ const d=document.createElement('div'); d.className='card'; d.innerHTML=html; document.getElementById('log').prepend(d); }

    window.addEventListener('message', ev => {
      const {type, payload} = ev.data;
      if(type==='message'){
        const items = payload.tasks?.map(t=>\`<li><button class="btn" onclick="ticket('\${t.key}')">\${t.key}</button> \${t.title} <code>\${t.status}</code></li>\`).join('')||'';
        log(\`<b>\${payload.text}</b><ul>\${items}</ul>\`);
      }
      if(type==='context.pack'){
        log('<b>Context Pack</b><pre>'+JSON.stringify(payload,null,2)+'</pre>');
    ...
    function approve(){ vscode.postMessage({type:'plan.approve', plan: window.currentPlan}); }
    function ticket(key){ vscode.postMessage({type:'ticket.select', key}); }
    vscode.postMessage({type:'session.open'});
  </script>
</body></html>`;
}
