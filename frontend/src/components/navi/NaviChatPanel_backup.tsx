...
export interface ChatMessage {
  ...
  role: ChatRole;
  content: string;
  responseData?: NaviChatResponse; // Store full response for reviews, actions, etc.
}
...
interface NaviChatResponse {
  ...
  // backend diagnostic state – we treat repo_fast_path specially
  state?: {
...
// Attachment interface moved to AttachmentChips.tsx
...
// VS Code API is now handled by vscodeApi utility
...
export default function NaviChatPanel() {
   ...
   * - Prefer WorkspaceContext values
   * - Fall back to ?workspaceRoot= query param from VS Code iframe URL
   */
  const getEffectiveWorkspace = () => {
    ...
    if ((!effectiveRepoName || effectiveRepoName === "current") && effectiveRoot) {
  ...
  useEffect(() => {
    ...
    return vscodeApi.onMessage((msg) => {
      ...
      if (msg.type === 'addAttachment' && msg.attachment) {
        const att = msg.attachment as AttachmentChipData & { content?: string };
  ...
  const sendNaviChatRequest = async (
    ...
    // If no effective root from context, request it from VS Code
    if (!effectiveRoot) {
      vscodeApi.postMessage({ type: "getWorkspaceRoot" });
    }
  ...
  const buildAssistantReply = (
    ...
    // Prefer backend repo_name, then UI-derived repoName, then a generic label.
    const name =
      backendRepoName ||
    ...
    const root =
      backendRepoRoot ||
      effectiveRoot ||
      ""; // for optional inclusion in the text

    // 1) "which repo / where are we?" – OR backend marks repo_fast_path.where
    ...
    // 2) "explain this repo / project" – Let backend handle this with dynamic analysis
    if (isExplainRepoQuestion || repoFastPathExplain) {
      const backendCandidate = (data.content || data.reply || "").trim();

      // Always use backend response for explain questions (it now does dynamic analysis)
      if (backendCandidate) {
        return backendCandidate;
      }

      // Fallback only if backend gave no response
    ...
    // 3) For all other questions, prefer backend content if present
    ...
    const isInspectRepo =
      intentKind.includes("inspect_repo") || titleLower.includes("inspect_repo");

    // 4) inspect_repo – explain what Navi is going to do
    ...
    // 5) generic "explain project" questions
    if (
      lowerUser.includes("explain") &&
  ...
  const handleSend = async (overrideText?: string) => {
    ...
    try {
      // Route through VS Code extension for proper message handling (repo questions, auto-context, etc.)
      console.log('[NAVI] 🚀 ROUTING MESSAGE THROUGH VS CODE EXTENSION:', text);
      ...
      setAttachments([]);
      // The extension will handle the response via message listeners
      return;

      console.log('[NAVI] ⚠️ NO VS CODE CONTEXT - FALLING BACK TO DIRECT API CALL');

      // Fallback to direct backend call if not in VS Code context
      const data = await sendNaviChatRequest(text);
      const replyText = buildAssistantReply(data, text);
      console.log("[NAVI Chat] Final reply:", replyText);
      console.log("[NAVI Chat] Reply type:", typeof replyText);
      console.log("[NAVI Chat] Reply is string:", typeof replyText === 'string');

      // Add progress information to reply if available
      ...
      const assistantMessage: ChatMessage = {
        ...
        responseData: data, // Store full response data for reviews, actions, etc.
      };
  ...
  /* ---------- attach handlers (icon) ---------- */

  // Old attachment functions removed - now handled by AttachmentToolbar
  ...
  /* ---------- per-message actions: copy / edit / undo / redo ---------- */
  ...
  const handleCopyMessage = async (msg: ChatMessage) => {
    try {
      if (navigator.clipboard?.writeText) {
        ...
        showToast(
          "Copy not supported here – you can still select text manually.",
          "warning"
        );
      }
    } catch (err) {
  ...
  // "Undo" here = remove this bubble from the transcript
  const handleUndoMessage = (msg: ChatMessage) => {
    setMessages((prev) => prev.filter((m) => m.id !== msg.id));
  };

  // "Redo" here = resend the same text as a new user message
  ...
  return (
    <div className="navi-chat-root">
        ...
        {messages.map((m) => (
          ...
          >
            ...
            <div className={`navi-chat-bubble navi-chat-bubble--${m.role}`}>
              ...
              {/* Reviews display and Apply All Fixes button */}
              {m.role === "assistant" && m.responseData?.reviews && m.responseData.reviews.length > 0 && (
                <div className="navi-reviews-section" style={{ marginTop: '16px', padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '6px' }}>
                  <h4 style={{ margin: '0 0 8px 0', fontSize: '14px', fontWeight: 600 }}>Code Review ({m.responseData.reviews.length} issues)</h4>
                  <div style={{ marginBottom: '12px' }}>
                    {m.responseData.reviews.slice(0, 3).map((review, idx) => (
                      <div key={idx} style={{ marginBottom: '8px', fontSize: '13px' }}>
                        <strong>{review.path}</strong>{review.line && ` (line ${review.line})`}: {review.summary}
                      </div>
                    ))}
                    {m.responseData.reviews.length > 3 && (
                      <div style={{ fontSize: '13px', color: '#666' }}>...and {m.responseData.reviews.length - 3} more issues</div>
                    )}
                  </div>
                  <button
                    type="button"
                    className="navi-pill navi-pill--primary"
                    onClick={() => handleApplyAllFixes(m.responseData!.reviews!)}
            ...
            <div className="navi-chat-bubble navi-chat-bubble--assistant">
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ fontStyle: 'italic', color: '#666' }}>
                  NAVI is working on your request...
                </span>
              </div>
            </div>
          </div>
      ...
      {/* Quick actions */}
      <QuickActionsBar className="mb-2" />

      {/* Attachment tools + chips, just above the input box */}
      <AttachmentToolbar className="mb-1" />
      <AttachmentChips attachments={attachments} />

      {/* Input row with attach icon */}
      <div className="navi-chat-input-row">
        {/* Removed old attach button - now using AttachmentToolbar */}
      ...
      <div className="navi-chat-mode-row">
        ...
        >
          {(Object.keys(PROVIDER_LABELS) as ProviderId[]).map((key) => (
            <option key={key} value={key}>
              {PROVIDER_LABELS[key]}
            </option>
          ))}
