...
def check_failing_builds():
    ...
    success, output, error = run_command(
        "gh pr checks 54 --json name,status,conclusion"
    )
    if not success:
...
def get_new_comments():
    ...
    try:
        ...
        for comment in data.get("comments", []):
            ...
            if (
                "CodeQL" in comment.get("body", "")
                or "failed" in comment.get("body", "").lower()
            ):
...
def commit_and_push_fixes():
    ...
    if output.strip():
        ...
        success, output, error = run_command(
            "git push origin feature/navi-execution-controls"
        )
        if success:
