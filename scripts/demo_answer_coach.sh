...
# Replace YOUR-PROJECT-123 with your actual project issue key before running
post '{"text":"We worked on YOUR-PROJECT-123 and fixed the retry loop."}'
post '{"text":"JWT expiry is parsed in auth layer per last sprint."}'
post '{"text":"Where is the JWT expiry parsing logic located now?"}'
echo ""
