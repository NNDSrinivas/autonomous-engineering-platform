...
"""
    ...
    if summary["latency_p95"] > 3000:
        recommendations.append(
            "⚠️ **High p95 latency detected.** Consider implementing response caching for common queries."
        )

    if summary["error_rate"] > 2:
        recommendations.append(
            "⚠️ **Error rate above 2%.** Investigate failed requests and add retry logic."
        )

    avg_cost = summary["total_cost"] / summary["total_tests"]
    if avg_cost > 0.03:
        recommendations.append(
            f"💰 **High average cost (${avg_cost:.4f} per request).** Consider using prompt optimization or caching."
        )

    if summary["throughput_rps"] < 0.5:
        recommendations.append(
            "🐌 **Low throughput detected.** Consider implementing concurrent request processing."
        )

    if not recommendations:
        recommendations.append(
            "✅ **All metrics look good!** System is ready for production deployment."
        )
