...
def seed_edges(
    session, org_id: str, edges: List[Dict[str, Any]], foreign_id_map: Dict[str, int]
):
...
def clear_existing_data(session, org_id: str):
    ...
    edges_deleted = (
        session.query(MemoryEdge).filter(MemoryEdge.org_id == org_id).delete()
    )
    nodes_deleted = (
        session.query(MemoryNode).filter(MemoryNode.org_id == org_id).delete()
    )
...
def verify_seeded_data(session, org_id: str, expected: Dict[str, Any]):
    ...
    print(
        f"  Nodes: {node_count} (expected: {expected_nodes}) {'✓' if node_count == expected_nodes else '✗'}"
    )
    print(
        f"  Edges: {edge_count} (expected: {expected_edges}) {'✓' if edge_count == expected_edges else '✗'}"
    )
...
def main():
    ...
    try:
        ...
        success = verify_seeded_data(
            session, org_id, fixture.get("expected_metrics", {})
        )
