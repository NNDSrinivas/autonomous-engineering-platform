...
def main():
    ...
    if not success:
        ...
        print("cd autonomous-engineering-platform && source .venv/bin/activate && PYTHONPATH=. python -m backend.api.main")
        return 1
    
    # Test 2: AI Assistant (will work with API key)
    test_endpoint("AI Assistant - Code Question", f"{base_url}/api/ask", "POST", {
        "question": "What's the difference between async and sync programming in Python?",
        "context": {"language": "python", "topic": "concurrency"}
    })
    
    # Test 3: Code Analysis
    test_endpoint("Code Analysis", f"{base_url}/api/analyze-code", "POST", {
        "code": """
...
""",
    ...
    test_endpoint("Team Context Search", f"{base_url}/api/team-context", "POST", {
