                    ...
                    try:
                        ...
                        if data.get("type") == "progress":
                            ...
                            if files:
                                ...
                                print(
                                    f"   Quality Score: {sample.get('quality_score', 'N/A')}/10"
                                )
                                print(
                                    f"   Issues Found: {len(sample.get('issues', []))}"
                                )
                                ...
                                if issues:
                                    issue = issues[0]
                                    print(
                                        f"   Sample Issue: {issue.get('type', 'N/A')} - {issue.get('message', 'N/A')[:80]}..."
                                    )
                            ...
                            print(
                                f"❌ Analysis Error: {data.get('message', 'Unknown error')}"
                            )
                            return False
