...
def test_analyze_endpoint():
    ...
    workspace_root = (
        "/Users/mounikakapa/Desktop/Personal Projects/autonomous-engineering-platform"
    )
    ...
    try:
        ...
        response = requests.post(
            url, headers=headers, params=params, stream=True, timeout=15
        )
                        ...
                        if data.get("type") == "review":
                            ...
                            if "files" in payload_data:
                                ...
                                for file_info in files[:2]:  # Show first 2 files
                                    ...
                                    print(
                                        f"  📄 {file_info.get('path')}: {len(issues)} issues"
                                    )
                                    if issues:
                                        print(
                                            f"    Sample issue: {issues[0].get('title', 'No title')}"
                                        )
                                success = True
                                break
