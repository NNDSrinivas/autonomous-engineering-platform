...
from backend.auth.auth0 import require_auth, require_role, _fetch_jwks, _verify_signature
...
class TestRoleBasedAccess:
    ...
    async def test_role_validation_with_permissions(self):
        ...
        roles = [p.replace("role:", "") for p in claims.get("permissions", []) 
...
class TestIntegrationWithAuth0:
    ...
    async def test_auth0_domain_reachability(self):
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                response = await client.get("https://auth.navralabs.com/.well-known/openid-configuration")
                assert response.status_code == 200
    ...
    async def test_jwks_endpoint_reachability(self):
        ...
        try:
            async with httpx.AsyncClient(timeout=5.0) as client:
                response = await client.get("https://auth.navralabs.com/.well-known/jwks.json")
                assert response.status_code == 200
                jwks = response.json()
                assert "keys" in jwks
