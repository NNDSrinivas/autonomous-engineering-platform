...
async def test_set_get_invalidate_roundtrip():
    ...
    with (
        patch.multiple("time", time=lambda: 1000.0),
        patch.multiple("backend.infra.cache.redis_cache.time", time=lambda: 1000.0),
    ...
    with (
        patch.multiple("time", time=lambda: 1001.1),
        patch.multiple("backend.infra.cache.redis_cache.time", time=lambda: 1001.1),
