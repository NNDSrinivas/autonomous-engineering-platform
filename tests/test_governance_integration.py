...
class TestGovernanceIntegration:
    ...
    async def test_approval_execution_flow(
        ...
        with patch.object(
            governed_orchestrator.execution_controller, "execute_action"
        ) as mock_execute:
    ...
    async def test_governed_execution_controller(
        self, governed_execution_controller, sample_action, sample_context
    ):
        """Test the governed execution controller directly"""

        # Mock governance components
        with patch.object(
            governed_execution_controller.approval_engine, "evaluate_action"
        ) as mock_evaluate:
            with patch.object(
    ...
    async def test_rollback_capability(
        self, governed_execution_controller, sample_action
    ):
        """Test rollback functionality"""

        # Mock rollback controller
        with patch.object(
            governed_execution_controller.rollback_controller, "rollback_action"
        ) as mock_rollback:
    ...
    async def test_safety_checks(
        ...
        with patch.object(
            governed_execution_controller, "_run_safety_checks"
        ) as mock_safety:
    ...
    def test_risk_detection_methods(self, governed_execution_controller, sample_action):
        ...
        assert (
            governed_execution_controller._detect_auth_impact(
                sample_action, auth_context
            )
            is True
        )
        ...
        assert (
            governed_execution_controller._detect_auth_impact(
                sample_action, non_auth_context
            )
            is False
        )
    ...
    def test_rollback_support_detection(self, governed_execution_controller):
        ...
        assert (
            governed_execution_controller._supports_rollback(rollback_supported_action)
            is True
        )
        assert (
            governed_execution_controller._supports_rollback(rollback_forbidden_action)
            is False
        )
...
class TestGovernancePerformance:
    ...
    async def test_concurrent_governance_decisions(self, mock_db_session):
        ...
        orchestrator = GovernedClosedLoopOrchestrator(
            db_session=mock_db_session, user_id="test_user"
        )
        ...
        with patch.object(
            orchestrator.approval_engine, "evaluate_action"
        ) as mock_evaluate:
            ...
            with patch.object(orchestrator, "_execute_with_governance") as mock_execute:
                ...
                assert all(
                    result.status == ExecutionStatus.COMPLETED for result in results
                )
