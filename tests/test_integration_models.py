...
class TestSlackConnection:
    ...
    def test_bot_token_encrypted_in_database(self, db_session):
        ...
        conn = SlackConnection(id="slack-2", org_id="org-1", bot_token=plaintext_token)
        db_session.add(conn)
        db_session.commit()
    ...
    def test_bot_token_decrypts_on_access(self, db_session):
        ...
        conn = SlackConnection(id="slack-3", org_id="org-1", bot_token=plaintext_token)
        db_session.add(conn)
        db_session.commit()
    ...
    def test_bot_token_update(self, db_session):
        ...
        conn = SlackConnection(id="slack-5", org_id="org-1", bot_token="original-token")
        db_session.add(conn)
        db_session.commit()
    ...
    def test_bot_token_none_handling(self, db_session):
        ...
        conn = SlackConnection(id="slack-6", org_id="org-1", bot_token=None)
        db_session.add(conn)
        db_session.commit()
    ...
    def test_slack_connection_fails_without_encryption_key(self):
        ...
        try:
            ...
            with pytest.raises(EncryptionKeyError):
                SlackConnection(
                    id="slack-no-key", org_id="org-1", bot_token="test-token"
                )
