...
async def send_request(session, message, attachments=None, mode="agent"):
    ...
    try:
        ...
        return {"error": str(e), "status": 0, "content": "", "thinking": "", "actions": [], "activities": []}
...
async def run_tests():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        result = await send_request(
            session,
            "What kind of project is this? What technologies does it use?"
        )
...
""",
        ...
        code_analyzed = any(k in combined for k in ["keyerror", "index", "exist", "none", "missing", "valid", "check"])
...
What's wrong and how do I fix it?"""
        ...
        meaningful, reason = is_meaningful_response(result)
        combined = (result["content"] + result["thinking"]).lower()
        error_understood = any(k in combined for k in ["none", "null", "subscript", "check", "if ", "get("])
        ...
        has_impl = len(result["actions"]) > 0 or "def " in result["content"] or "class " in result["content"] or "```" in result["content"]
        if meaningful and not has_impl:
            # Also check thinking for implementation details
            has_impl = "def " in result["thinking"] or "class " in result["thinking"] or "```" in result["thinking"]
        ...
        meaningful, reason = is_meaningful_response(result)
        combined = (result["content"] + result["thinking"]).lower()
        has_commands = any(k in combined for k in ["npm", "pip", "python", "node", "install", "run", "start", "uvicorn"])
        ...
        meaningful, reason = is_meaningful_response(result)
        combined = (result["content"] + result["thinking"]).lower()
        has_suggestions = any(k in combined for k in ["cache", "stream", "async", "timeout", "parallel", "batch", "model"])
...
""",
        ...
        meaningful, reason = is_meaningful_response(result, min_content=30)
        combined = (result["content"] + result["thinking"]).lower()
        found_issue = any(k in combined for k in ["hardcoded", "secret", "environment", "env", "expose", "leak", "commit"])
        ...
        handled = any(k in combined for k in ["what", "which", "specify", "clarify", "help", "improve", "better"])
