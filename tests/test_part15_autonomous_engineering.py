...
class TestSprintPlannerAgent:
    ...
    async def sprint_planner(self):
        ...
        with patch('backend.agents.sprint_planner_agent.LLMRouter'), \
    ...
    async def test_adjust_sprint_scope(self, sprint_planner):
        ...
        mock_response.text = json.dumps({
            "recommended_actions": [
                {"action": "remove_item", "item_id": "item3", "reason": "Low priority"},
...
class TestBacklogManagerAgent:
    ...
    async def backlog_manager(self):
        ...
        with patch('backend.agents.backlog_manager_agent.LLMRouter'), \
    ...
    async def test_analyze_backlog(self, backlog_manager):
        ...
        mock_response.text = json.dumps({
            "priority_recommendations": [
                {"item_id": "item1", "new_priority": "critical", "reasoning": "Blocks user access"},
                {"item_id": "item2", "new_priority": "medium", "reasoning": "Enhancement request"}
            ],
    ...
    async def test_rank_backlog_intelligent(self, backlog_manager):
        ...
        backlog_items = [
            BacklogItem(id="item1", title="Bug A", item_type=ItemType.BUG, priority=Priority.HIGH, estimated_hours=2),
            BacklogItem(id="item2", title="Feature B", item_type=ItemType.FEATURE, priority=Priority.LOW, estimated_hours=8),
            BacklogItem(id="item3", title="Critical C", item_type=ItemType.BUG, priority=Priority.CRITICAL, estimated_hours=1)
        ]
    ...
    async def test_generate_team_performance_report(self, kpi_engine):
        ...
        with patch.object(kpi_engine, 'llm') as mock_llm:
            ...
            mock_response.text = json.dumps({
                ...
                "recommendations": ["Implement better monitoring", "Optimize review process"]
            })
...
class TestMemoryLayer:
    ...
    async def memory_layer(self):
        ...
        with patch('backend.memory.memory_layer.DatabaseService'), \
    ...
    async def test_store_memory(self, memory_layer):
        ...
        memory_layer._enhance_memory_content = AsyncMock(return_value={
            ...
            mock_response.text = json.dumps([
                {
                    ...
                    "confidence": 0.85,
                    "actionable_recommendations": ["Review auth implementation", "Add more tests"]
                }
            ])
            mock_llm.run.return_value = mock_response
            
            insights = await memory_layer.generate_insights("current authentication issues")
...
class TestMultiRepoOrchestrator:
    ...
    async def orchestrator(self):
        ...
        with patch('backend.agents.multi_repo_orchestrator.DatabaseService'), \
    ...
    async def test_register_repository(self, orchestrator):
        ...
        orchestrator._analyze_repository_structure = AsyncMock(return_value={
    ...
    async def test_plan_multi_repo_operation(self, orchestrator):
        ...
        orchestrator._estimate_operation_duration = AsyncMock(return_value=timedelta(hours=12))
        orchestrator._generate_rollback_plan = AsyncMock(return_value="Rollback plan...")
...
class TestAutonomousPRReviewer:
    ...
    async def pr_reviewer(self):
        ...
        with patch('backend.agents.autonomous_pr_reviewer.DatabaseService'), \
    ...
    async def test_security_pattern_detection(self, pr_reviewer):
        ...
        assert len(issues) > 0
        sql_injection_issues = [i for i in issues if i.vulnerability_type == "sql_injection"]
    ...
    async def test_generate_review_summary(self, pr_reviewer):
        ...
        from backend.agents.autonomous_pr_reviewer import PRAnalysis, FileChange, FileChangeType
