...
def test_cross_org_forbidden_node_access(api_client: Client, other_org_client: Client, seeded_graph):
...
def test_org_scoped_rebuild(api_client: Client, other_org_client: Client, seeded_graph):
    ...
    response_default = api_client.post(
        ...
        json={"org_id": TEST_ORG_ID, "since": "7d"}
    )
    
    # Should succeed for same org
    assert response_default.status_code in (200, 202), \
    ...
    response_cross = api_client.post(
        "/api/memory/graph/rebuild",
        json={"org_id": "other_org", "since": "7d"}
    )
    
    # Should block or validate org_id matches X-Org-Id
    assert response_cross.status_code in (200, 202, 400, 403), \
...
def test_data_isolation_between_orgs(test_db, seeded_graph):
    ...
    default_node_ids = {
        node.id for node in 
        test_db.query(MemoryNode).filter(MemoryNode.org_id == TEST_ORG_ID).all()
    }
...
def test_node_foreign_id_can_duplicate_across_orgs(test_db, seeded_graph):
    ...
    # Verify both exist
    default_eng102 = test_db.query(MemoryNode).filter(
        MemoryNode.org_id == TEST_ORG_ID,
        MemoryNode.foreign_id == "ENG-102"
    ).first()
    
    other_eng102 = test_db.query(MemoryNode).filter(
        MemoryNode.org_id == "other_org",
        MemoryNode.foreign_id == "ENG-102"
    ).first()
